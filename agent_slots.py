...
def _time_words(t: _time) -> str:
    """Время словами БЕЗ предлога: «9 утра», «полдень», «4 часа дня».

    Отдельно от spoken_time, потому что предлог у времени бывает разный:
    «в 4 часа дня», но «с 9 утра до 5 часов дня». Склеивать строку
    «в …» и потом отрезать первые два символа — тот вид экономии, который
    ломается на первом же исключении.
    """
    if t.minute:
    ...
    if 5 <= ч <= 11:
        # «10 утра», а не «10 часов утра»: так говорят вслух.
        return f"{циферблат} утра"
    if 18 <= ч <= 22:
        return f"{циферблат} вечера"

    часть_суток = "дня" if 13 <= ч <= 17 else "ночи"   # ночь — это 23 и 1-4
    if циферблат == 1:
        return f"час {часть_суток}"
    return f"{циферблат} {_hours_word(циферблат)} {часть_суток}"


def _genitive(слова: str) -> str:
    """Родительный падеж для оборота «до …»: «до полудня», «до часа дня».

    Нужен ровно трём формам из всех: остальные в этом обороте не меняются
    («до 5 часов дня» верно как есть).
    """
    if слова == "полдень":
        return "полудня"
    if слова == "полночь":
        return "полуночи"
    if слова.startswith("час "):
        return "часа " + слова[len("час "):]
    return слова


def spoken_hours_range() -> str:
    """Часы приёма фразой: «с 9 утра до 5 часов дня».

    Собирается из тех же констант, что и сама проверка времени. Написать
    текст руками было бы короче ровно до того дня, когда границы подвинут,
    а фраза останется старой и начнёт врать вслух — молча, потому что
    строка синтаксически верна.
    """
    начало = _time_words(_time(hour=WORK_START_HOUR))
    конец = _genitive(_time_words(_time(hour=WORK_END_HOUR)))
    return f"с {начало} до {конец}"
...
def check_slot(request: Request, body: CheckRequest) -> Dict[str, Any]:
    ...
    if not is_working_hour(t):
        if t.minute != 0:
            # Пример берётся из самого диапазона, а не пишется руками: три
            # часа дня — то, что нужно сегодня, но если приёмные часы сдвинут,
            # пример обязан остаться внутри них, а не звать на несуществующее.
            пример = 15 if WORK_START_HOUR <= 15 <= WORK_END_HOUR else WORK_START_HOUR
            hint = ("встречи начинаются в начале часа, например, "
                    f"{spoken_time(_time(hour=пример))}")
        else:
            hint = f"встречи назначаем {spoken_hours_range()}"

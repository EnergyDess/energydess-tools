...
литералом не засчитывается: `собрано=1` — та же константа True. Ложный
литерал (`собрано=0`) законен: такой шаг всегда ПРОПУСК и OK не печатает.
...
def разобрать_текст(текст, файл="<память>"):
    ...
    for у in ast.walk(дерево):
        ...
        if not и:
            ...
            # ЛИТЕРАЛ ОПАСЕН, ТОЛЬКО ЕСЛИ ИСТИНЕН (письмо 4 задачи 343).
            # `собрано=1` — та же константа True: шаг не станет ПРОПУСКОМ
            # никогда. `собрано=0` обратен: шаг ВСЕГДА ПРОПУСК и напечатать
            # OK не может — это исход предусловия «мерить нечего», а не шаг,
            # проходящий без замера. Первая версия брала любой литерал
            # и объявила находкой `check_portfolio.py:764` (подлог кадра
            # при пустом томе), которая молчать не умеет: код 2.
            if собрано is not None and isinstance(собрано, ast.Constant) and собрано.value:
...
def _контроль():
    ...
    случаи = [
        ...
        ("ОБРАТНЫЙ: собрано=0 литералом — всегда ПРОПУСК",
         "def ф(x):\n    if not x:\n        о.шаг('а', False, собрано=0)\n        return\n"
         "    о.шаг('а', x.y > 1)\n", False),

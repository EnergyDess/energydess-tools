...
def _поднять_приложение(порт, база_копия):
    env = dict(os.environ)
    # ЗАМЕР ИДЁТ НА КОПИИ БАЗЫ СТЕНДА (заход 333, задача 332). Прежде здесь
    # стоял жёсткий путь к `app.db`: замер чистил переписку аптечки
    # и дописывал воду прямо на стенде, мимо `DB_PATH`, — сторож стенда
    # проверки 35 назвал пропажу 8 реплик и 4 файлов. Вопрос замера —
    # ждёт ли сосед блокировку — к содержимому стенда отношения не имеет
    env["DB_PATH"] = база_копия
    # ЗАНЯТЫЙ ПОРТ — ОТКАЗ, А НЕ «ПРИЛОЖЕНИЕ ПОДНЯЛОСЬ». Проверка ниже
    # ждёт, пока порт станет занят, и чужой процесс на нём проходил её
    # сразу: замер разговаривал с сиротой прежнего прогона, сидевшим
    # на базе стенда (заход 333)
...
def _один_прогон():
    ...
    import sqlite3
    import tempfile
    каталог = tempfile.mkdtemp(prefix="dbhold_live_")
    копия = os.path.join(каталог, "app.db")
    исх = sqlite3.connect(os.environ.get("DB_PATH") or os.path.join(КОРЕНЬ, "app.db"))
    цель = sqlite3.connect(копия)
    исх.backup(цель)
    цель.close()
    исх.close()
    try:
        п, журнал = _поднять_приложение(ПОРТ_ПРИЛОЖЕНИЯ, копия)
    except Exception:

...
ЧТО СПРАШИВАЕТСЯ:
  ...
  · колонок сетки 4 / 3 / 1 на 1600 / 1280 / 390 — решение владельца;
...
# Колонок на каждой ширине — решение владельца (письмо «enshrouded-1»,
# блок 2.1): «не во всю ширину, но свободнее, чем сейчас».
КОЛОНОК = {1600: 4, 1280: 3, 390: 1}
...
def прогон(база, подлог=None, ширины=None):
    ...
    плохо = [(ш, з["колонок"]) for ш, з in замеры
             if з["колонок"] != КОЛОНОК.get(ш, з["колонок"])]
    шаг("колонок-по-решению", not плохо,
        "; ".join("%d → %d (ждём %d)" % (ш, к, КОЛОНОК[ш]) for ш, к in плохо)
        or "; ".join("%d → %d" % (ш, з["колонок"]) for ш, з in замеры),

...
#
# ПОРОГИ БЕРУТСЯ У `check_v2_wide` ИМПОРТОМ: второй список порогов
# разошёлся бы с первым молча (§6.0.7), а он же повторяет `@container`
# в `static/enshrouded.css`.
from check_v2_wide import ожидаем_колонок  # noqa: E402
...
() => {
  ...
  // ПОЛОСА ПОИСКА — ОДНА РОЛЬ. Счётчики уехали в шапку (№352, письмо
  // «широкий режим везде», блок 2.1), и полоса обязана нести только
  // поиск: до правки она отвечала сразу на «чем сузить список»
  // и «сколько собрано», а у остальных инструментов на этом месте
  // кнопки действий. Спрашивается ТРИ вещи: счётчиков в полосе нет,
  // счётчики есть в группе действий шапки, перелива ряда нет.
  // Их ЦЕНТРОВКУ по строке h1 спрашивает проверка 56 — тем же шагом,
  // что у кнопок остальных инструментов, второго замера не заводим.
  ...
  const полоса = пс && рд ? {
    счётчиков_в_полосе: рд.querySelectorAll('.ens-stats').length,
    счётчиков_в_шапке: document.querySelectorAll('.v2-page-head .v2-head-actions .ens-stats').length,
    перелив: +(рд.scrollWidth - рд.clientWidth).toFixed(1),
    за_краем: +(кор(пс).правый - document.documentElement.clientWidth).toFixed(1),
  } : null;
...
def прогон(база, подлог=None, ширины=None):
    ...
    # ПОЛОСА НЕСЁТ ТОЛЬКО ПОИСК, СЧЁТЧИКИ — В ШАПКЕ (письмо «широкий
    # режим везде», блок 2.1).
    #
    # ЧЕТЫРЕ ПУНКТА §6.0.3 — ШАГ ПЕРЕФОРМУЛИРОВАН.
    #  · ПРЕЖНЯЯ ФОРМУЛИРОВКА: «на 390 счётчики стоят отдельной строкой,
    #    на десктопе — в одной строке с полем поиска; перелива нет».
    #  · НОВАЯ: «в полосе счётчиков нет вовсе, они лежат в группе
    #    действий шапки; перелива нет».
    #  · ПОЧЕМУ ПРЕЖНЯЯ СТАЛА НЕГОДНОЙ: решением владельца счётчики
    #    переехали в строку заголовка, на место, где у остальных
    #    инструментов кнопки действий. Требование «в одной строке
    #    с поиском» стало печатать находку про исправное — замер после
    #    переезда: `в_одной_строке False` на 1600 и 1280 при нуле
    #    перелива.
    #  · ОТРИЦАТЕЛЬНЫЙ КОНТРОЛЬ НА НОВОЙ: подлог возвращает счётчики
    #    в полосу (переносит узел), и шаг обязан упасть — `--контроль`,
    #    подлог «счётчики вернулись в полосу».
    полосы = [(ш, з["полоса"]) for ш, з in замеры if з["полоса"]]
    сломано = [(ш, п) for ш, п in полосы
               if п["перелив"] > 0 or п["за_краем"] > 0
               or п["счётчиков_в_полосе"] or not п["счётчиков_в_шапке"]]
    шаг("полоса-несёт-только-поиск", not сломано,
...
ПОДЛОГИ = [
    # СЧЁТЧИКИ ВЕРНУЛИСЬ В ПОЛОСУ — ровно то устройство, которое отменил
    # владелец. Подлог ПЕРЕНОСИТ УЗЕЛ, а не красит: шаг спрашивает, где
    # счётчики ЛЕЖАТ, и подмена стиля его не тронула бы вовсе.
    ("счётчики вернулись в полосу", "полоса-несёт-только-поиск", """
      document.addEventListener('DOMContentLoaded', () => {
        const сч = document.querySelector('.v2-page-head .ens-stats');
        const рд = document.querySelector('.ens-bar-row');
        if (сч && рд) рд.appendChild(сч);
      });
    """),

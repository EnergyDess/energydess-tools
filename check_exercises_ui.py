...
class Отчёт:
    ...
    def шаг(self, имя, ок, подробность="", собрано=None, отрицание=""):
...
def _прогон(подлог=None):
    ...
    with sync_playwright() as p:
        ...
        _пройти(стр, б, о)
        if not подлог:
            # ПУСТОТА ЗДЕСЬ И ЕСТЬ ИСКОМОЕ: ошибки копит слушатель
            # `pageerror`, и пустой список означает «страница не ругалась».
            # Что страница вообще открылась, сказали шаги `_пройти` выше.
            о.шаг("ошибок в консоли нет", not ошибки,
                  "; ".join(ошибки[:2]) if ошибки else "",
                  отрицание="пустой список ошибок и есть искомое")
        бр.close()
    return о

...
window.fetch = async (u, o) => {
  ...
  if (адрес.includes('/api/fetch-url')) {
    // №18: задержка и отказ загрузки ссылки ставятся пробой ДО вставки
    if (window.__ЗАДЕРЖКА_ССЫЛКИ) await new Promise(r => setTimeout(r, window.__ЗАДЕРЖКА_ССЫЛКИ));
    if (window.__ОТКАЗ_ССЫЛКИ)
      return new Response(JSON.stringify({error: 'Сайт вакансии не ответил'}),
...
# ── №18. «НАПИСАТЬ ПИСЬМО» ЖДЁТ ЗАГРУЗКУ ВАКАНСИИ ПО ССЫЛКЕ ─────────────
# Письмо «питание-2», блок 1.2. Вставили ссылку — вакансия грузится сама,
# и нажатие «Написать письмо» во время загрузки уводило в модель ОДНУ
# ССЫЛКУ: `текстВакансии()` отдаёт поле как есть, пока `fetchedText` пуст.
# Замер ДО — в BACKLOG №352. Спрашивается ТО, ЧТО УШЛО в запрос генерации
# (длина текста, совпадение со ссылкой), и вычисленное состояние кнопки,
# а не класс. Задержка загрузки — 3 с в подделке ответа.
КНОПКА_ПИСЬМА = """() => { const b = document.getElementById('generate-btn');
  return {disabled: b.disabled, busy: b.getAttribute('aria-busy'),
          текст: b.innerText.trim()}; }"""
СТАТУС = """() => { const s = document.getElementById('fetch-status');
  const st = getComputedStyle(s), r = s.getBoundingClientRect();
  return {видим: s.checkVisibility({checkOpacity: true, checkVisibilityCSS: true}) && r.height > 0,
          цвет: st.color, кегль: st.fontSize, текст: s.innerText.trim()}; }"""
УШЛО_В_ГЕНЕРАЦИЮ = ("window.__ЗАПРОСЫ.filter(з => з.адрес.includes('generate-letter'))"
                    ".map(з => JSON.parse(з.тело).job_text || '')")


def замечание_загрузки_ссылки(бр, подлог=None):
    ctx, стр = _страница(бр, подлог)
    стр.goto(ch.БАЗА + "/hh", wait_until="domcontentloaded")
    стр.wait_for_timeout(900)
    стр.evaluate("window.__ЗАДЕРЖКА_ССЫЛКИ = 3000; window.__ЗАПРОСЫ = []")
    стр.fill("#job-input", ССЫЛКА)
    стр.wait_for_timeout(700)
    кнопка = стр.evaluate(КНОПКА_ПИСЬМА)
    статус = стр.evaluate(СТАТУС)
    # Нажатие обычное, по органу, и горячая клавиша поля: обе дороги
    # обязаны упереться в одну блокировку
    стр.evaluate("document.getElementById('generate-btn').click()")
    стр.focus("#job-input")
    стр.keyboard.press("Control+Enter")
    стр.wait_for_timeout(300)
    во_время = стр.evaluate(УШЛО_В_ГЕНЕРАЦИЮ)
    шаг(18, "кнопка-письма-заблокирована-на-загрузке",
        кнопка["disabled"] and кнопка["busy"] == "true" and "Загружаю" in кнопка["текст"],
        "disabled %s, aria-busy %s, «%s»" % (кнопка["disabled"], кнопка["busy"], кнопка["текст"]))
    шаг(18, "генерация-не-уходит-во-время-загрузки", not во_время,
        "ушло запросов %d, длины %s" % (len(во_время), [len(т) for т in во_время]))
    шаг(18, "статус-загрузки-заметен", статус["видим"] and статус["кегль"] != "13px",
        "видим %s, цвет %s, кегль %s" % (статус["видим"], статус["цвет"], статус["кегль"]))
    стр.wait_for_timeout(3200)
    после = стр.evaluate(КНОПКА_ПИСЬМА)
    стр.evaluate("window.__ЗАПРОСЫ = []")
    if not после["disabled"]:
        стр.click("#generate-btn")
        стр.wait_for_timeout(700)
    ушло = стр.evaluate(УШЛО_В_ГЕНЕРАЦИЮ)
    шаг(18, "после-загрузки-уходит-текст-вакансии",
        bool(ушло) and len(ушло[0]) > 0 and ушло[0] != ССЫЛКА,
        "кнопка disabled %s, длина текста %s, это ссылка: %s" % (
            после["disabled"], len(ушло[0]) if ушло else None,
            (ушло[0] == ССЫЛКА) if ушло else None), собрано=1)
    ctx.close()

    ctx, стр = _страница(бр, подлог)
    стр.goto(ch.БАЗА + "/hh", wait_until="domcontentloaded")
    стр.wait_for_timeout(900)
    стр.evaluate("window.__ОТКАЗ_ССЫЛКИ = true")
    стр.fill("#job-input", ССЫЛКА)
    стр.wait_for_timeout(1200)
    статус = стр.evaluate(СТАТУС)
    повтор = стр.evaluate(ВИДЕН, "#fetch-retry")
    кнопка = стр.evaluate(КНОПКА_ПИСЬМА)
    шаг(18, "ошибка-загрузки-видна-с-повтором",
        статус["видим"] and bool(повтор) and повтор["видим"] and повтор["h"] > 0
        and not кнопка["disabled"],
        "статус «%s» %s, повтор %s, кнопка disabled %s" % (
            статус["текст"], статус["видим"], повтор, кнопка["disabled"]))
    # Повтор после починки источника — загрузка проходит
    стр.evaluate("window.__ОТКАЗ_ССЫЛКИ = false")
    if повтор and повтор["видим"]:
        стр.click("#fetch-retry")
        стр.wait_for_timeout(900)
    статус = стр.evaluate(СТАТУС)
    шаг(18, "повтор-загружает-вакансию", "Загружено" in статус["текст"],
        "статус «%s»" % статус["текст"], собрано=1 if повтор else 0)
    # вставить текст вручную можно
    стр.fill("#job-input", ВАКАНСИЯ)
    стр.wait_for_timeout(300)
    стр.evaluate("window.__ЗАПРОСЫ = []")
    стр.click("#generate-btn")
    стр.wait_for_timeout(700)
    ушло = стр.evaluate(УШЛО_В_ГЕНЕРАЦИЮ)
    шаг(18, "текст-вручную-после-ошибки-уходит", bool(ушло) and ушло[0] == ВАКАНСИЯ,
        "длина %s" % (len(ушло[0]) if ушло else None))
    ctx.close()
...
# ══════════════════════════════════════════════════════════════════════════
ПОДЛОГИ = {
    # №18: блокировка кнопки на загрузке снята — ровно поведение до правки
    "кнопка-не-блокируется": """
      addEventListener('DOMContentLoaded', () => {
        window.занятьЗагрузкой = function () {};
      });""",
...
ЧЬЯ_СТРОКА = {
    "кнопка-не-блокируется":   "кнопка-письма-заблокирована-на-загрузке",
...
def прогон(подлог=None, только=None):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if только is None or 18 in только:
                замечание_загрузки_ссылки(бр, подлог)

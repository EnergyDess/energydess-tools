...
до НАБЛЮДАЕМОГО результата (§6.3):
  ...
  5. МЕТКИ НЕХВАТКИ у вкладок «Досье» и «Резюме»: у заполненного
     раздела скрыты (№352, «мобильный-1», 1.3), и у метки есть имя.
...
КЛЮЧИ:
  --контроль   четыре подлога В СТРАНИЦУ, кода не трогают:
                 ...
                 метка-горит-всегда   — метка «не заполнено» не гаснет у заполненного;
...
def доказать(стр, подлог, перехвачено=0):
    ...
    if подлог == "метка-горит-всегда":
        return {"поставитьТочку прячет метку": стр.evaluate(
            "() => (window.поставитьТочку || '').toString().includes('!!ready')")}
...
def проход(подлог=None):
    ...
    with sync_playwright() as p:
        ...
        точки = стр.evaluate("""() => ['dosie-badge', 'resume-badge'].map(id => {
          ...
          return {id, есть: true, скрыта: т.hidden || cs.display === 'none',
                  имя: т.title, размер: Math.round(parseFloat(cs.width))};
        })""")
        шаг("точки-на-месте-и-названы",
            all(т["есть"] and т["имя"] for т in точки),
            "; ".join("%s: %s %r" % (т["id"], т.get("скрыта"), т.get("имя"))
                      for т in точки),
            собрано=len(точки))
        # С «мобильного-1» (№352, 1.3) метка — ТОЛЬКО о нехватке:
        # у заполненного раздела её не видно вовсе
        шаг("метка-скрыта-когда-заполнено",
            all(т.get("скрыта") for т in точки),
            "; ".join("%s скрыта=%s" % (т["id"], т.get("скрыта")) for т in точки),
            собрано=len(точки))
...
def контроль():
    ...
    for подлог in ["ссылка-не-грузится", "письмо-без-меток",
                   "метка-горит-всегда", "сверка-таблетками",

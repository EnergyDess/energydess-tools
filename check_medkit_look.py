...
# BACKLOG №222, D.1 и D.3: порядок в карточке и раскрытие
КАРТОЧКА = "--карточка" in sys.argv
ПЛАШКА = "--плашка" in sys.argv
ХОД = "--ход" in sys.argv
# BACKLOG №248, блок D: слой просмотра фото и читаемость списка долгов
ДОЛГИ = "--долги" in sys.argv
# Заход 250, блок A: читаемость знака минуса в кнопке приёма
МИНУС = "--минус" in sys.argv
# Задача 252, блок E: цветная рамка в ряду действий обязана быть одна
РАМКИ = "--рамки" in sys.argv
...
# ═════════════════════════════════════════════════════════════════════
#
# BACKLOG №218, блок D письма. Владелец в четвёртый раз сообщает:
# горизонтальная прокрутка появляется НА МГНОВЕНИЕ, когда мышь
# отводится с подсказки «?» в окне «Общая аптечка», и тут же исчезает.
#
# ПОЧЕМУ ЭТОГО НЕ ВИДЕЛА НИ ОДНА ПРОБА. Все они спрашивают про
# УСТОЯВШЕЕСЯ состояние: навели — замерили, отвели — замерили.
# И до наведения, и через секунду после отвода экран одинаков
# и исправен; дефект живёт МЕЖДУ этими двумя замерами.
#
# Это тот же класс, что §6.0.15 (мигание внутри одного документа)
# и §6.0.16 (пустой кадр между документами), только третья его
# разновидность: переход СОСТОЯНИЯ элемента внутри готовой страницы.
#
# МЕРИТСЯ ПОКАДРОВО, а не одним замером после паузы: величина, которую
# ищем, живёт ровно столько, сколько идёт переход (`--dur-base`).
# Снимок «после паузы» её не застаёт по построению — ровно поэтому
# прошлые заходы и писали «перелив не воспроизводится».
# ПЕРВАЯ ВЕРСИЯ ЭТОЙ ПРОБЫ БЫЛА СЛЕПА, и записано это затем, чтобы
# следующий не повторил. Она слала `pointerenter`/`pointerleave`,
# а `ui.js` слушает `mouseover`/`mouseout`: подсказка не открывалась
# ВОВСЕ, и проба печатала «перелива нет» — то есть ровно тот вердикт,
# который ждёшь от исправного кода. Тот же урок, что у `:focus-visible`
# и программного `el.focus()` (§6.0.3).
#
# ТЕПЕРЬ МЫШЬ НАСТОЯЩАЯ: наводит и отводит её playwright, а страница
# только СЧИТАЕТ кадры. Сэмплер ставится ДО движения мыши и снимается
# после — иначе между «навели» и «начали мерить» проходит переход,
# ради которого всё и затевалось.
ХОД_СЭМПЛЕР = """(сел) => {
  const блок = document.querySelector(сел);
  if (!блок) return false;
  window.__ход = [];
  window.__ходИдёт = true;
  const тик = () => {
    if (!window.__ходИдёт) return;
    window.__ход.push(Math.round(блок.scrollWidth - блок.clientWidth));
...
ХОД_СНЯТЬ = """() => {
  window.__ходИдёт = false;
  return window.__ход || [];
}"""

ХОД_ОТКРЫТА = """() => {
  const h = document.querySelector('#apt-circle .hint');
  return !!(h && h.classList.contains('is-open'));
}"""


async def _ход_на_подсказке(pg, сенсор):
    """Наводим и отводим НАСТОЯЩУЮ мышь, считая кадры между.

    На сенсорной ширине наведения нет вовсе (`естьНаведение()` в ui.js),
    и подсказка там открывается НАЖАТИЕМ. Слать туда мышь значило бы
    мерить состояние, которого у человека не бывает.
    """
    кн = await pg.query_selector("#apt-circle .hint-btn")
    тело = "#apt-circle .modal-body"
    if not кн:
        return None
    покой = await pg.evaluate(
        "(с) => { const б = document.querySelector(с);"
        "  return б ? Math.round(б.scrollWidth - б.clientWidth) : null; }",
        тело)
    if not await pg.evaluate(ХОД_СЭМПЛЕР, тело):
        return None
    к = await кн.bounding_box()
    if сенсор:
        await кн.click()
    else:
        await pg.mouse.move(к["x"] + к["width"] / 2, к["y"] + к["height"] / 2)
    await pg.wait_for_timeout(600)
    открылась = await pg.evaluate(ХОД_ОТКРЫТА)
    открыто = (await pg.evaluate(ХОД_СНЯТЬ))["перелив"]
    await pg.evaluate(ХОД_СЭМПЛЕР, тело)
    if сенсор:
        await кн.click()
    else:
        # ОТВОД В СТОРОНУ, а не в угол окна: `mouseout` должен уйти
        # на элемент ВНЕ подсказки, и точка внутри той же панели —
        # ровно то, что делает человек
        await pg.mouse.move(к["x"] + 400, к["y"] + 200)
    await pg.wait_for_timeout(900)
    снято = await pg.evaluate(ХОД_СНЯТЬ)
    закрытие, позиции = снято["перелив"], снято["позиции"]
    после = await pg.evaluate(
        "(с) => { const б = document.querySelector(с);"
        "  return б ? Math.round(б.scrollWidth - б.clientWidth) : null; }",
        тело)
    return {"покой": покой, "открыто": max(открыто or [0]), "после": после,
            "открылась": открылась,
            "макс_открытия": max(открыто or [0]),
            "макс_закрытия": max(закрытие or [0]),
            "позиции": позиции,
            "кадров": len(закрытие or [])}


# ── ОТРИЦАТЕЛЬНЫЙ КОНТРОЛЬ ХОДА (§6.0.3) ────────────────────────────
#
# Подлог ВОЗВРАЩАЕТ ПРЕЖНЕЕ ПОВЕДЕНИЕ: снимает инлайновый `position`
# в тот же кадр, в котором снят класс `is-open`, — ровно то, что делал
# `ui.js` до BACKLOG №218.
#
# ИМЕННО ЭТО, а не «сделать подсказку absolute насовсем»: та подмена
# дала бы перелив и при ОТКРЫТИИ, то есть доказывала бы, что проба
# видит перелив ВООБЩЕ, а не перелив НА ЗАКРЫТИИ. Разница и есть
# предмет задачи.
ХОД_ПОДЛОГ = """
(() => {
  const пуск = () => {
    document.querySelectorAll('.hint').forEach(h => {
      if (h.dataset.подлог) return;
      h.dataset.подлог = '1';
      new MutationObserver(() => {
        if (h.classList.contains('is-open')) return;
        const p = h.querySelector('.hint-pop');
        if (p) { p.style.position = ''; p.style.left = ''; p.style.top = ''; }
...
async def прогон_хода(подлог=False):
    """Перелив в панели участников ВО ВРЕМЯ открытия и закрытия подсказки."""
    ...
    async with async_playwright() as p:
        ...
        for ширина in ШИРИНЫ:
            ...
            if подлог:
                await pg.add_init_script(ХОД_ПОДЛОГ)
            await _войти(pg)
            await pg.goto(БАЗА + "/medkit", wait_until="networkidle")
            await pg.evaluate("() => аптКругОткрыть()")
            await pg.wait_for_timeout(700)
            итог[ширина] = await _ход_на_подсказке(pg, сенсор)
            await ctx.close()
        await b.close()
    return итог


def печать_хода(итог):
    print("=" * 74)
    print("ПЕРЕЛИВ ВО ВРЕМЯ ОТКРЫТИЯ И ЗАКРЫТИЯ ПОДСКАЗКИ (BACKLOG №218)")
    print("=" * 74)
    print("  Меряется ПОКАДРОВО. Замер после паузы такого не застаёт")
    print("  по построению — ровно поэтому прошлые заходы писали")
    print("  «перелив не воспроизводится».")
    плохо = 0
    for ш, д in итог.items():
        if not д:
            print("  %-6d панели или подсказки нет — вопрос не задавался" % ш)
            continue
        худ = max(д["макс_открытия"], д["макс_закрытия"])
        плохо += 1 if худ > 1 else 0
        # ОТКРЫЛАСЬ ЛИ ПОДСКАЗКА — ПЕЧАТАЕТСЯ ВСЕГДА. Первая версия
        # пробы её не открывала вовсе и печатала «перелива нет»:
        # вердикт слепой пробы неотличим от вердикта зрячей, если
        # не сказать, состоялось ли само действие (§6.0.3)
        print("  %-6d покой %3d · после %3d px · подсказка открылась: %s" %
              (ш, д["покой"], д["после"],
               "да" if д["открылась"] else "НЕТ — МЕРИТЬ НЕЧЕГО"))
        print("         ВО ВРЕМЯ ОТКРЫТИЯ %3d px, ВО ВРЕМЯ ЗАКРЫТИЯ %3d px"
              "  (кадров %d)  %s"
              % (д["макс_открытия"], д["макс_закрытия"], д["кадров"],
                 "— перелива нет" if худ <= 1 else "<- ПЕРЕЛИВ НА ХОДУ"))
        # НЕЗАВИСИМЫЙ ЗАМЕР, а не пересказ вердикта: чем была подсказка
        # ВО ВРЕМЯ закрытия. `fixed` — вне потока, `absolute` — внутри
        print("         `position` подсказки за время закрытия: %s"
              % ", ".join(д.get("позиции") or ["—"]))
    print("  ширин с переливом на ходу: %d из %d" % (плохо, len(итог)))
    return плохо
...
ЗАМЕР_ШТОРКИ = """() => {
  ...
  return {
    ...
    /* ОТСТУП ПОД ПОСЛЕДНИМ РЯДОМ — РАЗЛОЖЕН НА СВОЁ И СИСТЕМНОЕ
       (BACKLOG №247, B.4).

       Здесь стояло ОДНО число `подПоследней`, и по нему нельзя было
       ответить на единственный важный вопрос: своя это добавка
       или общий отступ всех 29 модалок проекта. Первое чинится
       в `medkit.css`, второе — решение об облике всего приложения
       (§6.0.8), и путать их нельзя.

       ТРИ ЧИСЛА ВМЕСТО ОДНОГО: собственный `margin-bottom` последнего
       блока, `padding-bottom` тела окна и остаток листа под телом. */
    ...(() => {
      const дети = [...тело.children].filter(
        э => э.getBoundingClientRect().height > 0);
      const посл = дети[дети.length - 1];
      if (!посл) return {};
      const лист = тело.closest('.modal-sh') || тело.parentElement;
      const ст = getComputedStyle(тело);
      return {
        последнийБлок: (посл.className || '').split(' ')[0],
        свояДобавка: Math.round(
          parseFloat(getComputedStyle(посл).marginBottom) || 0),
        паддингТела: Math.round(parseFloat(ст.paddingBottom) || 0),
        подТеломЛиста: лист ? Math.round(
          лист.getBoundingClientRect().bottom
          - тело.getBoundingClientRect().bottom) : null,
        отПоследнегоБлокаДоНизаЛиста: лист ? Math.round(
          лист.getBoundingClientRect().bottom
          - посл.getBoundingClientRect().bottom) : null,
      };
    })(),
  };
}"""

# ── ОБЛИК ШТОРКИ: ЯРКОСТЬ ПОЯСНЕНИЙ И РИТМ (BACKLOG №246, блок D) ────
#
# Владелец: «текст расписан хаотично, глаз цепляется». Живой случай —
# три серых абзаца подряд, из которых средний ЗАМЕТНО ЯРЧЕ соседей,
# и зазоры между ними разные.
#
# СПРАШИВАЕТСЯ НЕ «КРАСИВО ЛИ», А ТРИ ЧИСЛА: сколько РАЗНЫХ цветов
# у поясняющих абзацев, сколько РАЗНЫХ кеглей и сколько РАЗНЫХ зазоров
# между соседними. Единица в каждом — один вид; двойка и больше —
# разнобой, который глаз и ловит.
#
# ЗАЗОР МЕРЯЕТСЯ МЕЖДУ ПРЯМОУГОЛЬНИКАМИ ТЕКСТА, А НЕ КОРОБОК
# (тот же довод, что у `check_ens_width`, §139): коробка абзаца шире
# своего текста, и по коробкам разница видна не та, что глазом.
ЗАМЕР_ПОЯСНЕНИЙ = """() => {
  const тело = document.getElementById('apt-doses-body');
  if (!тело) return {ошибка: 'окна нет'};
  /* ТОЛЬКО ПОЯСНЕНИЯ, А НЕ ВЕСЬ ТЕКСТ ОКНА. Первая версия брала
     каждый абзац длиннее 40 знаков и сметала в замер САМУ ВЫДЕРЖКУ —
     дословный раздел справочника, у которого свои заголовки, отступы
     и цвета по построению. Она печатала «цветов 3» на исправном окне,
     то есть врала находкой. Субъект замера — служебные пояснения,
     и опознаются они классом, а не длиной. */
  const абз = [...тело.querySelectorAll('p.apt-doses-note')];
  const св = п => {
    const с = getComputedStyle(п);
    return {цвет: с.color, кегль: с.fontSize, интер: с.lineHeight};
  };
  const рамка = п => {
    const r = document.createRange();
    r.selectNodeContents(п);
    const b = r.getBoundingClientRect();
    return {top: b.top, bottom: b.bottom};
  };
  const данные = абз.map(п => ({...св(п), ...рамка(п),
                                класс: п.className,
                                знаков: п.textContent.trim().length}));
  const зазоры = [];
  for (let i = 1; i < данные.length; i++) {
    зазоры.push(Math.round(данные[i].top - данные[i - 1].bottom));
  }
  const тр = тело.getBoundingClientRect();
  return {
    абзацев: данные.length,
    цветов: new Set(данные.map(д => д.цвет)).size,
    кеглей: new Set(данные.map(д => д.кегль)).size,
    цвета: [...new Set(данные.map(д => д.цвет))],
    кегли: [...new Set(данные.map(д => д.кегль))],
    зазоры: зазоры,
    разныхЗазоров: new Set(зазоры).size,
    сверху: данные.length ? Math.round(данные[0].top - тр.top) : null,
    снизу: данные.length
      ? Math.round(тр.bottom - данные[данные.length - 1].bottom) : null,
    классы: данные.map(д => д.класс),
  };
}"""

# ── РИТМ ВНУТРИ ШТОРКИ: СОПРИКОСНОВЕНИЯ (BACKLOG №246, D.2) ─────────
#
# Спрашивается не «красиво ли», а ДВА числа: сколько РАЗНЫХ зазоров
# между блоками окна и сколько среди них НУЛЕЙ. Ноль — не «плотно»,
# это соприкосновение: блок читается продолжением того, что стоит
# над ним (§173, C).
#
# НАЙДЕНО ЭТИМ ЗАМЕРОМ, А НЕ ЧТЕНИЕМ: правило отбивки блоков написано
# СОСЕДСТВОМ (`.apt-own + .apt-doses-box`), и между ними встаёт строка
# источника — соседство рвётся, отступ становится нулём. Замер
# 2026-09-08 по состоянию «выдержка + своя запись»: зазоры
# [12, 0, 16, 12, 0, 12, …], два соприкосновения подряд.
ЗАМЕР_РИТМА = """() => {
  const тело = document.getElementById('apt-doses-body');
  if (!тело) return {ошибка: 'окна нет'};
  const дети = [...тело.children].filter(
    э => э.getBoundingClientRect().height > 0);
  const r = дети.map(э => э.getBoundingClientRect());
  const зазоры = [];
  for (let i = 1; i < r.length; i++) {
    зазоры.push(Math.round(r[i].top - r[i - 1].bottom));
  }
  return {блоков: дети.length,
          классы: дети.map(э => (э.className || '').split(' ')[0]),
          зазоры: зазоры,
          разныхЗазоров: new Set(зазоры).size,
          соприкосновений: зазоры.filter(з => з <= 0).length};
}"""

# ── МЕТКА СОСТОЯНИЯ ШТОРКИ (BACKLOG №247, B.5) ──────────────────────
#
# Состояний у окна больше двух, и склеивается метка из того, ЧТО В НЁМ
# СТОИТ: схема справочника, своя запись с упаковки, своё «от чего»,
# кнопка повтора, кнопка «Вписать вещество», ход наружу. Второго мнения
# о состоянии проба не заводит — она читает экран, а не данные.
МЕТКА_СОСТОЯНИЯ = """() => {
  ...
  const части = [];
  части.push(есть('.apt-doses-text, .apt-doses-blocks')
             ? 'справочник' : 'без справочника');
  if (есть('.apt-own')) части.push('своя запись');
  if (подписи.some(п => /Поискать|Найти в справочнике/.test(п)))
    части.push('повтор');
  if (подписи.some(п => /Вписать вещество/.test(п)))
    части.push('вещество пусто');
  if (есть('.apt-doses-away')) части.push('наружу');
  return части.join(' + ');
}"""
...
async def прогон_шторки():
    ...
    async with async_playwright() as pw:
        ...
        for ш in ШИРИНЫ:
            ...
            # ОТКРЫВАЕТСЯ, и состояние читается с экрана — по тем же
            # блокам, которые видит человек.
            #
            # ОБХОД ВСЕХ ПОЗИЦИЙ, А НЕ ПЕРВЫХ ВОСЬМИ — BACKLOG №247, B.5.
            #
            # ЗДЕСЬ СТОЯЛО `ид_все[:8]` и остановка на ДВУХ метках
            # («схема есть» / «схемы нет»). Прежняя формулировка отвечала
            # на вопрос «одинаково ли выглядит ряд при разном числе
            # кнопок» и была верна для него. ПОЧЕМУ СТАЛА НЕГОДНОЙ:
            # состояний окна не два, а больше — набор кнопок зависит
            # ещё и от ВИДА отказа справочника (повтор поможет или нет),
            # от пустого вещества и от своей записи. Останавливаясь
            # на второй метке, проба не открывала их НИ РАЗУ, и владелец
            # честно записал пятое состояние как «недостижимое
            # на стенде» — оно было недостижимо ПРОБЕ, а не данным.
            #
            # Метка склеивается из того, ЧТО СТОИТ В ОКНЕ, — второго
            # мнения о состоянии проба не заводит.
            ...
            for ид in ид_все:
                ...
                метка = await pg.evaluate(МЕТКА_СОСТОЯНИЯ)
                if метка not in состояния:
                    состояния[метка] = await pg.evaluate(ЗАМЕР_ШТОРКИ)
                    состояния[метка]["пояснения"] = await pg.evaluate(
                        ЗАМЕР_ПОЯСНЕНИЙ)
                    состояния[метка]["ритм"] = await pg.evaluate(ЗАМЕР_РИТМА)
...
def печать_шторки(итог):
    ...
    for ш, д in итог.items():
        п = д["поиск"]
        # СКОЛЬКО СОСТОЯНИЙ ОКНА ДОСТИЖИМО НА СТЕНДЕ — печатает проба,
        # а не отчёт (§6.0.4): вписанное руками «пять» протухнет
        # на первой же правке seed
        print(chr(10) + "### СОСТОЯНИЙ ШТОРКИ НА СТЕНДЕ: %d — %s"
              % (len(д["шторка"]), "; ".join(д["шторка"])))
        print(chr(10) + "### %s px" % ш)
        if "ошибка" in п:
        ...
        for метка, ш_ in д["шторка"].items():
            ...
            print("      ПОД ПОСЛЕДНИМ БЛОКОМ (%s): своя добавка %s px + "
                  "паддинг тела %s px + лист %s px = %s px до низа листа"
                  % (ш_.get("последнийБлок"), ш_.get("свояДобавка"),
                     ш_.get("паддингТела"), ш_.get("подТеломЛиста"),
                     ш_.get("отПоследнегоБлокаДоНизаЛиста")))
            я = ш_.get("пояснения") or {}
            if "ошибка" in я:
                print("      ПОЯСНЕНИЯ: %s" % я["ошибка"])
            elif я.get("абзацев"):
                print("      ПОЯСНЕНИЯ: абзацев %s, РАЗНЫХ ЦВЕТОВ %s,"
                      " РАЗНЫХ КЕГЛЕЙ %s"
                      % (я["абзацев"], я["цветов"], я["кеглей"]))
                print("         цвета %s" % я["цвета"])
                print("         кегли %s" % я["кегли"])
                print("         зазоры между абзацами %s — РАЗНЫХ %s"
                      % (я["зазоры"], я["разныхЗазоров"]))
                print("         сверху %s px, снизу %s px"
                      % (я["сверху"], я["снизу"]))
            р = ш_.get("ритм") or {}
            if р.get("блоков"):
                print("      РИТМ: блоков %s, зазоры %s — РАЗНЫХ %s,"
                      " СОПРИКОСНОВЕНИЙ %s"
                      % (р["блоков"], р["зазоры"], р["разныхЗазоров"],
                         р["соприкосновений"]))
...
# ═══════════════════════════════════════════════════════════════════
#
# BACKLOG №223. Заход 201 свёл ряд кнопок 388 -> 0, а полоску остатка
...
# ══════════════════════════════════════════════════════════════════════
# ПЛАШКА ПЕРЕПРОВЕРКИ: СКАЧЕТ ЛИ ОКНО (BACKLOG №232, блок C)
# ══════════════════════════════════════════════════════════════════════
#
# ЖАЛОБА ВЛАДЕЛЬЦА: строка состояния переходит с одной строки на две,
# когда попадается длинное название, и окно прыгает.
#
# МЕРА — ВЫСОТА ЛИСТА ОКНА, а не самой строки. Строка от текста растёт
# по построению, и сама по себе это не дефект; дефект в том, что вслед
# за ней ездит ВСЁ окно, включая кнопку под рукой у человека.
#
# ТРИ СОСТОЯНИЯ, А НЕ ДВА. Между «плашки нет» и «плашка в одну строку»
# скачок ЗАКОНЕН — блок появляется, и его высота обязана откуда-то
# взяться. Незаконен второй переход, «одна строка → две»: там ничего
...
ПЛАШКА_ЗАМЕР = """() => {
  ...
  const мера = () => ({
    лист: Math.round(лист.getBoundingClientRect().height * 10) / 10,
    кнопка_низ: Math.round(кн.getBoundingClientRect().bottom * 10) / 10,
    плашка: Math.round(пл.getBoundingClientRect().height * 10) / 10,
    строк: (() => {
      const ст = getComputedStyle(пл);
      const ин = parseFloat(ст.lineHeight) || 1;
      const h = пл.getBoundingClientRect().height
                - parseFloat(ст.paddingTop) - parseFloat(ст.paddingBottom);
      return Math.max(0, Math.round(h / ин));
    })(),
  });
  const было = мера();
  // ЗОВЁТСЯ БОЕВОЙ ПОСТРОИТЕЛЬ, А НЕ СТАВИТСЯ ТЕКСТ НАПРЯМУЮ.
  // Своя разметка тут значила бы, что мерка отвечает про экран,
  // которого у владельца нет (§6.0.7)
  if (typeof аптПерепроверкаХод !== 'function') return {нет_построителя: 1};
  аптПерепроверкаХод('Смотрю 1 из 20. Нашлось: 0.', 'Аспирин');
  const одна = мера();
  // ДЛИННОЕ ИМЯ. Синтетическое — боевые названия это сведения
  // о здоровье (§5.1), а проба коммитится
  аптПерепроверкаХод('Смотрю 1 из 20. Нашлось: 0.',
                     'Флурибексин комплекс для кишечника усиленный '
                     + 'с пребиотиком, порошок');
  const две = мера();
  пл.textContent = '';
  пл.hidden = true;
  return {было, одна, две};
}"""


# ПОДЛОГ ВОЗВРАЩАЕТ ДОРЕФОРМЕННОЕ СОСТОЯНИЕ ЦЕЛИКОМ — обе половины
# лечения сразу. Сними мы одну, скачок мог бы не вернуться, и проба
# печатала бы «слепа» про исправную пробу (§6.0.3, урок задачи 154:
# контроль обязан возвращать ВСЕ условия прежнего состояния).
ПЛАШКА_ПОДЛОГ = """() => {
  const ст = document.createElement('style');
  ст.textContent = '.apt-recheck-note{min-height:0 !important}'
                 + '.apt-recheck-what{white-space:normal !important;'
                 + 'display:inline !important;overflow:visible !important}';
  document.head.append(ст);
}"""


async def прогон_плашки(подлог=False):
    from playwright.async_api import async_playwright
    из = {}
    async with async_playwright() as p:
        ...
        for ш in ШИРИНЫ:
            ...
            if подлог:
                await стр.add_init_script(
                    ...
                    + ПЛАШКА_ПОДЛОГ.replace("() => {", "() => {", 1) + ")")
            await _войти(стр)
            await стр.goto(БАЗА + "/medkit", wait_until="load", timeout=60000)
            await стр.wait_for_timeout(1200)
            # ОКНО ОТКРЫВАЕТСЯ НАСТОЯЩИМ НАЖАТИЕМ, а не подстановкой
            # класса: подставленный класс показал бы лист, которого
            # никто не открывал, и мерка отвечала бы про другое
            ...
            из[ш] = await стр.evaluate(ПЛАШКА_ЗАМЕР)
            await кон.close()
        await бр.close()
    return из


def печать_плашки(из):
    print("=" * 74)
    print("ПЛАШКА ПЕРЕПРОВЕРКИ: СКАЧЕТ ЛИ ОКНО (BACKLOG №232, C.1)")
    print("=" * 74)
    print("  мера: высота ЛИСТА окна и низ КНОПКИ — то, что ездит")
    print("        под рукой у человека. Высота самой строки растёт")
    print("        от текста по построению и дефектом не является")
    print()
    плохо = 0
    for ш, з in из.items():
        if not з:
            print("  %-6d окна перепроверки нет" % ш)
            плохо += 1
            continue
        if з.get("нет_построителя"):
            print("  %-6d ПОСТРОИТЕЛЯ `аптПерепроверкаХод` НЕТ — "
                  "мерить нечего" % ш)
            плохо += 1
            continue
        од, дв = з["одна"], з["две"]
        скачок_листа = round(дв["лист"] - од["лист"], 1)
        скачок_кнопки = round(дв["кнопка_низ"] - од["кнопка_низ"], 1)
        print("  %-6d плашка: одна строка %.1f px (строк %d)  →  "
              "две %.1f px (строк %d)"
              % (ш, од["плашка"], од["строк"], дв["плашка"], дв["строк"]))
        print("         ЛИСТ окна   %.1f  →  %.1f   СКАЧОК %+.1f px"
              % (од["лист"], дв["лист"], скачок_листа))
        print("         низ КНОПКИ  %.1f  →  %.1f   СКАЧОК %+.1f px"
              % (од["кнопка_низ"], дв["кнопка_низ"], скачок_кнопки))
        if abs(скачок_листа) > 1 or abs(скачок_кнопки) > 1:
            плохо += 1
    print()
    print("  ширин со скачком: %d из %d  (цель 0)" % (плохо, len(из)))
    return плохо
...
def печать_ряда(из):
    print("=" * 74)
    print("РЯД КАРТОЧЕК: НА ОДНОМ ЛИ УРОВНЕ ПОДВАЛ У СОСЕДЕЙ (BACKLOG №223)")
    print("=" * 74)
...
# ═════════════════════════════════════════════════════════════════════
# РЕЖИМ `--карточка` — ПОРЯДОК В КАРТОЧКЕ И РАСКРЫТИЕ ПАЧЕК
# BACKLOG №222, блоки D.1 и D.3.
#
# ДВА ВОПРОСА, И ОНИ РАЗНЫЕ:
#
#   D.1 — раскрытие списка упаковок добавляет высоту ТОЛЬКО СНИЗУ.
#         Меряется смещение полоски остатка и ряда действий у ТОЙ ЖЕ
#         карточки до и после нажатия «Показать упаковки». До правки
#         список стоял МЕЖДУ списанием и действиями, а подвал прижат
#         к низу — раскрытие толкало содержимое ВВЕРХ.
#   D.3 — разъезд ряда действий у соседей ПО ОДНОМУ РЯДУ. Мера та же,
#         что у `--ряд`: `top` относительно ВЕРХА своей карточки,
#         потому что высота коробки о разъезде не говорит ничего.
#
# СМЕЩЕНИЕ СЧИТАЕТСЯ В КООРДИНАТАХ СТРАНИЦЫ, А НЕ ОКНА: раскрытие
# меняет высоту документа, и отсчёт от окна дал бы сдвиг от одной
# только прокрутки.
# ═════════════════════════════════════════════════════════════════════
КАРТОЧКА_ЗАМЕР = """(() => {
  const кор = (э) => {const b = э.getBoundingClientRect();
    return {x: b.x + scrollX, y: b.y + scrollY, w: b.width, h: b.height};};
  const карты = [...document.querySelectorAll('.apt-card')]
    .filter(к => к.offsetParent !== null);
  const снять = (к) => {
    const п = к.querySelector('.apt-qty-meter');
    const д = к.querySelector('.apt-acts');
    const кб = кор(к);
    return {
      имя: ((к.querySelector('.apt-name') || {}).textContent || '').trim(),
      полоска: п ? кор(п).y : null,
      действия: д ? кор(д).y : null,
      полоска_отн: п ? Math.round((кор(п).y - кб.y) * 10) / 10 : null,
      действия_отн: д ? Math.round((кор(д).y - кб.y) * 10) / 10 : null,
      верх: кб.y, низ: кб.y + кб.h,
      // ПУСТОЕ МЕСТО ПОД РЯДОМ ДЕЙСТВИЙ (BACKLOG №223, B.5). Пока
      // хоть одна карточка в ряду умела расти, у всех остальных
      // появлялся хвост — и он был не «плотностью», а дырой
      хвост: д ? Math.round((кб.y + кб.h - (кор(д).y + кор(д).h)) * 10) / 10
               : null,
      пачки: !!к.querySelector('.apt-stack[data-packs]')
    };
  };
  return карты.map(снять);
})()"""

# ПОРЯДОК ЭЛЕМЕНТОВ — ОТДЕЛЬНЫЙ ВОПРОС ОТ ГЕОМЕТРИИ. Владелец назвал
# его списком, и проверять его надо списком же: какой элемент за каким
# идёт в дереве. Совпавшие координаты ещё не значат, что порядок тот.
ПОРЯДОК_ЗАМЕР = """(() => {
  // КАРТОЧКА СО СНИМКОМ, а не первая попавшаяся: в ожидаемом порядке
  // владельца фото стоит ПЕРВЫМ, и карточка без него дала бы
  // «РАЗОШЁЛСЯ» на исправном коде — проба врала бы находкой
  const карты = [...document.querySelectorAll('.apt-card')]
    .filter(x => x.offsetParent !== null);
  const к = карты.find(x => x.querySelector('.apt-ph'))
         || карты.find(x => x.querySelector('.apt-stack[data-packs]'));
  if (!к) return null;
  const имена = {'apt-ph': 'фото', 'apt-name': 'название',
    'apt-mnn': 'вещество', 'apt-qty-cap': 'остаток текстом',
    'apt-qty-meter': 'полоска', 'apt-place': 'место',
    'apt-exp': 'срок', 'apt-take': 'кнопка списания',
    'apt-scale': 'ступени тюбика', 'apt-take-gap': 'дорожка',
    'apt-acts': 'ряд действий',
    // СКРЫТЫЙ ИСТОЧНИК СПИСКА, а не видимый блок: он лежит в карточке
    // затем, чтобы окно показало серверную разметку, и в поток
    // не входит вовсе (BACKLOG №223, блок B)
    'apt-packs-src': 'упаковки (скрытый источник)'};
  const из = [];
  const обход = (у) => {
    for (const д of у.children) {
      const кл = [...д.classList].find(c => имена[c]);
      if (кл) { из.push(имена[кл]); if (кл !== 'apt-ph') continue; }
      обход(д);
    }
  };
  обход(к);
...
async def прогон_карточки():
    ...
    async with async_playwright() as p:
        ...
        for ш in ШИРИНЫ:
            ...
            стр = await кон.new_page()
            await _войти(стр)
            о = await стр.goto(БАЗА + "/medkit", wait_until="load",
                               timeout=60000)
            # СТРАНИЦА ОБЯЗАНА ОТВЕТИТЬ 200. Без этой строки проба мерит
            # страницу ошибки и печатает про неё нули — ровно так первый
            # прогон этого захода напечатал «рядов ступеней 0»
            # про пятисотку (§6.0.1).
            assert о.status == 200, "СТРАНИЦА ОТВЕТИЛА %s" % о.status
            await стр.wait_for_timeout(1200)
            до = await стр.evaluate(КАРТОЧКА_ЗАМЕР)
            порядок = await стр.evaluate(ПОРЯДОК_ЗАМЕР)
            # ПРЕЖНЯЯ ФОРМУЛИРОВКА: нажать `<details>` в карточке
            # и замерить, куда уехало содержимое.
            # НОВАЯ: нажать МЕТКУ «N упак.» и замерить, что в карточке
            # не сдвинулось НИЧЕГО.
            # ПОЧЕМУ ПРЕЖНЯЯ СТАЛА НЕГОДНОЙ: списка в карточке больше
            # нет — он уехал в окно (BACKLOG №223, блок B), и прежний
            # замер печатал бы «вопрос не задавался» на исправном коде.
            # ВОПРОС ПРИ ЭТОМ СТАЛ СТРОЖЕ, а не мягче: раньше спрашивали
            # «куда сдвинулось», теперь «сдвинулось ли вообще».
            сум = стр.locator(".apt-stack[data-packs]").first
            раскрыли = await сум.count() > 0
            if раскрыли:
                await сум.click()
                await стр.wait_for_timeout(500)
            после = await стр.evaluate(КАРТОЧКА_ЗАМЕР)
            из[ш] = {"до": до, "после": после, "порядок": порядок,
                     "раскрыли": раскрыли}
            await кон.close()
        await бр.close()
    return из
...
def печать_карточки(из):
    print("=" * 74)
    print("КАРТОЧКА АПТЕЧКИ: ПОРЯДОК, РАСКРЫТИЕ ПАЧЕК, РАЗЪЕЗД ДЕЙСТВИЙ")
    print("BACKLOG №222, блоки D.1 и D.3")
    print("=" * 74)
    ожидание = ["фото", "название", "вещество", "остаток текстом", "полоска",
                "место", "срок"]
    for ш, з in из.items():
        print()
        print("── ширина %d ──" % ш)
        п = з["порядок"]
        if п:
            print("  ПОРЯДОК: %s" % " -> ".join(п))
            начало = [x for x in п if x in ожидание]
            print("  первые семь по списку владельца: %s"
                  % ("СОВПАЛ" if начало[:7] == ожидание
                     else "РАЗОШЁЛСЯ: %s" % начало[:7]))
            хвост = [x for x in п if x in
                     ("кнопка списания", "ступени тюбика", "дорожка",
                      "ряд действий")]
            # ПРЕЖДЕ ХВОСТ КОНЧАЛСЯ «упаковками»: список стоял в карточке
            # последним видимым блоком. Теперь он в окне, а в карточке
            # ниже ряда действий нет НИЧЕГО (BACKLOG №223, B.1)
            print("  хвост (списание -> действия, ниже НИЧЕГО): %s  %s"
                  % (" -> ".join(хвост),
                     "ВЕРНО" if хвост[-1:] == ["ряд действий"]
                     else "РАЗОШЁЛСЯ"))
        if not з["раскрыли"]:
            print("  D.1 ОКНО УПАКОВОК: группы с пачками на экране нет — "
                  "вопрос не задавался")
        else:
            пары = list(zip(з["до"], з["после"]))
            дп = max((abs((б["полоска"] or 0) - (а["полоска"] or 0))
                      for а, б in пары if а["полоска"] is not None),
                     default=0)
            дд = max((abs((б["действия"] or 0) - (а["действия"] or 0))
                      for а, б in пары if а["действия"] is not None),
                     default=0)
            своя = next((i for i, а in enumerate(з["до"]) if а["пачки"]), None)
            сп = сд = 0.0
            if своя is not None:
                a, b = з["до"][своя], з["после"][своя]
                if a["полоска"] is not None:
                    сп = abs(b["полоска"] - a["полоска"])
                if a["действия"] is not None:
                    сд = abs(b["действия"] - a["действия"])
            # СМЕЩЕНИЕ ОТНОСИТЕЛЬНО СВОЕЙ КАРТОЧКИ — это и есть вопрос
            # D.1: «всё, что выше ряда кнопок, не сдвигается». Карточка
            # целиком может уехать на пару пикселей от пересчёта высот
            # рядов сетки (замер: 2.0 px при росте сетки на 84), и это
            # НЕ переезд содержимого, а другая величина. Печатаются обе.
            оп = од = 0.0
            if своя is not None:
                a, b = з["до"][своя], з["после"][своя]
                if a["полоска_отн"] is not None:
                    оп = abs(b["полоска_отн"] - a["полоска_отн"])
                if a["действия_отн"] is not None:
                    од = abs(b["действия_отн"] - a["действия_отн"])
            print("  D.1 ОКНО УПАКОВОК (карточка «%s»):"
                  % (з["до"][своя]["имя"] if своя is not None else "?"))
            print("      ВНУТРИ неё (отсчёт от верха карточки): полоска "
                  "%.1f px, действия %.1f px  %s"
                  % (оп, од,
                     "— в карточке не сдвинулось ничего"
                     if оп == 0 and од == 0 else "— СДВИНУЛОСЬ"))
            print("      она же на СТРАНИЦЕ: полоска %.1f px, действия %.1f px"
                  % (сп, сд))
            print("      по всем карточкам: полоска %.1f px, действия %.1f px"
                  % (дп, дд))
        ряды = {}
        for а in з["после"]:
            ряды.setdefault(round(а["верх"]), []).append(а)
        худ_д = худ_п = 0.0
        for гр in ряды.values():
            if len(гр) < 2:
                continue
            зн = [x["действия_отн"] for x in гр
                  if x["действия_отн"] is not None]
            зп = [x["полоска_отн"] for x in гр if x["полоска_отн"] is not None]
            if len(зн) > 1:
                худ_д = max(худ_д, max(зн) - min(зн))
            if len(зп) > 1:
                худ_п = max(худ_п, max(зп) - min(зп))
        рядов = sum(1 for г in ряды.values() if len(г) > 1)
        if not рядов:
            print("  D.3 РАЗЪЕЗД: рядов по 2+ карточки нет — на этой ширине "
                  "карточки идут в столбик, разъезжаться нечему")
        else:
            print("  D.3 РАЗЪЕЗД в ряду (рядов %d): действия %.1f px, "
                  "полоска %.1f px" % (рядов, худ_д, худ_п))
        # ── B.5: ПУСТОЕ МЕСТО ПОД РЯДОМ ДЕЙСТВИЙ ────────────────────
        #
        # Спрашивается У ВСЕХ карточек, а не у соседей по ряду: хвост
        # у одной-единственной карточки — это тоже дыра, просто её
        # не с чем сравнить глазом
        хвосты = [(а["имя"], а["хвост"]) for а in з["после"]
                  if а["хвост"] is not None]
        худой = max((х for _, х in хвосты), default=0.0)
        тощий = min((х for _, х in хвосты), default=0.0)
        # РАЗБРОС ВАЖНЕЕ ХУДШЕГО, и это не придирка. Ноль под рядом
        # действий буквально означал бы, что кнопки прижаты к краю
        # карточки, — паддинг подвала это не «пустое место», а поле.
        # ДЫРА — это когда у одной карточки поля БОЛЬШЕ, чем у соседей:
        # ровно её видно глазом и ровно её лечил этот блок
        print("  B.5 ПУСТОЕ МЕСТО ПОД РЯДОМ ДЕЙСТВИЙ: от %.1f до %.1f px "
              "по %d карточкам, РАЗБРОС %.1f px%s"
              % (тощий, худой, len(хвосты), худой - тощий,
                 "" if худой - тощий <= 1 else " — худшая «%s»"
                 % next(и for и, х in хвосты if х == худой)))
    print()
    print("РЯД ДЕЙСТВИЙ — ТО, ЧТО РЕЗЕРВ И РОВНЯЕТ (пункт 7 порядка).")
    print("Полоска и срок с этой правки следуют за содержимым: у карточки")
    print("со снимком они ниже, чем у карточки без него, — цена порядка,")
    print("названного владельцем, и она замерена, а не забыта.")
    return 0
...
ЗАМЕР_ДОЛГОВ = """() => {
  const вид = (э) => {
    ...
    return {кегль: parseFloat(s.fontSize), вес: s.fontWeight,
  ...
  return {
    строк: строки.length,
    строка: вид(строки[0]),
    имя: вид(строки[0] && строки[0].querySelector('.apt-debt-name')),
    метка: вид(строки[0] && строки[0].querySelector('.apt-debt-chip')),
    зазоры: строки.slice(1).map((э, i) => Math.round(
      (э.getBoundingClientRect().top
       - строки[i].getBoundingClientRect().bottom) * 10) / 10),
    окно_h: кор ? Math.round(кор.height) : null,
    окно_за_экраном: кор ? Math.round(кор.bottom - innerHeight) : null,
    прокрутка: тело ? тело.scrollHeight > тело.clientHeight + 1 : null,
    итог_виден: (() => {
      const и = document.getElementById('apt-debt-sub');
      if (!и) return null;
      const r = и.getBoundingClientRect();
      return r.top >= (кор ? кор.top : 0) - 1 && r.bottom <= innerHeight + 1;
    })(),
  };
}"""

# СЛОЙ ПРОСМОТРА ФОТО: у окна и у просмотрщика z-index ОДИН (200),
# и кто сверху, решает ПОРЯДОК В РАЗМЕТКЕ. Меряется не число, а ФАКТ:
...
ЗАМЕР_СЛОЯ = """() => {
  ...
  const т = document.elementFromPoint(x, y);
  return {
    просмотр_открыт: в.classList.contains('open'),
    z_просмотр: getComputedStyle(в).zIndex,
    z_окно: getComputedStyle(о).zIndex,
    в_точке_просмотр: !!(т && т.closest('#apt-img-viewer')),
    в_точке: т ? (т.id || т.className || т.tagName) : null,
  };
}"""


# СИНТЕТИЧЕСКОЕ ТЕЛО ДЛЯ D.3. Поля — ровно те, что отдаёт
# `/medkit/api/gaps`; имена условные, названий из аптечки владельца
# в пробах не печатается и не подставляется (§5.1, проверка 26).
СИНТЕТИКА = """(сколько) => {
  const виды = [{id: 'схема', подпись: 'нет схемы приёма', куда: 'дозы'},
                {id: 'показания', подпись: 'нет показаний к применению',
                 куда: 'дозы'},
                {id: 'вещество', подпись: 'нет действующего вещества',
                 куда: 'правка'},
                {id: 'категория', подпись: 'нет категории',
                 куда: 'категория'}];
  const позиции = [];
  for (let i = 0; i < сколько; i++) {
    позиции.push({id: 900000 + i, name: 'Проба ' + (i + 1),
                  photo: null, 'упаковок': 1,
                  'долги': виды.slice(0, 1 + (i % 4)).map(в => в.id)});
  }
  аптДолгиНарисовать({'виды': виды, 'позиции': позиции,
                      'неполных': сколько, 'карточек': сколько + 10,
                      'по_видам': {}});
}"""


async def прогон_долгов(подлог=False):
    """D.1 и D.2: слой раскрытого снимка и читаемость списка."""
    from playwright.async_api import async_playwright
    итог = {}
    async with async_playwright() as pw:
        бр = await pw.chromium.launch()
        for ш in ШИРИНЫ:
            ...
            pg = await ctx.new_page()
            if подлог:
                # ПОДЛОГ: просмотрщик возвращается ПОД окно — ровно то,
                # что было до правки. Кода экрана не трогаем
            ...
            # РАСКРЫТИЕ СНИМКА — НАСТОЯЩИМ НАЖАТИЕМ по плитке, а не
            # вызовом просмотрщика: дефект живёт в том, что происходит
            # ПОСЛЕ нажатия внутри уже открытого окна
            слой, есть = None, await pg.evaluate(
                "() => !!document.querySelector('.apt-debt-ph[data-shot]')")
            if есть:
                await pg.click(".apt-debt-ph[data-shot]")
                await pg.wait_for_timeout(400)
                слой = await pg.evaluate(ЗАМЕР_СЛОЯ)
            # ── D.3: ТО ЖЕ ОКНО ПРИ 15 И ПРИ 40 СТРОКАХ ──────────
            #
            # Число строк — свойство ДАННЫХ, и ждать, когда у владельца
            # накопится сорок неполных карточек, значит не проверить
            # это никогда. Рисует их БОЕВОЙ построитель (`аптДолгиНарисовать`)
            # тем же телом, какое отдаёт сервер, — второго построителя
            # списка в браузере нет (§6.0.7), и подставляется здесь
            # ВХОД, а не разметка.
            объёмы = {}
            for сколько in (15, 40):
                await pg.evaluate(СИНТЕТИКА, сколько)
                await pg.wait_for_timeout(250)
                объёмы[сколько] = await pg.evaluate(ЗАМЕР_ДОЛГОВ)
            итог[ш] = {"долги": долги, "слой": слой, "плитка_с_фото": есть,
                       "объёмы": объёмы}
            await ctx.close()
        await бр.close()
    return итог
...
def печать_долгов(из):
    print("=" * 76)
    print("ОКНО ПЕРЕПРОВЕРКИ: СЛОЙ СНИМКА И ЧИТАЕМОСТЬ СПИСКА — блок D")
    print("=" * 76)
    плохо = 0
    for ш, д in из.items():
        дол, сл = д["долги"], д["слой"]
        имя, метка, стр = дол["имя"], дол["метка"], дол["строка"]
        print("  ширина %d:" % ш)
        print("    строк в списке      %s" % дол["строк"])
        print("    имя позиции         кегль %s, вес %s"
              % (имя and имя["кегль"], имя and имя["вес"]))
        print("    метка (чип)         кегль %s" % (метка and метка["кегль"]))
        print("    строка              высота %s px, граница низа %s (%s)"
              % (стр and стр["h"], стр and стр["граница"],
                 стр and стр["гцвет"]))
        print("    зазоры между строк  %s" % (дол["зазоры"][:6],))
        print("    окно %s px, за экраном %s, итог виден %s, прокрутка %s"
              % (дол["окно_h"], дол["окно_за_экраном"], дол["итог_виден"],
                 дол["прокрутка"]))
        if сл is None:
            print("    СНИМОК НЕ РАСКРЫТ: плитки с фото в списке нет")
        else:
            print("    просмотр открыт %s, z просмотр %s против окна %s"
                  % (сл["просмотр_открыт"], сл["z_просмотр"], сл["z_окно"]))
            print("    в точке центра снимка: %s  (просмотр сверху: %s)"
                  % (сл["в_точке"], сл["в_точке_просмотр"]))
            if not сл["в_точке_просмотр"]:
                плохо += 1
        # ИМЯ ОБЯЗАНО БЫТЬ КРУПНЕЕ МЕТКИ: это и есть «развести размером»
        if имя and метка and имя["кегль"] <= метка["кегль"]:
            print("    ПЛОХО: имя не крупнее метки (%s против %s)"
                  % (имя["кегль"], метка["кегль"]))
            плохо += 1
        if дол["окно_за_экраном"] and дол["окно_за_экраном"] > 1:
            print("    ПЛОХО: окно уходит за экран на %s px"
                  % дол["окно_за_экраном"])
            плохо += 1
        # ── D.3: ТО ЖЕ ОКНО ПРИ 15 И ПРИ 40 СТРОКАХ ──────────────
        for сколько, з in sorted((д.get("объёмы") or {}).items()):
            print("    при %2s строках: строк %s, окно %s px, за экраном %s, "
                  "прокрутка %s, итог виден %s"
                  % (сколько, з["строк"], з["окно_h"], з["окно_за_экраном"],
                     з["прокрутка"], з["итог_виден"]))
            if з["окно_за_экраном"] and з["окно_за_экраном"] > 1:
                print("    ПЛОХО: при %s строках окно за экраном на %s px"
                      % (сколько, з["окно_за_экраном"]))
                плохо += 1
            if not з["итог_виден"]:
                print("    ПЛОХО: при %s строках итог не виден" % сколько)
                плохо += 1
    print("  ШИРИН С БЕДОЙ: %d" % плохо)
    return плохо


# ── ЗНАК МИНУСА В КНОПКЕ ПРИЁМА (заход 250, блок A) ──────────────────
#
# МЕРИТСЯ ФАКТИЧЕСКИЙ ПИКСЕЛЬ, А НЕ ВЫЧИСЛЕННАЯ ПЕРЕМЕННАЯ (§6.2).
# Подложка знака объявлена через `color-mix` с прозрачностью, то есть
# её вычисленное значение — `color(srgb … / 0.16)`, и цвет НА ЭКРАНЕ
# из него не следует: он зависит от того, что лежит под ней. Первая
# версия этой пробы разбирала строку стиля и упала на ней же.
#
# ТОЧКИ БЕРУТСЯ СО СНИМКА КНОПКИ при DPR 3: штрих минуса при 15 px
# знака занимает 1.25 CSS-пикселя (`stroke-width: 2` в системе
# координат 24), и на DPR 1 он размазан сглаживанием — замер попал бы
# в полутон, а не в цвет штриха.
МИНУС_ГЕОМ = r"""() => {
  const кн = document.querySelector('.apt-take:not([disabled])');
  if (!кн) return null;
  const svg = кн.querySelector('svg');
  if (!svg) return null;
  const бк = кн.getBoundingClientRect();
  const бs = svg.getBoundingClientRect();
  const карт = кн.closest('.apt-card');
  return {
    кнопка: {x: бк.x, y: бк.y, w: бк.width, h: бк.height},
    знак:   {x: бs.x - бк.x, y: бs.y - бк.y, w: бs.width, h: бs.height},
    подпись: кн.textContent.trim(),
    цвет_текста: getComputedStyle(кн).color,
    вес: getComputedStyle(кн).fontWeight,
    рамка: getComputedStyle(кн).borderTopWidth,
    цвет_рамки: getComputedStyle(кн).borderTopColor,
    фон_карточки: карт ? getComputedStyle(карт).backgroundColor : null,
    // СОСЕДИ ПО ПОДВАЛУ: «Изменить», «Инструкция», «Удалить».
    // Иерархия — это не «кнопка ярче», а «она отличается от них
    // тем, чего у них нет»: заливкой, цветом надписи и весом
    соседи: [...(карт ? карт.querySelectorAll('.apt-acts .btn') : [])]
      .map(э => { const с = getComputedStyle(э); return {
        подпись: э.textContent.trim().slice(0, 20),
        фон: с.backgroundColor, цвет: с.color, вес: с.fontWeight,
        рамка: с.borderTopWidth,
        цвет_рамки: с.borderTopColor }; }),
  };
}"""

# ПОДЛОГ ВОЗВРАЩАЕТ ПРЕЖНЕЕ ПОВЕДЕНИЕ — подложку `--surface-3`,
# прибитую к поверхности карточки. Это и есть дефект захода 249,
# снятый с полки, а не выдуманное состояние.
# Кладётся В СТРАНИЦУ, кода не трогает.
#
# ПОСЛЕ ЗАГРУЗКИ, А НЕ `add_init_script`. Первая версия вставляла стиль
# init-скриптом, и подлог МОЛЧА НЕ СОСТОЯЛСЯ: тот исполняется до разбора
# документа, и вставленный узел не переживает построение дерева. Поймало
# это ДОКАЗАТЕЛЬСТВО, а не вердикт пробы, — та печатала «0 находок»
# и с подлогом, и без (§6.0.3, та же ловушка, что в §6.0.15).
#
# ВТОРОЙ ПОДЛОГ БЫЛ НАПИСАН И ОТБРОШЕН СОБСТВЕННЫМ ЗАМЕРОМ, и это
# запись о находке. Он красил подложку ЦВЕТОМ ЗАЛИВКИ КНОПКИ — ровно
# то состояние, которое постановка захода 250 называла причиной
# («штрих красится тем же цветом, что фон кнопки»). Подлог СОСТОЯЛСЯ
# (подложка `rgb(7,154,180)` -> `rgb(6,182,212)`, контраст к фону
# 1.38 -> 1.00), а проба его не нашла — И НЕ ДОЛЖНА БЫЛА: штрих там
# отличается от подложки на 8.11, то есть знак читается ОТЛИЧНО.
# Постулат опровергнут не рассуждением, а исполненным подлогом.
МИНУС_ПОДЛОГ = (".apt-take svg { background: var(--surface-3) !important; }")

# ПОДЛОГ БЛОКА E: рамки соседей покрашены акцентом. Иерархия ряда
# держится на том, что цветная рамка ровно одна; покрась соседей —
# и «главная» кнопка перестанет быть главной, оставшись такой же
# по всем прочим признакам. Замер обязан это назвать.
РАМКИ_ПОДЛОГ = (".apt-act { border-color: var(--tool-accent) !important; }")

# ДОЛЯ ЦВЕТНОГО ПИКСЕЛЯ В СЕТКЕ (задача 252, блок E). Считается
# по СНИМКУ сетки карточек, а не по объявленным цветам: вопрос
# владельца был про то, сколько цвета НА ЭКРАНЕ, а не сколько его
# в CSS. Фотографии упаковок из счёта вычитаются — они цветные сами
# по себе и к решению об облике кнопок отношения не имеют.
СЕТКА_КОРОБКИ = r"""() => {
  const с = document.querySelector('.apt-grid');
  if (!с) return null;
  const r = э => { const k = э.getBoundingClientRect();
                   return {x: Math.round(k.x), y: Math.round(k.y),
                           w: Math.round(k.width), h: Math.round(k.height)}; };
  return {
    сетка: r(с),
    фото: [...с.querySelectorAll('.apt-ph')].map(r),
  };
}"""


def _доля_акцента(png, коробки, акцент=(6, 182, 212)):
    """Доля пикселей, близких к акценту, за вычетом плиток фото."""
    try:
        from PIL import Image
    except ImportError:
        return None
    им = Image.open(io.BytesIO(png)).convert("RGB")
    ш, в = им.size
    к = коробки["сетка"]
    мш = ш / max(к["w"], 1)          # снимок мог сняться при другом DPR
    # ПЛИТКИ ФОТО ВЫЧИТАЮТСЯ: цветной снимок упаковки — не наш акцент
    маска = []
    for ф in коробки["фото"]:
        маска.append((int((ф["x"] - к["x"]) * мш), int((ф["y"] - к["y"]) * мш),
                      int((ф["x"] - к["x"] + ф["w"]) * мш),
                      int((ф["y"] - к["y"] + ф["h"]) * мш)))
    # NUMPY, А НЕ ПЕРЕБОР В PYTHON: снимок сетки при DPR 3 — это
    # 46 миллионов пикселей, и построчный обход шёл минутами.
    # Замер, который не дожидаются, не отличается от несделанного.
    import numpy as _np
    a = _np.asarray(им, dtype=_np.int16)
    берём = _np.ones(a.shape[:2], dtype=bool)
    for x0, y0, x1, y1 in маска:
        берём[max(y0, 0):max(y1, 0), max(x0, 0):max(x1, 0)] = False
    r_, g_, b_ = a[..., 0], a[..., 1], a[..., 2]
    # БЛИЗОСТЬ ПО ОТТЕНКУ, а не точное совпадение: заливка 15 % даёт
    # не сам акцент, а его примесь к подложке
    свои = ((b_ > r_ + 24) & (g_ > r_ + 16) & (b_ > 60) & берём).sum()
    всего = берём.sum()
    return round(100.0 * float(свои) / max(int(всего), 1), 2)
...
def _контраст(a, b):
    la, lb = _яркость(a), _яркость(b)
    hi, lo = max(la, lb), min(la, lb)
    return round((hi + 0.05) / (lo + 0.05), 2)


def _цвет(s):
    """`rgb(a, b, c)` -> кортеж. Прозрачное и незнакомое — None."""
    if not s or "rgba(0, 0, 0, 0)" in s:
        return None
    т = s[s.find("(") + 1:s.find(")")].split(",")
    try:
        return tuple(int(float(x)) for x in т[:3])
    except ValueError:
        return None


def _разбор_знака(png, геом, dpr):
    """Три цвета СО СНИМКА: фон кнопки, подложка знака, штрих."""
    import io as _io
    from PIL import Image
    им = Image.open(_io.BytesIO(png)).convert("RGB")
    з = геом["знак"]
    цx = int((з["x"] + з["w"] / 2) * dpr)
    цy = int((з["y"] + з["h"] / 2) * dpr)
    # ФОН КНОПКИ — левее знака, вне его коробки
    фx = max(1, int(з["x"] * dpr) - int(4 * dpr))
    фон = им.getpixel((фx, цy))
    # ПОДЛОЖКА — внутри круга, но ВЫШЕ штриха: штрих горизонтальный
    # и лежит ровно в середине знака
    подл = им.getpixel((цx, max(0, цy - int(4 * dpr))))
    # ШТРИХ — самый непохожий на подложку пиксель в центральной ПОЛОСЕ.
    # Брать «ровно центр» вслепую нельзя: при чётной высоте снимка
    # середина попадает между двумя рядами сглаживания
    полоса = []
    шаг = max(1, int(з["w"] * dpr / 3))
    for dy in (-1, 0, 1):
        y = min(им.height - 1, max(0, цy + dy))
        for dx in range(-шаг, шаг + 1):
            x = min(им.width - 1, max(0, цx + dx))
            полоса.append(им.getpixel((x, y)))
    штрих = max(полоса, key=lambda c: _контраст(c, подл))
    return {"фон": фон, "подложка": подл, "штрих": штрих}


async def прогон_минуса(подлог=False):
    from playwright.async_api import async_playwright
    из = {}
    async with async_playwright() as p:
        бр = await p.chromium.launch()
        for ш in ШИРИНЫ:
            ...
            await стр.wait_for_timeout(900)
            if подлог:
                await стр.add_style_tag(
                    content=(РАМКИ_ПОДЛОГ if подлог == "рамки"
                             else МИНУС_ПОДЛОГ))
                await стр.wait_for_timeout(200)
            геом = await стр.evaluate(МИНУС_ГЕОМ)
            if not геом:
                из[ш] = None
                await кон.close()
                continue
            кноп = стр.locator(".apt-take:not([disabled])").first
            png = await кноп.screenshot()
            # ДОЛЯ ЦВЕТА В СЕТКЕ — снимком, а не суммой объявлений
            коробки = await стр.evaluate(СЕТКА_КОРОБКИ)
            доля = None
            if коробки:
                кадр = await стр.locator(".apt-grid").first.screenshot()
                доля = _доля_акцента(кадр, коробки)
            из[ш] = {"геом": геом, "цвета": _разбор_знака(png, геом, 3),
                     "доля_акцента": доля}
            await кон.close()
        await бр.close()
    return из
...
def печать_минуса(из):
    print("=" * 74)
    print("КНОПКА ПРИЁМА: ЗНАК МИНУСА И ИЕРАРХИЯ (заход 250, блоки A и B)")
    print("ПИКСЕЛИ СО СНИМКА при DPR 3, а не вычисленные переменные")
    print("=" * 74)
    # ПОРОГ 3.0 — WCAG 1.4.11 для НЕТЕКСТОВЫХ элементов: знак минуса
    # это графика, а не буква, и мерка у него своя
    ПОРОГ = 3.0
    плохо = 0
    for ш, д in из.items():
        if not д:
            print("  ширина %d: кнопки приёма НЕТ — мерить нечего" % ш)
            continue
        ц = д["цвета"]
        к_шп = _контраст(ц["штрих"], ц["подложка"])
        к_пф = _контраст(ц["подложка"], ц["фон"])
        к_шф = _контраст(ц["штрих"], ц["фон"])
        print("  ширина %d:" % ш)
        print("    фон кнопки %s  подложка знака %s  штрих %s"
              % (ц["фон"], ц["подложка"], ц["штрих"]))
        print("    КОНТРАСТ штрих/подложка %5.2f   (порог %.1f)"
              % (к_шп, ПОРОГ))
        print("    КОНТРАСТ подложка/фон   %5.2f" % к_пф)
        print("    КОНТРАСТ штрих/фон      %5.2f" % к_шф)
        # ЗНАК ЧИТАЕТСЯ, ЕСЛИ ОТЛИЧАЕТСЯ ОТ ТОГО, НА ЧЁМ ЛЕЖИТ, —
        # то есть от ПОДЛОЖКИ. Контраст к фону кнопки был 8.11
        # и в сломанном состоянии тоже, спрашивать его бесполезно
        if к_шп < ПОРОГ:
            print("    ПЛОХО: штрих сливается с подложкой (%.2f < %.1f) — "
                  "знак читается выколотой дыркой" % (к_шп, ПОРОГ))
            плохо += 1
        # ── ИЕРАРХИЯ ПОДВАЛА (блок B захода 250) ─────────────────────
        г = д["геом"]
        под = _цвет(г["цвет_текста"])
        карт = _цвет(г["фон_карточки"])
        к_под = _контраст(под, ц["фон"]) if под else None
        к_фк = _контраст(ц["фон"], карт) if карт else None
        print("    надпись %s вес %s рамка %s"
              % (г["цвет_текста"], г["вес"], г["рамка"]))
        print("    КОНТРАСТ надпись/фон кнопки   %s   (порог 4.5, WCAG AA)"
              % ("%5.2f" % к_под if к_под else "?"))
        print("    КОНТРАСТ фон кнопки/фон карт. %s"
              % ("%5.2f" % к_фк if к_фк else "?"))
        # НАДПИСЬ КНОПКИ — ОБЫЧНЫЙ ТЕКСТ, порог 4.5 (WCAG 1.4.3)
        if к_под is not None and к_под < 4.5:
            print("    ПЛОХО: надпись на фоне кнопки %.2f < 4.5" % к_под)
            плохо += 1
        сос = г.get("соседи") or []
        if д.get("доля_акцента") is not None:
            print("    ДОЛЯ ЦВЕТНОГО ПИКСЕЛЯ В СЕТКЕ (без плиток фото): %.2f %%"
                  % д["доля_акцента"])
        print("    соседей в подвале %d" % len(сос))
        for с in сос:
            свой = (с["фон"] != "rgba(0, 0, 0, 0)")
            print("      %-20s фон %-22s вес %-3s рамка %s"
                  % (с["подпись"], с["фон"], с["вес"], с["рамка"]))
            # ИЕРАРХИЯ ДЕРЖИТСЯ ТРЕМЯ ПРИЗНАКАМИ РАЗОМ, и хватит
            # РАЗОЙТИСЬ ХОТЯ БЫ ОДНОМУ: заливка, цвет надписи, вес.
            # Требовать все три значило бы запретить сводить соседа
            # к тому же весу решением об облике
            разошлись = sum([
                свой != True,                       # у соседа заливки нет
                с["цвет"] != г["цвет_текста"],      # надпись другого цвета
                с["вес"] != г["вес"],               # вес другой
            ])
            if разошлись == 0:
                print("      ПЛОХО: сосед %r неотличим от кнопки приёма"
                      % с["подпись"])
                плохо += 1
        # ── ЦВЕТНАЯ РАМКА В РЯДУ РОВНО ОДНА (задача 252, E4) ─────────
        # Толщина у всех совпадает по замыслу, и различает их ЦВЕТ.
        # Считается по ОТТЕНКУ: акцентная рамка синéе своего красного
        # и серого соседей.
        рамки = [(с["подпись"], с.get("цвет_рамки")) for с in сос]
        рамки.append(("Приём", г.get("цвет_рамки")))
        акц = [п for п, ц in рамки
               if ц and _цвет(ц) and _цвет(ц)[2] > _цвет(ц)[0] + 24]
        print("    рамок всего %d, из них АКЦЕНТНЫХ %d: %s"
              % (len([1 for _, ц in рамки if ц]), len(акц), акц or "—"))
        if len(акц) != 1:
            print("    ПЛОХО: акцентная рамка обязана быть ровно одна")
            плохо += 1
    print("  ШИРИН С БЕДОЙ: %d" % плохо)
    return плохо
...
def main():
    if РАМКИ:
        print("ЦВЕТНАЯ РАМКА В РЯДУ ДЕЙСТВИЙ (задача 252, блок E)")
        из_ч = asyncio.run(прогон_минуса())
        чисто = печать_минуса(из_ч)
        if not КОНТРОЛЬ:
            return 1 if чисто else 0
        # КОНТРОЛЬ НА ГРЯЗНОЙ ОСНОВЕ НЕДЕЙСТВИТЕЛЕН (§6.0.3)
        if чисто:
            print("")
            print("ОСТАНОВЛЕНО: беда есть и БЕЗ подлога — "
                  "контроль ничего не доказывает.")
            return 2
        print()
        из_п = asyncio.run(прогон_минуса(подлог="рамки"))
        с_подлогом = печать_минуса(из_п)
        # ДОКАЗАТЕЛЬСТВО НЕЗАВИСИМО ОТ ВЕРДИКТА: спрашивается ЦВЕТ
        # РАМКИ СОСЕДА, а не число находок пробы
        def _рамки(из):
            из_ = set()
            for д in из.values():
                if д:
                    for с in (д["геом"].get("соседи") or []):
                        из_.add(с.get("цвет_рамки"))
            return sorted(x for x in из_ if x)
        рч, рп = _рамки(из_ч), _рамки(из_п)
        print("")
        print("ДОКАЗАТЕЛЬСТВО ПОДЛОГА: цвета рамок СОСЕДЕЙ "
              "чисто %r  ->  с подлогом %r" % (рч, рп))
        if рч == рп:
            print("  ПОДЛОГ НЕ СОСТОЯЛСЯ: рамки соседей не изменились.")
            return 2
        print("ВЕРДИКТ: ширин с бедой  чисто %d  ->  с подлогом %d"
              % (чисто, с_подлогом))
        return 0 if с_подлогом else 1
    ...
    if МИНУС:
        ...
        if чисто:
            print("")
            print("ОСТАНОВЛЕНО: знак не читается и БЕЗ подлога — "
                  "контроль ничего не доказывает.")
            return 2
        print()
        из_п = asyncio.run(прогон_минуса(подлог=True))
        с_подлогом = печать_минуса(из_п)
        # ДОКАЗАТЕЛЬСТВО — НЕЗАВИСИМЫЙ ЗАМЕР ТОГО, ЧТО ПОДЛОГ СОБИРАЛСЯ
        # ИЗМЕНИТЬ: цвет подложки на снимке. «0 → 3 находки» это тот же
        # вердикт, напечатанный дважды (§6.0.3)
        пч = sorted({tuple(д["цвета"]["подложка"]) for д in из_ч.values() if д})
        пп = sorted({tuple(д["цвета"]["подложка"]) for д in из_п.values() if д})
        print("")
        print("ДОКАЗАТЕЛЬСТВО ПОДЛОГА: цвет подложки знака НА СНИМКЕ "
              "чисто %r  ->  с подлогом %r" % (пч, пп))
        print("  (подлог возвращает подложку `--surface-3` — поведение "
              "до починки, то есть дефект, снятый с полки)")
        if пч == пп:
            print("  ПОДЛОГ НЕ СОСТОЯЛСЯ: подложка не изменилась, "
                  "и вердикт пробы после него ничего не значит.")
            return 2
        print("ВЕРДИКТ: ширин с нечитаемым знаком  чисто %d  ->  "
              "с подлогом %d" % (чисто, с_подлогом))
        if с_подлогом:
            print("КОНТРОЛЬ ПРОЙДЕН: проба видит слипшийся знак.")
            return 0
        print("КОНТРОЛЬ НЕ ПРОЙДЕН: подлог не замечен — проба слепа.")
        return 1
    if ДОЛГИ:
        чисто = печать_долгов(asyncio.run(прогон_долгов()))
        if not КОНТРОЛЬ:
            return 1 if чисто else 0
        if чисто:
            print("")
            ...
            return 2
        print()
        из_п = asyncio.run(прогон_долгов(подлог=True))
        с_подлогом = печать_долгов(из_п)
        # ДОКАЗАТЕЛЬСТВО — НЕЗАВИСИМЫЙ ЗАМЕР: что лежит в точке центра
        # раскрытого снимка. «0 -> N находок» это тот же вердикт дважды
        точки = sorted({(з.get("слой") or {}).get("в_точке")
                        for з in из_п.values()} - {None})
        print("")
        print("ДОКАЗАТЕЛЬСТВО ПОДЛОГА: в точке центра снимка %r "
              "(без подлога там сам просмотрщик)" % (точки,))
        if not точки:
            print("  ПОДЛОГ НЕ СОСТОЯЛСЯ: снимок не раскрывался вовсе.")
            return 2
        print("ВЕРДИКТ: ширин с бедой  чисто %d  ->  с подлогом %d"
              % (чисто, с_подлогом))
        if с_подлогом:
            print("КОНТРОЛЬ ПРОЙДЕН: проба видит просмотр ПОД окном.")
            return 0
        print("КОНТРОЛЬ НЕ ПРОЙДЕН: подлог не замечен — проба слепа.")
        return 1
    if ХОД:
        итог_чисто = asyncio.run(прогон_хода())
        плохо = печать_хода(итог_чисто)
        if not КОНТРОЛЬ:
            return 0
        # КОНТРОЛЬ НА ЧИСТОЙ ОСНОВЕ НЕДЕЙСТВИТЕЛЕН, если перелив есть
        # и без подлога: «нашла» и «нашла подлог» тогда неотличимы
        if плохо:
            print("")
            print("ОСТАНОВЛЕНО: перелив есть и БЕЗ подлога — "
                  "контроль на грязной основе ничего не доказывает.")
            return 2
        print()
        итог_подлог = asyncio.run(прогон_хода(подлог=True))
        с_подлогом = печать_хода(итог_подлог)
        print()
        # ДОКАЗАТЕЛЬСТВО — НЕЗАВИСИМЫЙ ЗАМЕР, а не вердикт пробы.
        # «0 -> 2 находки» это тот же вердикт, напечатанный дважды;
        # ловить надо ТО, ЧТО ПОДЛОГ СОБИРАЛСЯ ИЗМЕНИТЬ (§6.0.3)
        чисто_поз = sorted({p for д in итог_чисто.values() if д
                            for p in (д.get("позиции") or [])})
        подлог_поз = sorted({p for д in итог_подлог.values() if д
                             for p in (д.get("позиции") or [])})
        print("ДОКАЗАТЕЛЬСТВО ПОДЛОГА: `position` подсказки за время "
              "закрытия  чисто %r  ->  с подлогом %r"
              % (чисто_поз, подлог_поз))
        print("  (подлог возвращает СНЯТИЕ инлайнового `position` в кадр "
              "смены класса — поведение до задачи 218)")
        if чисто_поз == подлог_поз:
            print("  ПОДЛОГ НЕ СОСТОЯЛСЯ: он ничего не изменил, "
                  "и вердикт пробы после него ничего не значит.")
            return 2
        print("ВЕРДИКТ: ширин с переливом на ходу  чисто %d  ->  "
              "с подлогом %d" % (плохо, с_подлогом))
        if с_подлогом:
            print("КОНТРОЛЬ ПРОЙДЕН: проба видит перелив НА ХОДУ.")
            return 0
        print("КОНТРОЛЬ НЕ ПРОЙДЕН: подлог не замечен — проба слепа.")
        return 1
    if ПЛАШКА:
        чисто = печать_плашки(asyncio.run(прогон_плашки()))
        if not КОНТРОЛЬ:
            return 1 if чисто else 0
        # КОНТРОЛЬ НА ГРЯЗНОЙ ОСНОВЕ НЕДЕЙСТВИТЕЛЕН: скачок, который
        # есть и без подлога, делает «нашла» неотличимым от «нашла
        # подлог» (§6.0.3)
        if чисто:
            print("\nОСТАНОВЛЕНО: скачок есть и БЕЗ подлога — "
                  "контроль ничего не доказывает.")
            return 2
        print()
        из_п = asyncio.run(прогон_плашки(подлог=True))
        с_подлогом = печать_плашки(из_п)
        # ДОКАЗАТЕЛЬСТВО — НЕЗАВИСИМЫЙ ЗАМЕР ТОГО, ЧТО ПОДЛОГ
        # СОБИРАЛСЯ ИЗМЕНИТЬ: сколько строк заняла плашка с длинным
        # именем. «0 → 3 находки» это тот же вердикт дважды
        строк = sorted({(з or {}).get("две", {}).get("строк")
                        for з in из_п.values()} - {None})
        print("\nДОКАЗАТЕЛЬСТВО ПОДЛОГА: строк в плашке при длинном имени "
              "с подлогом %r (без подлога везде 2)" % (строк,))
        if строк == [2]:
            print("  ПОДЛОГ НЕ СОСТОЯЛСЯ: плашка осталась двухстрочной, "
                  "и вердикт пробы после него ничего не значит.")
            return 2
        print("ВЕРДИКТ: ширин со скачком  чисто %d  ->  с подлогом %d"
              % (чисто, с_подлогом))
        if с_подлогом:
            print("КОНТРОЛЬ ПРОЙДЕН: проба видит скачок окна.")
            return 0
        print("КОНТРОЛЬ НЕ ПРОЙДЕН: подлог не замечен — проба слепа.")
        return 1
    if КАРТОЧКА:
        return печать_карточки(asyncio.run(прогон_карточки()))
    if ПЕРЕХОД:

...
КАРТОЧКА_ЗАМЕР = """(() => {
  ...
  const снять = (к) => {
    ...
    return {
      ...
      // ПУСТОЕ МЕСТО ПОД РЯДОМ ДЕЙСТВИЙ (BACKLOG №209, B.5). Пока
...
ПОРЯДОК_ЗАМЕР = """(() => {
  ...
  const имена = {'apt-ph': 'фото', 'apt-name': 'название',
    ...
    // не входит вовсе (BACKLOG №209, блок B)
...
async def прогон_карточки():
    ...
    async with async_playwright() as p:
        ...
        for ш in ШИРИНЫ:
            ...
            # нет — он уехал в окно (BACKLOG №209, блок B), и прежний
...
def печать_карточки(из):
    ...
    for ш, з in из.items():
        ...
        if п:
            ...
            # ниже ряда действий нет НИЧЕГО (BACKLOG №209, B.1)

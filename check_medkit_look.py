...
КАРТОЧКА_ЗАМЕР = """(() => {
  ...
  const снять = (к) => {
    ...
    return {
      ...
      пачки: !!к.querySelector('.apt-packs summary')
    };
  };
...
ПОРЯДОК_ЗАМЕР = """(() => {
  const к = [...document.querySelectorAll('.apt-card')]
    ...
    .find(x => x.querySelector('.apt-packs summary'));
  if (!к) return null;
  const имена = {'apt-ph': 'фото', 'apt-name': 'название',
    ...
    'apt-acts': 'ряд действий', 'apt-packs': 'упаковки'};
  const из = [];
  const обход = (у) => {
        ...
        if п:
            ...
            хвост = [x for x in п if x in
                     ("кнопка списания", "ступени тюбика", "дорожка",
                      "ряд действий", "упаковки")]
            print("  хвост (списание -> действия -> упаковки): %s  %s"
                  % (" -> ".join(хвост),
                     "ВЕРНО" if хвост[-2:] == ["ряд действий", "упаковки"]
                     else "РАЗОШЁЛСЯ"))
        if not з["раскрыли"]:
            print("  D.1 РАСКРЫТИЕ: группы с пачками на экране нет — "
                  "вопрос не задавался")
        else:
            ...
            print("  D.1 РАСКРЫТИЕ (карточка «%s»):"
                  % (з["до"][своя]["имя"] if своя is not None else "?"))
            print("      ВНУТРИ неё (отсчёт от верха карточки — это и есть")
            print("      вопрос D.1): полоска %.1f px, действия %.1f px"
                  % (оп, од))

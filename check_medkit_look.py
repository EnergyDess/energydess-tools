...
async def _ход_на_подсказке(pg, сенсор):
    ...
    открыто = await pg.evaluate(ХОД_СНЯТЬ)
    ...
    await pg.wait_for_timeout(900)
    закрытие = await pg.evaluate(ХОД_СНЯТЬ)
...
def main():
    if ХОД:
        плохо = печать_хода(asyncio.run(прогон_хода()))
        if not КОНТРОЛЬ:
            return 0
        ...
        if плохо:
            ...
            return 2
        print()
        с_подлогом = печать_хода(asyncio.run(прогон_хода(подлог=True)))
        print()
        print("ДОКАЗАТЕЛЬСТВО ПОДЛОГА: он возвращает СНЯТИЕ `position` "
              "в кадр смены класса — ровно поведение до задачи 218.")

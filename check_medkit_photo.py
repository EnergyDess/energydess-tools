...
проду (`[[vm]] memory` ЧИТАЕТСЯ ИЗ `fly.toml`), и в него уходит файл того
...
def _память_прода_мб():
    """Сколько памяти у боевой машины — ИЗ `fly.toml`, а не числом.

    Здесь стояло `256` с комментарием «как в fly.toml», и 2026-09-05
    это разошлось: машину подняли до 512, а проба осталась на 256 —
    то есть держала потолок, «равный проду», который проду больше
    не равен. Число в двух местах, которое правят порознь, — та же
    ловушка, что у retention снимков тома (§10).

    Комментарии снимаются: в них лежит РАЗБОР прежних значений,
    и плоское чтение взяло бы за настройку первое попавшееся число.
    """
    путь = os.path.join(КОРЕНЬ, "fly.toml")
    try:
    ...
    без = chr(10).join(строки)
    блок = re.search(r"\[\[vm\]\](.*?)(?=" + chr(10) + r"\[|\Z)", без, re.S)
    if not блок:
        return None
    м = re.search(r'memory\s*=\s*"?(\d+)\s*mb"?', блок.group(1),
                  re.I)
    return int(м.group(1)) if м else None


_из_файла = _память_прода_мб()
if _из_файла is None:
    # «НЕ ПРОЧИТАЛОСЬ» НЕ РАВНО «ПУСТЬ БУДЕТ 256»: молчаливое
    # умолчание вернуло бы ровно ту рассинхронизацию, ради снятия
    # которой всё это и написано (§6.0.1)
    raise SystemExit("ОСТАНОВЛЕНО: не прочитал [[vm]] memory из fly.toml — "
                     "потолок пробы задать нечем")
ПОТОЛОК_МБ = int(os.environ.get("MEDKIT_PHOTO_CAP", str(_из_файла)))

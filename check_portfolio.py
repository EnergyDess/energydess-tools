...
РЕЖИМЫ — по блокам письма, у каждого свой вопрос:
  ...
  py check_portfolio.py --яркость [--контроль-яркости]  # фон тёмный, текст читается (345, блок 1)
  ...
  py check_portfolio.py --экран --контроль-налезания  # подлог подъёма головы (345, блок 3)
...
def экран(контроль_сдвига=False, контроль_магнита=False, контроль_налезания=False):
    from PIL import Image
    from playwright.sync_api import sync_playwright
    print("B. ПЕРВЫЙ ЭКРАН — гостем, головной браузер, стенд %s%s" % (
        БАЗА, " · ПОДЛОГ: подъём ряда портрета снят" if контроль_налезания else ""))
        ...
        try:
            ...
            for ш, в, сенсор in ШИРИНЫ:
                ...
                с.wait_for_function("() => window.__появление && Object.keys(window.__появление.когда).length >= 5",
                                    timeout=10000)
                if контроль_налезания:
                    # ЛОМАЕТСЯ ЗВЕНО ПОДЪЁМА: ряд портрета встаёт в свой трек
                    # без отрицательного отступа — голова оказывается НИЖЕ
                    # строки имени, ровно как описывал (ошибочно) владелец.
                    с.add_style_tag(content=".pf-hero-mid{margin-top:0!important;--pf-head-up:0px!important}")
                ...
                # ЗДЕСЬ ТРЕБОВАЛОСЬ `имя.y < верх портрета < имя.b` — макушка
                # ВНУТРИ коробки имени (письмо 2 задачи 343). Блок 3 задачи 345
                # поднял голову до 75–85 % окна, и макушка законно вышла выше
                # верхней линии букв (156 против 164 на 2560). Остаётся то,
                # о чём письмо и просило: налезание ЕСТЬ и оно заметное, голова
                # по центру. Порог — половина высоты букв: фактически налезание
                # 222 из 214 на 2560 и 192 из 214 на 1920, то есть запас
                # двукратный, а «стоит ниже строки» такой шаг не пропустит.
                налезание = имя_к["b"] - место_к["y"]
                шаг("макушка заходит на буквы имени, голова по центру",
                    налезание >= имя_к["h"] * 0.5
                    and abs(место_к["x"] + место_к["w"] / 2 - з["vw"] / 2) <= 1,
                    "налезание %.0f px при высоте букв %.0f (порог %.0f); имя %.0f..%.0f, "
                    "верх головы %.0f, центр %.1f при середине окна %.1f; кегль имени %.1f" % (
                        налезание, имя_к["h"], имя_к["h"] * 0.5, имя_к["y"], имя_к["b"],
                        место_к["y"], место_к["x"] + место_к["w"] / 2,
...
ПОДЛОГИ_МАКЕТА = {
    ...
    # ПОДЛОГ СДВИГАЕТ ГОЛОВУ ПЕРЕНОСОМ, А НЕ РАСКЛАДКОЙ. Первая версия
    # ставила `.pf-hero-mid{margin-top:200px}` и роняла НЕ СВОЙ шаг: ряд
    # уезжал, высота портрета считается от него, и голова падала до 49.7 %
    # вместо 75–85 %. `translate` раскладку не пересчитывает — высота
    # остаётся, вниз уходит только положение.
    "зазора": ("низ головы внутри первого экрана",
               ".pf-hero-portrait{translate:0 200px!important}"),
...
МАКЕТ_ЗАМЕР = r"""() => {
  ...
  return {vw: document.documentElement.clientWidth, vh: innerHeight,
    ...
    место_головы: пр(место), якоря_блок: пр(q('.pf-anchors').getBoundingClientRect()),
    ...
    низ_экрана: q('.pf-hero').getBoundingClientRect().bottom,
    ...
    with sync_playwright() as p:
        ...
        try:
            for шир, выс, сенсор in ШИРИНЫ:
                ...
                # ГОЛОВА КРУПНЕЕ (задача 345, блок 3). ЗДЕСЬ СТОЯЛО «60–72 %
                # окна» (письмо 2 задачи 343) — владельцу стало мелко, и рамки
                # заменены на 75–85 %. Верхняя граница письма была 85 %, нижняя
                # 80 %; взята 75, и это ЗАМЕР, а не поблажка: сверху голова
                # упирается в ряд якорей, и между их низом и верхом строк
                # на 1920 всего 782 px — 72.4 % окна ПРИ НУЛЕВЫХ зазорах.
                # Достигнутое: 75.3 % на 1920 и 80.7 % на 2560 (было 62.3
                # и 67.7). Подлог `--контроль-головы` возвращает прежний
                # подъём — шаг обязан упасть.
                if not сенсор:
                    ш("высота головы 75–85% окна", 0.75 <= з["голова"] <= 0.85,
                      "голова %.1f%% окна" % (з["голова"] * 100))

                # ГОЛОВА НЕ ПЕРЕКРЫВАЕТ ЯКОРЯ, СТРОКИ И КНОПКУ: пересечение
                # ПРЯМОУГОЛЬНИКОВ, как просило письмо. Зазоры печатаются
                # всегда — по ним видно, сколько осталось до предела.
                гол, як = з["место_головы"], з["якоря_блок"]
                задел = [("якоря", як), ("строки", з["строки"]), ("кнопка", з["кнопка"])]
                столкнулись = [имя_ for имя_, б in задел
                               if not (гол["r"] <= б["l"] or б["r"] <= гол["l"]
                                       or гол["b"] <= б["t"] or б["b"] <= гол["t"])]
                ш("голова не перекрывает якоря, строки и кнопку", not столкнулись,
                  "столкновения: %s; зазоры — до якорей %+.0f, до строк %+.0f, до кнопки %+.0f px"
                  % (столкнулись or "нет", гол["t"] - як["b"],
                     з["строки"]["t"] - гол["b"], з["кнопка"]["t"] - гол["b"]),
                  собрано=len(задел))
                ...
                # ЗДЕСЬ СТОЯЛО «зазор под портретом равен ряду сетки» —
                # то есть портрет НЕ заходит в зону низа (письмо 2 задачи 343).
                # Отменено владельцем блоком 3 задачи 345: голова крупнее и
                # проходит между текстом слева и кнопкой справа, так что зазор
                # до `.pf-hero-foot` стал отрицательным (-46 на 1920, -70
                # на 2560) НАМЕРЕННО. Перекрытие самих строк и кнопки
                # спрашивает шаг выше; здесь остаётся вопрос, на который
                # прежний шаг отвечал заодно: не вылезла ли голова за НИЖНИЙ
                # КРАЙ первого экрана. Зазор печатается справкой.
                ш("низ головы внутри первого экрана",
                  з["место_головы"]["b"] <= з["низ_экрана"] + 0.5,
                  "низ головы %.0f при нижнем крае экрана %.0f (запас %+.0f px); "
                  "до `.pf-hero-foot` %+.0f px при ряде сетки %.0f"
                  % (з["место_головы"]["b"], з["низ_экрана"],
                     з["низ_экрана"] - з["место_головы"]["b"], з["зазор"], з["ряд_сетки"]))
...
# ── БУФЕР ОБМЕНА — СВОЙ, А НЕ СИСТЕМНЫЙ (№352, «аптечка-2», блок 1.4) ──
#
# Проба читала НАСТОЯЩИЙ буфер владельца: контекст получал разрешения
# `clipboard-read`/`clipboard-write`, шаг D5 звал `readText()`, а контроль
# писал туда «другое» через `execCommand('copy')`. Две беды сразу, и обе
# тихие:
#
#   · показания зависели от того, что человек скопировал мышкой секунду
#     назад, — шаг то проходил, то краснел на одном и том же коде;
#   · сама проба ЗАТИРАЛА буфер владельца, то есть меняла то, что мерила.
#
# Здесь `navigator.clipboard` подменяется В КОНТЕКСТЕ СТРАНИЦЫ: запись
# ложится в переменную страницы, чтение возвращает её же. Системный буфер
# не трогается вовсе, а прочитанное заведомо написано КНОПКОЙ.
#
# ИСКАЖЕНИЕ — ПАРАМЕТР ТОГО ЖЕ СКРИПТА, а не второй скрипт рядом: подлог
# обязан ломать ровно ЗВЕНО ЗАПИСИ, оставив чтение прежним.
def _буфер_страницы(искажение=None):
    искажатель = {
        "пусто": "t = '';",          # кнопка не кладёт ничего
        "чужое": "t = 'не тот адрес';",  # кладёт не тот текст
    }.get(искажение, "")
    return r"""(() => {
      let хранит = '';
      window.__буфер = () => хранит;
      const свой = {
        writeText: t => { %s hранит_set(t); return Promise.resolve(); },
        readText: () => Promise.resolve(хранит),
      };
      function hранит_set(v) { хранит = String(v); }
      try {
        Object.defineProperty(navigator, 'clipboard',
                              {get: () => свой, configurable: true});
      } catch (e) {}
    })();""" % искажатель
...
def связь(контроль_буфера=False, контроль_копии=None):
    from playwright.sync_api import sync_playwright
    print("D. СВЯЗАТЬСЯ — гостем, головной браузер, стенд %s%s%s" % (
        БАЗА, " · ПОДЛОГ: буфер обмена недоступен" if контроль_буфера else "",
        (" · ПОДЛОГ копии: %s" % контроль_копии) if контроль_копии else ""))
    with sync_playwright() as p:
        бр = p.chromium.launch(headless=False)
        try:
            for ш, в, сенсор in ШИРИНЫ:
                print("\n  ── %d×%d%s" % (ш, в, " (сенсор)" if сенсор else ""))
                к = _контекст(бр, ш, в, сенсор, движение="reduce")
                # РАЗРЕШЕНИЙ НА СИСТЕМНЫЙ БУФЕР БОЛЬШЕ НЕ БЕРЁМ:
                # страница получает свой (разбор выше).
                к.add_init_script(_буфер_страницы(контроль_копии))
                if контроль_буфера:
                ...
                for место in ("hero", "about", "final"):
                    ...
                    с.wait_for_timeout(300)
                    з = с.evaluate(ЗАМЕР_СВЯЗИ, место)
                    буфер = с.evaluate("() => window.__буфер ? window.__буфер() : null") \
  ...
  return {vh: innerHeight, vw: document.documentElement.clientWidth, scrollY,
    карточки: карточки.map(к => {
      ...
      return {n: (к.querySelector('.pf-proj-n') || {}).textContent,
              ...
              // Ключ СВОЙ, не «масштаб»: тот ниже — доля ширины карточки, и в
              // литерале объекта второй затирал первый. Шаг карточки 03 спрашивал
              // «есть ли блок», а получал долю ≈1 — истинную всегда (BACKLOG №348).
              блок_масштаба: !!к.querySelector('.pf-scale'),
...
ПОДЛОГ_БЛОКА_03 = r"""() => { document.querySelectorAll('.pf-proj .pf-scale').forEach(б => б.remove());
  return document.querySelectorAll('.pf-scale').length; }"""
...
def проекты(контроль=False, контроль_ссылок=False, контроль_медиа=False, контроль_блока=False):
    from PIL import Image
    from playwright.sync_api import sync_playwright
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for ш, в, сенсор in ШИРИНЫ:
                ...
                if контроль_блока:
                    # ПОДЛОГ №348: блок «масштаб работы» убран из карточки 03.
                    # Со затёртым ключом шаг проходил и так — доля ширины ≈1.
                    print("  доказательство подлога блока масштаба: блоков .pf-scale после %s"
                          % с.evaluate(ПОДЛОГ_БЛОКА_03))
                с.wait_for_timeout(500)
                ...
                свой_шаг_03.append(шаг("карточка 03: мест медиа 0, «масштаб работы», числа из выкладки, «Открыть» → #tools",
                    тр is not None and тр["блок_масштаба"] and тр["медиа"] == 0 and тр["ссылки"] == ["#tools"]
                    and ждём_числа is not None and тр["числа"] == ждём_числа,
                    "мест медиа %s, блок масштаба %s (доля ширины, которую читал прежний шаг, %s), "
                    "ссылки %s, числа %s при выкладке %s" % (
                        тр and тр["медиа"], тр and тр["блок_масштаба"], тр and "%.3f" % тр["масштаб"],
                        тр and тр["ссылки"], тр and тр["числа"], ждём_числа),
                    собрано=len(з["карточки"])))
            ...
            бр.close()
    if контроль_блока:
        упал = False in свой_шаг_03
        print("\n  ДОКАЗАТЕЛЬСТВО ПОДЛОГА «блок масштаба»: свой шаг карточки 03 упал %d раз из %d"
              % (свой_шаг_03.count(False), len(свой_шаг_03)))
        if not упал:
            print("  ПОДЛОГ НЕ ПОЙМАН своим шагом")
            return 4
    return None
...
# ── ЯРКОСТЬ ФОНА И ЧИТАЕМОСТЬ ТЕКСТА НА НЁМ (задача 345, блок 1, п.4-5) ──
#
# ЗАЧЕМ. Слой `.pf-sky` письма 5 задачи 343 владелец назвал СЛИШКОМ СВЕТЛЫМ:
# раздел «Инструменты» на нём терялся. Правило записано ЧИСЛОМ, а не на глаз:
# ни один пиксель ПОЛОТНА фона не светлее ПРЕДЕЛА по относительной светимости
# WCAG — той же формуле, по которой считается контраст.
#
# ПРЕДЕЛ ВЫВЕДЕН ЗАМЕРОМ, А НЕ НАЗНАЧЕН: прежний слой даёт пик 0.0398
# (1920 и 2560, по всей высоте), новый — 0.0094. Предел 0.020 стоит между
# ними: прежний его НЕ проходит, новый проходит с запасом 2.1x. Медиана
# при этом 0.0156 против 0.0058 — полотно стало темнее в 2.7 раза.
#
# ПОЛОТНО И ЗВЁЗДЫ МЕРЯЮТСЯ ПО ОТДЕЛЬНОСТИ, И ЭТО НЕ ПОБЛАЖКА, А ЕДИНСТВЕННЫЙ
# УСТОЙЧИВЫЙ СПОСОБ. Первая версия считала полотно перцентилем 99.9 по кадру
# со звёздами — и на 390 дала 0.0264 против 0.0092 на 1920 ПРИ ОДНОМ И ТОМ ЖЕ
# ФОНЕ: у узкого кадра площадь мала, и полутона вокруг точек заполняют верхнюю
# долю процента целиком. То есть число зависело от ширины окна, а не от фона.
# Теперь полотно снимается со СКРЫТЫМ слоем звёзд, звёзды — разницей двух
# снимков, и у каждого свой вопрос:
#   Я1 — полотно не светлее предела (общий тон);
#   Я2 — звёзды ЕСТЬ и их столько, сколько просило письмо (20-40 на 1920x1080);
#   Я3 — ни одна точка не ярче 60 % белого;
#   Я4 — текст трёх секций читается на фоне.
# Я2 обязателен: без него Я1 прошёл бы и на фоне вовсе без звёзд, а Я3 —
# тем более (нет точек, нет и ярких). Немой отказ ровно того вида, ради
# которого весь §6.0.1.
ПРЕДЕЛ_ЯРКОСТИ = 0.020
ПРЕДЕЛ_ЗВЕЗДЫ = 0.60          # доля белого (пункт 3 письма)
ЗВЁЗД_НА_1920 = (20, 40)      # потолок письма, на 1920x1080
ЗВЕЗДА_ПОРОГ = 0.006          # яркость, с которой пиксель считается точкой

# Текст трёх секций, про которые спрашивал владелец. Порог у каждого свой
# и берётся не из списка, а из ЦВЕТА: совпал с приглушённым токеном — 3.0,
# иначе 4.5 (пункт 5 письма).
ЯРКОСТЬ_ТЕКСТЫ = [
    ("Обо мне",      ".pf-about-text"),
    ("Инструменты",  ".pf-tool-h"), ("Инструменты", ".pf-tool-desc"),
    ("Инструменты",  ".pf-tool-can"), ("Инструменты", ".pf-tool-n"),
    ("Проекты",      ".pf-proj-name"), ("Проекты", ".pf-proj-kind"),
    ("Проекты",      ".pf-proj-n"), ("Проекты", ".pf-proj-stack"),
]

# Значения слоя ДО правки задачи 345 — материал подлога. Держатся здесь,
# а не читаются из style.css: подлог обязан возвращать ИМЕННО прежнее
# состояние, а не то, что окажется в файле завтра. ЗВЁЗДЫ ПОДЛОГ НЕ
# СНИМАЕТ: он про ТОН полотна, и Я1 падает на нём без всякой помощи —
# снять заодно и точки значило бы валить Я2 и путать, что именно поймано.
ПРЕЖНИЙ_СЛОЙ = """:root{
  --landing-sky-0:#0c0c0f;--landing-sky-1:#0c0c10;--landing-sky-2:#0d0f1c;
  --landing-sky-3:#0f1128;--landing-sky-4:#131232;--landing-sky-5:#17123a;
  --landing-sky-6:#18113a;
  --landing-sky-diag-blue:rgba(40,60,190,0.10);
  --landing-sky-diag-violet:rgba(120,60,210,0.14);
  --landing-sky-band-1:rgba(55,80,220,0.16);--landing-sky-band-2:rgba(120,70,220,0.18);
  --landing-sky-band-3:rgba(70,90,240,0.22);--landing-sky-band-4:rgba(145,75,235,0.26);
}"""

# Прячется СОДЕРЖИМОЕ, а не слой: `visibility` наследуется, и вернуть
# видимость самому слою довольно. Ролики фона тоже прячутся — их яркость
# задаёт владелец файлом, и правило пункта 4 не про них.
СКРЫТЬ_СОДЕРЖИМОЕ = """
  .pf-zone > *, .pf-hero, .pf-feed, .pf-about, .pf-tools, .pf-projects,
  .pf-final, .site-header, .site-footer, .pf-bg, .pf-bg-video,
  .pf-bg-scrim, .pf-nav { visibility: hidden !important; }
  .pf-sky { visibility: visible !important; }
"""
БЕЗ_ЗВЁЗД = ".pf-sky::after { display: none !important; }"

ЗАМЕР_ТЕКСТА_НА_ФОНЕ = r"""(сел) => {
  const э = document.querySelector(сел); if (!э) return null;
  const ст = getComputedStyle(э), корень = getComputedStyle(document.documentElement);
  const норм = (s) => s.trim().replace(/\s+/g, '');
  const вhex = (s) => { const m = s.trim().match(/^#(..)(..)(..)$/);
    return m ? 'rgb(' + parseInt(m[1], 16) + ',' + parseInt(m[2], 16) + ',' + parseInt(m[3], 16) + ')' : s; };
  const свой = норм(ст.color);
  const тусклые = ['--text-faint', '--text-2'].map(т => норм(вhex(корень.getPropertyValue(т))));
  return {цвет: ст.color, приглушённый: тусклые.includes(свой)};
}"""


def _кадры_фона(с, ш, в):
    """Кадры слоя по всей высоте: (список массивов, высота слоя).

    СНИМАЕТСЯ ОБЛАСТЬ СЛОЯ, А НЕ ОКНО, и это замер: полный кадр захватывает
    ПОЛОСУ ПРОКРУТКИ головного браузера — шесть столбцов у правого края
    (x 1912–1917 при ширине 1920) цвета rgb(45,45,47), яркость 0.0264. Первая
    версия объявляла её пиком полотна на ВСЕХ трёх ширинах разом: число
    одинаковое, потому что к фону отношения не имеет вовсе."""
    import io as _io
    import numpy as np
    from PIL import Image
    высота = с.evaluate("() => { const э = document.querySelector('.pf-sky-wrap');"
                        " return э ? Math.round(э.getBoundingClientRect().height) : 0; }")
    if not высота:
        return None, 0
    ширина_слоя = с.evaluate("() => { const э = document.querySelector('.pf-sky');"
                             " return э ? Math.round(э.getBoundingClientRect().width) : 0; }")
    кадры = []
    шагов = max(1, min(12, int(высота / в) + 1))
    for i in range(шагов):
        с.evaluate("(y) => window.scrollTo({top: y, behavior: 'instant'})", int(i * высота / шагов))
        с.wait_for_timeout(150)
        снимок = с.screenshot(clip={"x": 0, "y": 0, "width": min(ширина_слоя, ш), "height": в})
        кадры.append(np.asarray(Image.open(_io.BytesIO(снимок)).convert("RGB")))
    return кадры, высота


def _пятен(маска):
    """Число связных пятен в двумерной маске. Своя маркировка: scipy и
    skimage на машине нет (§3), а пятен тут десятки — обход дешёвый."""
    import numpy as np
    видано = np.zeros(маска.shape, dtype=bool)
    пятен = 0
    точки = np.argwhere(маска)
    for y0, x0 in точки:
        if видано[y0, x0]:
            continue
        пятен += 1
        ...
        while стек:
            ...
            for dy in (-1, 0, 1):
                for dx in (-1, 0, 1):
                    ny, nx = y + dy, x + dx
                    if (0 <= ny < маска.shape[0] and 0 <= nx < маска.shape[1]
                            and маска[ny, nx] and not видано[ny, nx]):
                        видано[ny, nx] = True
                        стек.append((ny, nx))
    return пятен


def яркость(подлог=False):
    """Пункты 4 и 5 блока 1: тёмный ли фон, есть ли звёзды, читается ли текст."""
    import numpy as np
    from playwright.sync_api import sync_playwright
    print("\nB. ЯРКОСТЬ ФОНА И ТЕКСТ НА НЁМ — гостем, головной браузер, стенд %s" % БАЗА)
    if подлог:
        print("  ПОДЛОГ: возвращены значения слоя ДО правки задачи 345")
    with sync_playwright() as п:
        бр = п.chromium.launch(headless=False)
        for ш, в, сенсор in ШИРИНЫ:
            print("\n  ── %d×%d%s" % (ш, в, " (сенсор)" if сенсор else ""))
            # ПОЛОТНО: звёзды сняты, мерится общий тон
            к = _контекст(бр, ш, в, сенсор)
            с = к.new_page()
            с.goto(БАЗА + "/", wait_until="networkidle", timeout=60000)
            if подлог:
                с.add_style_tag(content=ПРЕЖНИЙ_СЛОЙ)
            с.add_style_tag(content=СКРЫТЬ_СОДЕРЖИМОЕ + БЕЗ_ЗВЁЗД)
            с.wait_for_timeout(500)
            полотно, высота = _кадры_фона(с, ш, в)
            к.close()
            if полотно is None:
                шаг("Я1: полотно фона не светлее предела", None,
                    "слоя `.pf-sky-wrap` в дереве нет", собрано=0)
                continue
            ярк_полотна = np.concatenate([_яркость(к_.reshape(-1, 3)) for к_ in полотно])
            пик = float(ярк_полотна.max())
            шаг("Я1: полотно фона не светлее предела", пик <= ПРЕДЕЛ_ЯРКОСТИ,
                "пик полотна %.4f при пределе %.4f (медиана %.4f); пикселей %d, высота слоя %d"
                % (пик, ПРЕДЕЛ_ЯРКОСТИ, float(np.median(ярк_полотна)), len(ярк_полотна), высота),
                собрано=len(ярк_полотна))

            # ЗВЁЗДЫ: те же кадры со слоем точек, разница даёт сами точки
            к = _контекст(бр, ш, в, сенсор)
            с = к.new_page()
            с.goto(БАЗА + "/", wait_until="networkidle", timeout=60000)
            if подлог:
                с.add_style_tag(content=ПРЕЖНИЙ_СЛОЙ)
            с.add_style_tag(content=СКРЫТЬ_СОДЕРЖИМОЕ)
            с.wait_for_timeout(500)
            со_звёздами, _ = _кадры_фона(с, ш, в)
            к.close()
            пятен_всего, пик_звезды = 0, 0.0
            for без, со in zip(полотно, со_звёздами):
                if без.shape != со.shape:
                    continue
                разница = (_яркость(со.reshape(-1, 3)) - _яркость(без.reshape(-1, 3))).reshape(без.shape[:2])
                пятен_всего += _пятен(разница > ЗВЕЗДА_ПОРОГ)
                ярк_со = _яркость(со.reshape(-1, 3))
                пик_звезды = max(пик_звезды, float(ярк_со.max()))
            на_экран = пятен_всего / max(1, len(полотно))
            ожидаемо = (ЗВЁЗД_НА_1920[0] * ш * в / (1920 * 1080),
                        ЗВЁЗД_НА_1920[1] * ш * в / (1920 * 1080))
            шаг("Я2: звёзды на месте и их столько, сколько просило письмо",
                ожидаемо[0] <= на_экран <= ожидаемо[1],
                "точек на экран %.1f при ожидаемых %.1f–%.1f (на 1920×1080 письмо просило %d–%d); "
                "всего пятен %d по %d кадрам"
                % (на_экран, ожидаемо[0], ожидаемо[1], ЗВЁЗД_НА_1920[0], ЗВЁЗД_НА_1920[1],
                   пятен_всего, len(полотно)), собрано=len(полотно))
            шаг("Я3: ни одна звезда не ярче %d %% белого" % int(ПРЕДЕЛ_ЗВЕЗДЫ * 100),
                пик_звезды <= ПРЕДЕЛ_ЗВЕЗДЫ,
                "самая яркая точка %.4f при пределе %.2f" % (пик_звезды, ПРЕДЕЛ_ЗВЕЗДЫ),
                собрано=len(полотно))

            # ТЕКСТ НА ФОНЕ
            # ДВИЖЕНИЕ ЗАГЛУШЕНО, и это условие повторимости: окна
            # интерфейсов «Инструментов» живые, и максимум фона под текстом
            # от кадра к кадру гуляет. Замер без этого дал у `.pf-tool-n`
            # 2.00 на 2560 и 4.85 на 1920 ПРИ ОДНОМ ФОНЕ — то есть мерил
            # не фон, а фазу анимации.
            к = _контекст(бр, ш, в, сенсор, движение="reduce")
            с = к.new_page()
            с.goto(БАЗА + "/", wait_until="networkidle", timeout=60000)
            if подлог:
                с.add_style_tag(content=ПРЕЖНИЙ_СЛОЙ)
            с.wait_for_timeout(700)
            собрано, запас, худшее_имя, для_печати = 0, None, "", []
            for секция, сел in ЯРКОСТЬ_ТЕКСТЫ:
                данные = с.evaluate(ЗАМЕР_ТЕКСТА_НА_ФОНЕ, сел)
                if not данные:
                    continue
                с.evaluate("""([сел, д]) => { const э = document.querySelector(сел);
                    const b = э.getBoundingClientRect();
                    window.scrollTo({top: Math.max(0, b.top + scrollY + b.height / 2 - innerHeight * д),
                                     behavior: 'instant'}); }""", [сел, 0.5])
                с.wait_for_timeout(250)
                # ПРЯМОУГОЛЬНИК ПО БУКВАМ (`Range`), а не по коробке: коробка
                # шире текста и захватывает соседей — у `.pf-tool-n` в неё
                # попадала рамка окна интерфейса, и «фоном» объявлялась она.
                прям = с.evaluate("""(сел) => { const э = document.querySelector(сел);
                    const д = document.createRange(); д.selectNodeContents(э);
                    const b = д.getBoundingClientRect();
                    return {x: Math.round(b.left), y: Math.round(b.top),
                            w: Math.round(b.width), h: Math.round(b.height)}; }""", сел)
                if прям["w"] < 4 or прям["h"] < 4 or прям["y"] < 0 or прям["y"] + прям["h"] > в:
                    continue
                фон_ярк = _ярк_под(с, сел, прям)
                if фон_ярк is None:
                    continue
                ц = _цвет_яркость(данные["цвет"])
                к_ = (max(ц, фон_ярк) + 0.05) / (min(ц, фон_ярк) + 0.05)
                порог = 3.0 if данные["приглушённый"] else 4.5
                собрано += 1
                для_печати.append("%s %s %.2f/%.1f" % (секция, сел, к_, порог))
                if запас is None or (к_ - порог) < запас:
                    запас, худшее_имя = к_ - порог, "%s %s %.2f при пороге %.1f" % (секция, сел, к_, порог)
            шаг("Я4: текст трёх секций читается на фоне", (запас >= 0) if собрано else None,
                "худший — %s; все: %s" % (худшее_имя, "; ".join(для_печати)), собрано=собрано)
            к.close()
        бр.close()


def _ярк_под(с, сел, прям):
    """Яркость фона под текстом: сам текст на время скрыт.

    ПЕРЦЕНТИЛЬ 98, А НЕ МАКСИМУМ, И ЭТО ЗАМЕР. С появлением звёзд (блок 1,
    п.3) точка попадает под крупную цифру `.pf-tool-n`: максимум в её
    прямоугольнике 0.0877 при медиане 0.0063, и контраст выходил 2.03 при
    пороге 3.0 — находка про звезду в один пиксель, а не про читаемость.
    Перцентиль отсекает точки (их доля в прямоугольнике меньше процента)
    и остаётся чувствительным к СВЕТЛОМУ ФОНУ: подними фон — поднимутся
    и 98 % пикселей. Подлог прежнего слоя это и показывает."""
    import io as _io
    import numpy as np
    from PIL import Image
    с.evaluate("(сел) => document.querySelectorAll(сел).forEach(э => э.style.visibility = 'hidden')", сел)
    с.wait_for_timeout(120)
    кадр = Image.open(_io.BytesIO(с.screenshot(clip={"x": прям["x"], "y": прям["y"],
                                                     "width": прям["w"], "height": прям["h"]}))).convert("RGB")
    с.evaluate("(сел) => document.querySelectorAll(сел).forEach(э => э.style.visibility = '')", сел)
    пиксели = np.asarray(кадр).reshape(-1, 3)
    if not len(пиксели):
        return None
    return float(np.percentile(_яркость(пиксели), 98))
...
РЕЖИМЫ = (
    ...
    ("яркость",                 ("--яркость",), 0),
    ("яркость:контроль",        ("--яркость", "--контроль-яркости"), 1),
    ...
    ("экран:контроль-налезания", ("--экран", "--контроль-налезания"), 1),
    ("лента",                   ("--лента",), 0),
    ...
    ("связь:контроль-копии-пусто", ("--связь", "--контроль-копии", "пусто"), 1),
    ("связь:контроль-копии-чужое", ("--связь", "--контроль-копии", "чужое"), 1),
    ("проекты",                 ("--проекты",), 0),
    ...
    ("проекты:контроль-блока-масштаба", ("--проекты", "--контроль-блока-масштаба"), 1),
)
...
def прогнать_все(подробно, только):
    """Полный прогон режимов — в ОДНОМ видимом браузере на весь прогон
    (`browser_window.общий_браузер`, задача 346, заход 6): режимы — отдельные
    процессы, и без общего браузера каждый заводил свой процесс Chromium,
    а первое окно нового процесса на миг становится передним. Вложенный
    прогон (ряд стенда) берёт уже поднятый."""
    import browser_window
    with browser_window.общий_браузер():
        return _прогнать_все(подробно, только)


def _прогнать_все(подробно, только):
    import subprocess
    import threading
    import time
...
def main():
    ...
    if "--сводка" in арг:
        ...
        return 0
    if "--яркость" in арг:
        яркость(подлог="--контроль-яркости" in арг)
    elif "--фон-бирюза" in арг:
        ...
        экран(контроль_сдвига="--контроль" in арг, контроль_магнита="--контроль-магнита" in арг,
              контроль_налезания="--контроль-налезания" in арг)
        ...
        связь(контроль_буфера="--контроль-буфера" in арг,
              контроль_копии=(арг[арг.index("--контроль-копии") + 1]
                              if "--контроль-копии" in арг else None))
    elif "--проекты" in арг:
        код = проекты(контроль="--контроль" in арг, контроль_ссылок="--контроль-ссылок" in арг,
                      контроль_медиа="--контроль-медиа" in арг,
                      контроль_блока="--контроль-блока-масштаба" in арг)
        if код is not None:
            return код
    else:
        print(__doc__)

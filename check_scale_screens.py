    ...
    with sync_playwright() as p:
        ...
        отчёт.append(("минимальный кегль в блоке, px",
                      стр.evaluate("""() => {
                        ...
                        return м; }""")))
        # ── Подписи зон (2026-08-19, вторым заходом) ───────────────────
        # Ряда два, и виден ровно один: названия ВСЕХ зон под своими
        # зонами либо, когда карточка узка, названия крайних. Считаем оба
        # и сверяем, что они не показаны одновременно: два ряда подписей
        # разом — не «чуть хуже», а разные утверждения об одной полосе.
        # Перелив считается ПО КАЖДОЙ подписи: подпись, которая не влезла
        # в свою зону, стоит не под ней, а порог подписей тогда выбран
        # неверно — и увидеть это по одному общему числу нельзя
        подписи = стр.evaluate("""() => {
            let сНазваниями = 0, сКраями = 0, оба = 0, перелив = 0, минКегль = 99;
            const узкие = [];
            for (const к of document.querySelectorAll('#wt-body-grid .wt-bcell')) {
              const имена = к.querySelector('.scale-names');
              const края  = к.querySelector('.scale-ends');
              const виднаИмена = имена && getComputedStyle(имена).display !== 'none';
              const виднаКрая  = края  && getComputedStyle(края).display  !== 'none';
              if (виднаИмена) сНазваниями++;
              if (виднаКрая)  сКраями++;
              if (виднаИмена && виднаКрая) оба++;
              for (const ряд of [виднаИмена ? имена : null, виднаКрая ? края : null]) {
                if (!ряд) continue;
                for (const п of ряд.children) {
                  минКегль = Math.min(минКегль, parseFloat(getComputedStyle(п).fontSize));
                  if (п.scrollWidth > п.clientWidth + 1) {
                    перелив++;
                    узкие.push(к.querySelector('.wt-bname').textContent + ' / ' + п.textContent);
                  }
                }
              }
            }
            const выс = [...document.querySelectorAll('#wt-body-grid .wt-bcell')]
                          .map(к => Math.round(к.getBoundingClientRect().height));
            const сетка = document.getElementById('wt-body-grid');
            return {названий: сНазваниями, краёв: сКраями, оба, перелив, узкие,
                    кегль: минКегль === 99 ? null : минКегль,
                    колонок: getComputedStyle(сетка).gridTemplateColumns.split(' ').length,
                    карточка: Math.round(сетка.querySelector('.wt-bcell')
                                              .getBoundingClientRect().width),
                    высот: new Set(выс).size,
                    переливСетки: сетка.scrollWidth > сетка.clientWidth + 1};
          }""")
        отчёт.append(("колонок в сетке / ширина карточки",
                      f"{подписи['колонок']} / {подписи['карточка']}px"))
        отчёт.append(("карточек с названиями ВСЕХ зон", подписи["названий"]))
        отчёт.append(("карточек с подписями крайних зон", подписи["краёв"]))
        отчёт.append(("оба ряда подписей разом (обязан быть 0)", подписи["оба"]))
        отчёт.append(("подпись не влезла в свою зону", подписи["перелив"]))
        if подписи["узкие"]:
            отчёт.append(("   какие именно", "; ".join(подписи["узкие"])))
        отчёт.append(("минимальный кегль подписей зон, px", подписи["кегль"]))
        отчёт.append(("перелив сетки за свой контейнер", подписи["переливСетки"]))
        # Разных высот карточек. В несколько колонок обязана быть ОДНА:
        # ради этого в сетке стоит grid-auto-rows: 1fr. В одну колонку
        # выравнивать не с чем, и число здесь любое
        отчёт.append(("разных высот карточек", подписи["высот"]))
...
def главная():
    ...
    print("═" * 72)
    # 2560 ДОБАВЛЕНА 2026-08-19: на ней корневой кегль 17px, то есть
    # растут и слова, и порог показа подписей — а растут ли они в одной
    # пропорции, из кода не следует, это надо снять. Строка ДОБАВЛЕНА,
    # а не заменена: покрытие расширяется, сузить вопрос такая правка
    # не может по построению (CLAUDE.md §6.0.3)
    for ширина, высота, сенсор in ((1440, 900, False), (2560, 1440, False),
                                   (390, 844, True)):

...
ЧТО СНИМАЕТСЯ
  ...
  вкладка «Вес»      блок «Состав тела»: шкалы, числа границ, категории,
                     перелив карточки вправо И ВНИЗ (разные оси, вторую
                     `scrollWidth` не видит)
...
# снаружи не воспроизводятся вовсе (§8.0). Имена БЕРУТСЯ У НЕГО, а не
# переписываются: свой список из пяти строк лежал здесь до 2026-08-19
# и мог разойтись и с `make_local_user`, и с самим приложением молча —
# съёмка бы просто перестала снимать состояние, которого «нет».
# Сам `make_local_user` сверяет свои ключи с `main.СОСТОЯНИЯ_ВЕСОВ`.
import make_local_user as _сид   # noqa: E402

_сид.сверить_состояния()
СОСТОЯНИЯ = list(_сид.СОСТОЯНИЯ_ВЕСОВ)
    ...
    with sync_playwright() as p:
        ...
        отчёт.append(("карточка шире своего места",
                      стр.evaluate("""() => {
                        ...
                        return плохих; }""")))
        # ПЕРЕЛИВ ВНИЗ — отдельная ось, и до 2026-08-19 её тут не было вовсе.
        # `scrollWidth` над горизонталью её не видит: содержимое, вылезшее
        # за нижний край карточки, ширину не меняет. А случай не выдуманный:
        # `grid-auto-rows: 1fr` равняет строки по самой высокой, и карточка
        # с лишней строкой (у идеального веса это подпись «шкала по текущему
        # весу») — первый кандидат вылезти, если высота однажды перестанет
        # считаться по содержимому.
        #
        # Считается низ КАЖДОГО потомка против низа карточки, а не
        # scrollHeight: у элемента с `overflow: visible` scrollHeight
        # переполнение не показывает, то есть проверка молчала бы ровно
        # в том случае, ради которого написана. Печатается ХУДШЕЕ число
        # со знаком: отрицательное — запас до края, положительное — вылез.
        перелив = стр.evaluate("""() => {
            let худ = {вылез: -9999};
            for (const к of document.querySelectorAll('#wt-body-grid .wt-bcell')) {
              const r = к.getBoundingClientRect();
              for (const э of к.querySelectorAll('*')) {
                const rr = э.getBoundingClientRect();
                if (!rr.height) continue;
                const в = Math.round(rr.bottom - r.bottom);
                if (в > худ.вылез)
                  худ = {вылез: в, имя: к.querySelector('.wt-bname').textContent,
                         кто: э.className.slice(0, 26)};
              }
            }
            return худ; }""")
        отчёт.append(("худший вылез вниз, px (минус — запас)",
                      f"{перелив['вылез']:+} у «{перелив['имя']}» / {перелив['кто']}"))

...
ЗАМЕР = r"""() => {
  ...
  if (tabs) { const b = R(tabs); out.вкладки = [b.left, b.right];
    ...
    // и тянет за собой ВСЮ шапку вместе с рядом вкладок.
    const кон = h.parentElement;
    out.вкладки_шире = b.width - (кон ? кон.clientWidth : b.width); }
  // ГОРИЗОНТАЛЬНОЙ ПРОКРУТКИ У СТРАНИЦЫ НЕТ — вторая половина того же
  // вопроса: ряд может влезть в контейнер, а контейнер вылезти за окно.
  out.прокрутка_вбок = document.documentElement.scrollWidth
...
def _разборы(з):
    ...
    # ── 1.5 РЯД ВКЛАДОК НЕ ШИРЕ КОНТЕЙНЕРА (№352, «аптечка-2») ──────
    # Дефект `1fr` вместо `minmax(0, 1fr)` был СИСТЕМНЫМ: колонка кнопок
    # шапки росла содержимым, шапка вылезала за колонку, и на 390 ряд
    # вкладок уезжал за край вместе с ней. Чинился он письмом
    # «аптечка-1», а сторожа у него не было — оставался только замер
    # краёв, который ловит это лишь косвенно.
    if "вкладки_шире" in з:
        р.append(("вкладки_влезли", з["вкладки_шире"], з["вкладки_шире"] <= 1))
    if "прокрутка_вбок" in з:
        р.append(("страница_без_прокрутки_вбок", з["прокрутка_вбок"],
                  з["прокрутка_вбок"] <= 1))
    return р
...
def проверка(подлог=None, печать=True):
    ...
    for вид, строка in (("центр", "центр-кнопок-на-центре-h1"),
                        ...
                        ("вкладки_влезли", "вкладки-влезли-в-контейнер"),
                        ("страница_без_прокрутки_вбок",
                         "страницы-не-листает-вбок")):
...
ПОДЛОГИ = [
    ...
    # 1.5: РЯД ВКЛАДОК ШИРЕ КОНТЕЙНЕРА.
    #
    # ПОДЛОГОМ ЗДЕСЬ ДОЛЖЕН БЫЛ СТАТЬ ВОЗВРАТ `1fr` — ровно то, что
    # чинило письмо «аптечка-1». ЗАМЕР ЭТО ОТВЁРГ: с `1fr` ряд вкладок
    # на 390 по-прежнему уже контейнера (−32 px, то есть на паддинги)
    # у всех шести экранов с вкладками. Колонка кнопок сегодня
    # содержимым не переполняется — её ужали тем же письмом, — и старый
    # дефект этим способом больше не воспроизводится.
    #
    # Поэтому подлог ломает ЗВЕНО, которое шаг и сторожит: ряд вкладок
    # перестаёт ужиматься и растёт по содержимому.
    ("ряд вкладок шире контейнера", "вкладки-влезли-в-контейнер",
     _стиль(".v2-page-head .v2-tabs { min-width: max-content !important; }")),
]

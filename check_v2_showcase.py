...
КЛЮЧИ:
  ...
  --контроль   четыре подлога в СТРАНИЦУ (кода не трогают), у каждого своё
               доказательство:
...
# ПЕРВЫЙ ВИДИМЫЙ, А НЕ ПЕРВЫЙ В ДЕРЕВЕ («питание-5», блок 2.1). С письма 2
# в каркасе страницы живёт СКРЫТОЕ меню «Инструменты» с теми же
# `.v2-menu-item`, и оно стоит в дереве раньше витрины: `.first` брал его,
# и проба падала по таймауту прокрутки к невидимому пункту. Видимость —
# по вычисленному стилю всей цепочки предков и по размерам, а не по классу
ПЕРВЫЙ_ВИДИМЫЙ = """(сел) => { const все = [...document.querySelectorAll(сел)];
  const виден = e => { const r = e.getBoundingClientRect(); if (r.width <= 0 || r.height <= 0) return false;
    for (let x = e; x; x = x.parentElement) { const st = getComputedStyle(x);
      if (st.display === 'none' || st.visibility === 'hidden' || parseFloat(st.opacity) === 0) return false; }
    return true; };
  return все.findIndex(виден); }"""
...
def замер_фокуса(стр):
    ...
    for вид, сел in СЕЛЕКТОР.items():
        н = стр.evaluate(ПЕРВЫЙ_ВИДИМЫЙ, сел)
        if н < 0:
            continue
        эл = стр.locator(сел).nth(н)
...
# Пункты меню ВИТРИНЫ спрятаны: скрытое меню каркаса остаётся в дереве,
# и проба обязана сказать «не найден», а не взять его вместо витрины
ПОДЛОГ_МЕНЮ = "#v2s-menus ~ * .v2-menu-item { display: none !important; }"
...
ДОКАЗАТЕЛЬСТВА = {
    ...
    "меню": "видимого пункта меню витрины нет: ложь → истина",
}
...
def контроль():
    ...
    for имя, стиль in (("фокус", ПОДЛОГ_ФОКУС), ("градиент", ПОДЛОГ_ГРАДИЕНТ),
                       ("загрузка", ПОДЛОГ_ЗАГРУЗКА), ("меню", ПОДЛОГ_МЕНЮ)):
        находок = 0
        print(f"\n--- подлог «{имя}» ---")
        ...
        if имя == "фокус":
            ...
            до, после = "пункт меню" not in ф0, "пункт меню" not in ф
        elif имя == "градиент":
        ...
        if not (not до and после and найдено):
            итог = 1
    print("\nКОНТРОЛЬ:", "4 ПОДЛОГА ИЗ 4 НАЙДЕНЫ" if итог == 0 else "ПРОВАЛЕН")
    return итог

...
class ChatMessage(Base):
    ...
    tool = Column(String, nullable=False, default="nutrition")  # nutrition/workout/medkit — общая таблица на все чаты
    # ── СТРУКТУРНАЯ ЧАСТЬ РЕПЛИКИ (BACKLOG №178, блок C) ─────────────
    #
    # JSON ответа сервера целиком, у аптечки это выдача запроса: что
    # нашлось, что просрочено, что рецептурно и предложение занести
    # ненайденное в покупки.
    #
    # ЗАЧЕМ ХРАНИТЬ, ЕСЛИ ЕСТЬ `content`. Восстановленная из одного
    # текста реплика ВЫГЛЯДИТ ТАК ЖЕ, а кнопок под ней нет — и человек
    # не может понять, почему вчерашний ответ отвечал, а сегодняшний
    # только рассказывает. Немой отказ в чистом виде.
    #
    # С payload восстановление зовёт ТОТ ЖЕ построитель (`аптАИОтвет`),
    # что рисовал ответ живьём, — второго построителя карточек ответа
    # не появляется (§6.0.15).
    #
    # ПУСТО У ВСЕХ ПРЕЖНИХ РЕПЛИК И У ОБЫЧНОГО ТЕКСТА, и это законно:
    # структурной части у них нет, восстанавливать нечего.
...
class MedkitItem(Base):
    ...
    # ОТКУДА ВЗЯЛОСЬ ВЕЩЕСТВО — код из `medkit_defs.ИСТОЧНИК_ВЕЩЕСТВА`
    # (BACKLOG №234, A.3). NULL — «неизвестно», и это ПЯТОЕ состояние,
    # а не прочерк: все позиции, заведённые до правки, лежат именно так,
    # и выдать их за прочитанные с упаковки значило бы соврать ровно
    # в том поле, ради честности которого колонка и заводится.
    #
    # ПОЧЕМУ КОЛОНКА, А НЕ ВЫЧИСЛЕНИЕ ПРИ ЧТЕНИИ. Происхождение — факт
    # МОМЕНТА ЗАВЕДЕНИЯ, и восстановить его потом нечем: справочник
    # меняется, снимок можно удалить, а модель на тот же кадр отвечает
    # каждый раз чуть иначе. Ровно поэтому же срок после вскрытия лежит
    ...
    # ── КОЛОНОК `volume` / `volume_unit` В МОДЕЛИ БОЛЬШЕ НЕТ ─────────
    # Задача 253, решение владельца. Поле «Во флаконе» показывалось
    # 14 позициям из 77 и было заполнено у НУЛЯ — то есть отвечало
    # на вопрос, которого никто не задавал, а читалось в сорока пяти
    # местах кода, включая карточку, сводку группы и промпт модели.
    #
    # `DROP COLUMN` НЕ ДЕЛАЕТСЯ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ. Замер «0 из 77»
    # снят ПРОШЛЫМ заходом, а поле с тех пор оставалось на экране
    # у четырнадцати позиций; копии боевой базы, чтобы перепроверить
    # это сегодня, у захода нет, а обходить прод запрещено (§5.8).
    # Удаление колонки необратимо, и цена ошибки здесь несимметрична:
    # оставленная пустая колонка не стоит ничего, стёртое значение
    # владельца не вернуть ничем. Тот же довод, по которому осталась
    # `cover_letters.custom_context` выше в этом файле.
    #
    # Колонка при этом МЕРТВА: её не читает и не пишет ни одно место
    # (`py check_volume_refs.py` печатает 0). На новых базах она
    # не заводится вовсе — строки `ADD COLUMN` убраны из миграций.
    ...
    # позиции: «Вещество В густой экстракт» против «вещество В»,
...
class MedkitBuyItem(Base):
    """СТРОКА СПИСКА ПОКУПОК.

    BACKLOG №178, блок B. Ценность не в самом списке, а в том, что
    позиции попадают в него САМИ — в момент, когда стало ясно, что
    чего-то нет. Источников четыре, и они названы полем `source`:

        `ai`       ассистент не нашёл — кнопка прямо в его ответе
        `expired`  выбросили просроченное
        `low`      остаток ушёл в красную зону шкалы либо в ноль
        `hand`     вписано руками

    ═══════════════════════════════════════════════════════════════════
    ПОЛЯ ПРЕПАРАТА — КОПИЯ, А НЕ ССЫЛКА НА КАРТОЧКУ
    ═══════════════════════════════════════════════════════════════════

    Соблазн держать `item_id` велик: позиция пришла из существующей
    карточки, и всё про неё там уже написано. Но карточку УДАЛЯЮТ —
    у источника `expired` это буквально следующее действие человека, —
    а строка покупок обязана его пережить (постановка B.6). Ссылка
    осиротела бы молча: строка осталась бы в списке, а «Повторить
    пачку» перестала бы заполнять что-либо, и признака этого не было бы
    никакого.

    Поэтому копируются ровно те поля, которых хватает обратному ходу
    (B.3): название, вещество, форма, единица, разовая доза,
    рецептурность и место хранения. НЕ копируются срок годности
    и количество — именно их человек и вписывает у новой пачки.

    Копия — не дубль в смысле §6.0.7: это СНИМОК на момент занесения,
    и расходиться ему с карточкой не только можно, но и нужно —
    карточки к тому времени может не быть вовсе.

    ═══════════════════════════════════════════════════════════════════
    КУПЛЕННОЕ НЕ ИСЧЕЗАЕТ СРАЗУ
    ═══════════════════════════════════════════════════════════════════

    `bought_on` — день отметки В ПОЯСЕ ПОЛЬЗОВАТЕЛЯ (§5.0.6, источник
    один — `_сегодня(user)`). Строка остаётся зачёркнутой до конца дня,
    чтобы галочку можно было снять, если передумали; назавтра уборка
    её уносит. Уборка ЛЕНИВАЯ, при чтении списка, — задача по
    расписанию была бы ещё одной внешней штукой, которую можно не
    завести и не заметить (тот же довод, что у журнала попыток входа).
    """
    __tablename__ = "medkit_buy_items"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=False, index=True)
    name = Column(String, nullable=False)
    # ПОЧЕМУ ЭТА СТРОКА ЗДЕСЬ — готовой фразой, а не выводится при показе.
    # «Спрашивали 25.08 — в аптечке этой группы нет» ссылается на день
    # и на состояние аптечки НА ТОТ МОМЕНТ; посчитай мы её при чтении,
    # завтра она говорила бы про сегодня и была бы неправдой
    why = Column(String, nullable=True)
    source = Column(String, nullable=False, default="hand")
    # ── СНИМОК ПОЛЕЙ ДЛЯ ОБРАТНОГО ХОДА ──────────────────────────────
    substance = Column(String, nullable=True)
    ...
    bought_on = Column(String, nullable=True)      # 'ГГГГ-ММ-ДД'
    created_at = Column(DateTime, default=datetime.utcnow)



class MedkitArchive(Base):
    """АРХИВ УДАЛЁННЫХ УПАКОВОК — чтобы не заполнять карточку заново.

    ═══════════════════════════════════════════════════════════════════
    ЗАЧЕМ, ПРЯМАЯ ЗАДАЧА ВЛАДЕЛЬЦА
    ═══════════════════════════════════════════════════════════════════

    Лекарство кончилось или просрочилось — карточку удаляют. Купив
    то же самое, человек заполняет её С НУЛЯ: название, форму,
    дозировку, вещество, категории, схему приёма с упаковки,
    показания с вкладыша.

    ЗАМЕР ДО ЗАВЕДЕНИЯ АРХИВА (заход 249, G0), удаление живой позиции
    стенда через боевой эндпоинт: строк позиции 1 → 0, строк
    категорий 1 → 0. Своя схема (37 знаков), своё «от чего»
    (143 знака), вещество, форма и дозировка стёрты ФИЗИЧЕСКИ
    вместе со строкой; таблицы архива в схеме не было вовсе.

    ═══════════════════════════════════════════════════════════════════
    СНИМОК, А НЕ ССЫЛКА, И НЕ ПОМЕТКА НА ПОЗИЦИИ
    ═══════════════════════════════════════════════════════════════════

    Устроен как `MedkitBuyItem` и по той же причине: строка обязана
    пережить удаление карточки, а ссылка на удалённую строку осиротела
    бы молча.

    ПОМЕТКОЙ `deleted_at` НА САМОЙ ПОЗИЦИИ — НЕ ВЫШЛО БЫ. Удаление
    в общей аптечке делает любой участник, позиция при этом уходит
    из списка ЦЕЛИКОМ, а `_апт_круг` и все двенадцать выборок экрана
    ходят по `medkit_items` — каждой пришлось бы дописать «и не
    удалённая», и место, где забыли, показало бы человеку выброшенную
    пачку среди живых. Тот же довод, по которому гейт подтверждённой
    почты стоит ОДНИМ middleware, а не проверкой в каждом обработчике
    (§5.3).
    ...
    КОПИРУЕТСЯ то, что человек вносил РУКАМИ и что не зависит от
    конкретной пачки: название, форма, дозировка, единица, вещество,
    рецептурность, GTIN, свои записи схемы и показаний, категории
    (именами, а не номерами — категорию могли удалить).

    НЕ КОПИРУЕТСЯ то, чем пачки и отличаются: срок годности,
    количество, дата вскрытия, фото, место хранения. Ровно этот
    список не переносит и действие «Ещё упаковка» (§5.8, задача 175),
    и разойтись им нельзя — иначе «повторная покупка» подставила бы
    срок годности ВЫБРОШЕННОЙ пачки, а это единственное поле, где
    выдуманное значение опасно и при этом незаметно.

    НЕ КОПИРУЮТСЯ И ВЫДЕРЖКИ СПРАВОЧНИКА (`dosage_text`,
    `indications_text`): они не запись человека, а снимок чужой
    страницы на дату, и новая карточка возьмёт их сама — фоновым
    поиском при заведении (§5.8, задача 175, блок B). Класть их
    в архив значило бы показать человеку выдержку годичной давности
    как свежую.

    ═══════════════════════════════════════════════════════════════════
    ВЛАДЕЛЕЦ — ТОТ, ЧЬЯ БЫЛА ПОЗИЦИЯ, А НЕ ТОТ, КТО УДАЛИЛ
    ═══════════════════════════════════════════════════════════════════

    В общей аптечке чужую пачку удаляет любой участник (§5.8, задача
    182), и записать архив на удалившего значило бы отдать ему чужую
    работу: при выходе из круга он унёс бы её с собой. Владелец
    берётся у САМОЙ позиции — тем же полем, каким считается, чьё фото
    убирать с тома.

    Видимость — в пределах круга, как у активных позиций: тот же
    `_апт_круг`, второго правила видимости не заводится (§6.0.7).
    """
    __tablename__ = "medkit_archive"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=False, index=True)
    # ── ЧЕМ ОПОЗНАЁМ ПРИ ПОВТОРНОЙ ПОКУПКЕ ───────────────────────
    name = Column(String, nullable=False)
    # GTIN — ОСНОВНОЙ СЦЕНАРИЙ, ради которого коды и складывались:
    # 14 цифр опознают торговую единицу целиком (§5.8, сканер)
    code_gtin = Column(String, index=True)
    form = Column(String)
    dose = Column(Float)
    unit = Column(String)
    # ── ЧТО ПОДСТАВЛЯЕМ ──────────────────────────────────────────
    substance = Column(String)
    substance_src = Column(String)
    is_rx = Column(Boolean, default=False)
    own_dosage_text = Column(Text)
    own_indications_text = Column(Text)
    # КАТЕГОРИИ ИМЕНАМИ, А НЕ НОМЕРАМИ: категорию могли удалить,
    # и номер осиротел бы молча — подстановка вписала бы пустоту
    categories = Column(Text)
    # Объём упаковки убран вместе с полем формы — разбор у MedkitItem
    leaflet_url = Column(String)
...
# ══ ОБЩАЯ АПТЕЧКА (BACKLOG №182) ════════════════════════════════════
#
# ОДНА ОБЩАЯ АПТЕЧКА, А НЕ ГРАФ ДРУЗЕЙ (постановка A.1). Все участники
# видят ОДИН И ТОТ ЖЕ список. Граф — «я делюсь с А и с Б, а они друг
# друга не видят» — для одной полки в ванной не значит ничего и стоит
# дорого: у каждой позиции пришлось бы держать своё множество зрителей,
# и вопрос «кто это видит» перестал бы иметь один ответ.
#
# ОТСЮДА ИНВАРИАНТ: пользователь состоит НЕ БОЛЬШЕ ЧЕМ В ОДНОМ круге.
# Он держится проверкой на входе (нельзя принять приглашение, состоя
# в другом круге), а не надеждой: два круга у одного человека сделали бы
# из общей аптечки тот самый граф через заднюю дверь.
#
# ПРАВ У КРУГА ВСЕГО ДВОЕ И ОБА ПРО УЧАСТНИКОВ, А НЕ ПРО ПОЗИЦИИ:
# владелец круга может убрать участника, участник может уйти сам.
# С ЧУЖОЙ ПОЗИЦИЕЙ ЛЮБОЙ УЧАСТНИК ДЕЛАЕТ ВСЁ ТО ЖЕ, ЧТО СО СВОЕЙ —
# списывает, правит, удаляет (A.3). Владелец описал сценарий прямо:
# «она выпила последние таблетки, удалила — у меня тоже пропало».
# Плата за это — ЛЕНТА ИЗМЕНЕНИЙ (`MedkitEvent`): без неё общий список
# менялся бы молча, и «потерялось» было бы неотличимо от «выбросили».


class MedkitCircle(Base):
    """КРУГ — общая аптечка нескольких человек.

    `user_id` — ВЛАДЕЛЕЦ круга, тот, кто позвал первым (A.5). Колонка
    названа `user_id`, а не `owner_id`, не для красоты: каскад удаления
    аккаунта ходит по `USER_TABLES` строго по этому имени, и своё имя
    означало бы, что круг переживёт удаление своего владельца молча
    (`check_user_tables_complete` сверяет список со схемой ПО ЭТОЙ
    колонке — §6.1).

    КРУГ ЗАВОДИТСЯ НЕ ПРИГЛАШЕНИЕМ, А ПРИНЯТИЕМ. Приглашение может
    висеть неделю и быть отклонённым; круг из одного человека — это
    личная аптечка, у которой почему-то есть лента и панель участников.
    Ровно поэтому при уходе предпоследнего участника круг РАСПУСКАЕТСЯ
    (`_апт_круг_прибрать` в main.py), а не остаётся с одним членом:
    иначе личная аптечка навсегда осталась бы с чужими элементами
    на экране (F.4).
    """
    __tablename__ = "medkit_circles"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=False, index=True)
    created_at = Column(DateTime, default=datetime.utcnow)


class MedkitMember(Base):
    """УЧАСТНИК КРУГА. Владелец круга — тоже строка здесь.

    Владелец лежит участником НАРАВНЕ с остальными, а не выводится
    из `MedkitCircle.user_id`: иначе множество «кто видит этот список»
    считалось бы в двух местах — строки таблицы плюс отдельно владелец, —
    и забыть второе слагаемое можно было бы ровно один раз, после чего
    владелец перестал бы видеть собственную аптечку (§6.0.7).
    """
    __tablename__ = "medkit_members"
    id = Column(Integer, primary_key=True)
...
class MedkitInvite(Base):
    """ПРИГЛАШЕНИЕ. Только тому, кто УЖЕ зарегистрирован (B.1).

    Приглашать по чужому адресу «в никуда» нельзя, и это не удобство
    реализации: письмо незнакомому человеку по адресу, введённому
    третьим лицом, — это рассылка, а не приглашение. Несуществующий
    адрес получает ВНЯТНЫЙ ОТКАЗ, а не молчание: «принято» про адрес,
    которого нет, — немой отказ в чистом виде (§6.0).

    `user_id` — КТО ЗОВЁТ (и по нему же работает каскад удаления),
    `to_user_id` — кого. Приглашение, присланное удалённому аккаунту,
    убирается встречной ссылкой (`ВСТРЕЧНЫЕ_ССЫЛКИ` ниже): без неё
    строка осталась бы сиротой, которую не покажет ни один экран
    и не уберёт ни одна уборка — ровно тот вид, что ловит проверка 24
    у сетов Enshrouded.

    СОСТОЯНИЕ НЕ ХРАНИТСЯ ПОЛЕМ `status`, И ЭТОГО ПОЛЯ ЗДЕСЬ НЕТ.
    Живое приглашение — это существующая строка; принятое превращается
    в строку `MedkitMember`, отклонённое и отозванное удаляется.
    Хранить рядом «принято/отклонено» значило бы завести второе место,
    где написано, состоит ли человек в круге, — и разойтись оно могло
    бы молча.
    """
    __tablename__ = "medkit_invites"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=False, index=True)
...
class MedkitBlock(Base):
    """БЛОК. `user_id` заблокировал `blocked_user_id`.

    БЛОКИРОВКА ДЕЙСТВУЕТ ДО ПРИГЛАШЕНИЯ (B.3): заблокированный не может
    его прислать, а уже висящее снимается в тот же миг. Иначе смысла
    в ней нет — спам уже пришёл, и «заблокировать» означало бы «убрать
    с глаз то, что уже показали».
    """
    __tablename__ = "medkit_blocks"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=False, index=True)
    blocked_user_id = Column(Integer, nullable=False, index=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class LandingMedia(Base):
    ...
    Места не заводятся строками: их двадцать восемь и они описаны кодом.
...
class RefRequestDay(Base):
    ...
    BACKLOG №201, блок E. Опасность не в цене, а в цикле, ушедшем
    в петлю: перепроверка всей аптечки — это сотни обращений
    к чужому сайту, и обход, зациклившийся на ошибке, отличим
    от исправного только счётчиком.

    ПОЧЕМУ В БАЗЕ, А НЕ В ПАМЯТИ ПРОЦЕССА. Ровно тот урок, который
    §8.1 уже оплатил защитой входа: словарь в памяти гибнет при каждом
    деплое (замер: 106 прогонов за 30 дней, три-четыре в сутки),
    и потолок, живущий в памяти, не ограничивает НИЧЕГО. Отказ при
    этом немой — счётчик просто обнуляется, и в журнале ни строки.

    ПЕРСОНАЛЬНЫМИ ДАННЫМИ НЕ ЯВЛЯЕТСЯ И НЕ ДОЛЖНА СТАТЬ. Ни `user_id`,
    ни названия препарата, ни адреса страницы: только сутки, хост
    и число. ВНИМАНИЕ на будущее — появится здесь колонка «кто
    спросил» или «что искали», и таблица немедленно превратится
    в историю обращений конкретного человека к справочнику лекарств,
    то есть в сведения о здоровье (§5.1, §6.1). Тот же предупреждающий
    абзац стоит у `food_translations`, и по той же причине.

    СУТКИ СТРОКОЙ `ГГГГ-ММ-ДД`, а не датой: месяц из неё берётся
    срезом первых семи знаков, и второго мнения о границе месяца
    не заводится.
    """

    __tablename__ = "ref_requests"
    id = Column(Integer, primary_key=True, index=True)
...
class MedkitEvent(Base):
    ...
    ПОЧЕМУ ОНА ОБЯЗАТЕЛЬНА, А НЕ ПРИЯТНА. Правило A.3 разрешает любому
    участнику удалить чужую пачку. Без ленты список меняется МОЛЧА:
    карточка исчезла — и «её выбросили» неотличимо от «я сам её удалил
    и забыл» и от «сломалось». Лента — это плата за A.3, названная
    в самой постановке (A.4).

    ЧТО ПОПАДАЕТ (C.1): заведение, удаление, списание, правка
    количества или срока, занесение в покупки и отметка о покупке,
    приход и уход участников.

    ЧТО НЕ ПОПАДАЕТ (C.2): открытие схемы приёма, поиск, просмотр
    карточки. Видеть ИЗМЕНЕНИЯ СПИСКА — не то же самое, что читать
    чужие действия; лента наблюдения за человеком тут не заводится.

    ЛЕНТА ЕСТЬ ТОЛЬКО У КРУГА. В личной аптечке писать её некому
    и некому читать: все действия там свои (F.3, F.4). Поэтому
    `circle_id` обязателен, а не «NULL — значит личная».

    ТЕКСТ СОБИРАЕТСЯ ПРИ ПОКАЗЕ, А НЕ ХРАНИТСЯ ГОТОВОЙ ФРАЗОЙ, и это
    отличие от `MedkitBuyItem.why` не случайно: та строка ссылается
    на СОСТОЯНИЕ АПТЕЧКИ на момент занесения («в аптечке этой группы
    нет»), а событие ленты описывает СВОЁ СОБСТВЕННОЕ действие —
    название пачки и число, — и оба лежат прямо здесь.
    """
    __tablename__ = "medkit_events"
    id = Column(Integer, primary_key=True)
    circle_id = Column(Integer, nullable=False, index=True)
    user_id = Column(Integer, nullable=False, index=True)   # кто НАЖАЛ
    # ЗА КОГО ЗАПИСАНО — ВТОРОЕ ЛИЦО СОБЫТИЯ (BACKLOG №260, блок B).
    #
    # ДВА РАЗНЫХ ЧЕЛОВЕКА, И ПОДМЕНЯТЬ ОДНО ДРУГИМ НЕЛЬЗЯ. Таблетку
    # даёт тот, кто рядом; принимает её тот, кому она нужна. До этой
    # колонки лента знала только первого, и «Зина — приём: Препарат А»
    # означало и «Зина выпила», и «Зина дала выпить матери» — два
    # разных факта одной строкой, различить их было нечем.
    #
    # `user_id` НЕ ПОДМЕНЯЕТСЯ НИКОГДА (B.7): правка автора меняет
    # только это поле. Иначе исправление ошибки стирало бы след того,
    # кто её сделал, — то есть лента переставала бы отвечать на вопрос,
    # ради которого заведена (кто трогал общий список).
    #
    # NULL У СТАРЫХ СТРОК ЧИТАЕТСЯ КАК «ЗА СЕБЯ», и бэкфилла тут нет
    # намеренно: до этой колонки другого случая не существовало,
    # проставлять нечего. Чтение идёт через `_апт_за_кого` — одно
    # место, а не `or` на каждом из мест показа (§6.0.7).
    for_user_id = Column(Integer, nullable=True, index=True)
    kind = Column(String(16), nullable=False)
    # Название пачки или строки покупок. КОПИЯ, а не ссылка на позицию,
    # по той же причине, что у списка покупок: карточку удаляют, и
    # событие «удалила Смекту» обязано её пережить. Ссылка осиротела бы
    # молча — событие осталось бы, а название исчезло
    name = Column(String, nullable=True)
    detail = Column(String, nullable=True)                  # «осталось 13»
    created_at = Column(DateTime, default=datetime.utcnow, index=True)
...
def migrate_db():
    ...
    for col in [
        ...
        # BACKLOG №234, A.3: откуда взялось действующее вещество.
        # Существующие строки остаются NULL — «неизвестно», и это
        ...
        # ЗДЕСЬ СТОЯЛИ `ADD COLUMN volume` и `volume_unit` (BACKLOG
        # №238, A.1). Убраны задачей 253 вместе с самим полем: новая
        # база их не получает. На старых колонки остаются пустыми
        # и никем не читаются — разбор у модели `MedkitItem`.
        ...
        # Переписка аптечки (BACKLOG №178, блок C): структурная часть
        # реплики, чтобы восстановленный ответ рисовался ТЕМ ЖЕ
        # построителем, что живой
        "ALTER TABLE chat_messages ADD COLUMN payload TEXT",
        # BACKLOG №260, блок B: за кого записан приём. Существующие
...
USER_TABLES = [
    ...
    "medkit_buy_items",
    "medkit_archive",
    # Общая аптечка. У всех пяти колонка называется `user_id` именно
    # затем, чтобы попасть сюда механически: `check_user_tables_complete`
    # сверяет этот список со схемой по имени колонки, и таблица со своим
    # `owner_id` выпала бы из-под каскада молча (§6.1)
    "medkit_circles", "medkit_members", "medkit_invites",
    "medkit_blocks", "medkit_events",
]

# ВСТРЕЧНЫЕ ССЫЛКИ: колонка указывает на пользователя, но НЕ называется
# `user_id`, потому что «свой» у строки другой.
#
# Приглашение принадлежит тому, КТО ЗОВЁТ, блок — тому, КТО
# ЗАБЛОКИРОВАЛ. Удали мы только их строки — у второй стороны остались бы
# записи, указывающие на несуществующий аккаунт: приглашение от никого
# и блок на никого. Показать их некому и убрать нечем — ровно тот вид
# сироты, что ловит проверка 24 у сетов Enshrouded, только в другой
# таблице.
#
# Перечнем, а не признаком, и это законно (§6.0.7): множество конечно
# и объявляется решением о схеме. Сторожит его тест
# `test_встречные_ссылки_описывают_все_колонки_на_users`.
ВСТРЕЧНЫЕ_ССЫЛКИ = [
    ("medkit_invites", "to_user_id"),
    ("medkit_blocks", "blocked_user_id"),
]

# ССЫЛКИ, КОТОРЫЕ ПЕРЕЖИВАЮТ УХОД ЧЕЛОВЕКА — И ЭТО РЕШЕНИЕ, А НЕ ПРОБЕЛ
#
# BACKLOG №260, блок B. `medkit_events.for_user_id` — «за кого записан
# приём», и строка эта принадлежит ТОМУ, КТО НАЖАЛ: он уносит её своим
# каскадом. Унести её ВТОРЫМ каскадом — за ушедшим «за кого» — значило
# бы стереть у оставшегося запись о его собственном действии.
#
# СИРОТА ЗДЕСЬ НАЗВАННАЯ, А НЕ МОЛЧАЛИВАЯ: лента показывает «участник
# ушёл» ровно так же, как делает это для автора (`_апт_лента`), то есть
# строка остаётся правдивой — «приём был, за кого именно, сказать уже
# некому». Обнулять поле в NULL нельзя: NULL означает «за себя»,
# и событие начало бы утверждать, что нажавший принял лекарство сам.
#
# Перечнем и с причиной у строки — тот же порядок, что у соседа выше.
ССЫЛКИ_ПЕРЕЖИВАЮЩИЕ_УДАЛЕНИЕ = [
    ("medkit_events", "for_user_id"),
]
...
PRIVACY_MENTIONS = {
    ...
    "medkit_buy_items":          ["список покупок"],
    "medkit_archive":            ["аптечк"],
    # Общая аптечка: с кем список разделён и что в нём меняли. Это
    # сведения о ДРУГОМ человеке в вашем аккаунте (его адрес и его
    # действия), то есть категория, которой в политике раньше не было
    # вовсе
    "medkit_circles":            ["общая аптечка"],
    "medkit_members":            ["общая аптечка"],
    "medkit_invites":            ["приглашени"],
...
PRIVACY_NOT_PERSONAL = {
    ...
    # Счётчик обращений к чужому справочнику (BACKLOG №201, E).
    # Ни user_id, ни названия препарата, ни адреса — только сутки,
...
def delete_user_cascade(user_id: int, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        # 2б. Встречные ссылки: строки, где удаляемый — ВТОРАЯ сторона
        for таблица, поле in ВСТРЕЧНЫЕ_ССЫЛКИ:
            n = conn.execute(f"SELECT COUNT(*) FROM {таблица} WHERE {поле} = ?",
                             (user_id,)).fetchone()[0]
            if n and not dry_run:
                conn.execute(f"DELETE FROM {таблица} WHERE {поле} = ?", (user_id,))
            отчёт[f"{таблица} ({поле})"] = n

        # 2в. КРУГ, ОСТАВШИЙСЯ БЕЗ ВЛАДЕЛЬЦА ИЛИ БЕЗ ВТОРОГО УЧАСТНИКА.
        #
        # Шаг 2 удалил круги, которыми удаляемый ВЛАДЕЛ, и его собственные
        # строки участия. У обеих операций остаётся хвост, и обе дыры
        # немые:
        #
        #   · круг владельца удалён — строки участия ОСТАЛЬНЫХ висят
        #     на несуществующем `circle_id`. Такой участник видел бы
        #     чужие позиции по кругу, которого нет;
        #   · круг пережил уход, но в нём остался ОДИН человек. Это
        #     личная аптечка, у которой почему-то есть панель участников
        #     и лента (F.4).
        #
        # Оба чинятся здесь, а не «когда-нибудь при чтении»: уборка при
        # удалении не может отстать от удаления по построению.
        if not dry_run:
            conn.execute("DELETE FROM medkit_members WHERE circle_id NOT IN "
                         "(SELECT id FROM medkit_circles)")
            conn.execute("DELETE FROM medkit_events WHERE circle_id NOT IN "
                         "(SELECT id FROM medkit_circles)")
            одиночки = [r[0] for r in conn.execute(
                "SELECT c.id FROM medkit_circles c WHERE ("
                "SELECT COUNT(*) FROM medkit_members m WHERE m.circle_id = c.id"
                ") < 2")]
            for cid in одиночки:
                conn.execute("DELETE FROM medkit_members WHERE circle_id = ?", (cid,))
                conn.execute("DELETE FROM medkit_events WHERE circle_id = ?", (cid,))
                conn.execute("DELETE FROM medkit_circles WHERE id = ?", (cid,))
            отчёт["круги распущены"] = len(одиночки)

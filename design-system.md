...
### 6.1. Тонкий вариант — `footer_thin`

Копирайт и три правовые ссылки, без колонок:
...
Подключается флагом перед include, разметка — там же, в `_footer.html`:

```jinja
{% set footer_thin = true %}
{% include '_footer.html' %}
```

Где применяется: страницы входа — `/login`, `/register`, `/forgot-password`,
`/reset-password`. Полный футер там вреден: колонка «Инструменты» ведёт на
то, что за авторизацией и человеку сейчас недоступно, а «Аккаунт» — на
профиль, которого у него ещё нет. Правовые ссылки, наоборот, обязаны быть
именно здесь: условия и политику принимают на этом шаге, и прочитать их
надо ДО отправки формы.

Вариант живёт в системе, а не вёрсткой на странице (`CLAUDE.md`, §6.0.2):
`.site-footer-thin` в `style.css`, ветка в `_footer.html`. Страница не
собирает свой футер из трёх `<a>`.
...
- [ ] Есть футер — полный или тонкий (`{% set footer_thin = true %}`). Отсутствие футера — осознанное исключение, названное в §6.0.2 `CLAUDE.md`.

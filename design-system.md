...
Открыть и закрыть — `openModal('modal-water')` / `closeModal('modal-water')`.
Своя уборка при закрытии (остановить камеру, сбросить флаг) вешается
на событие `modal:close`, а НЕ на отдельную кнопку: событие приходит от любого
из трёх способов закрытия, и забыть один из них становится нельзя.

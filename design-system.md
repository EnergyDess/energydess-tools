...
**Три состояния, и все три выводятся из данных.** Страница не задаёт состояние
вручную — она просто подключает `_header.html`:

| Состояние | Условие | Что в шапке | Где встречается |
|---|---|---|---|
| гость | `user` нет | логотип + Войти / Регистрация | `/login`, `/register`, `/forgot-password`, `/reset-password`, `/verify-pending`, лендинг, витрина инструмента |
...
Поиск рисуется по третьему состоянию, а не по факту авторизации: пока почта
...
Класс `.site-header-nosearch` вешается по признаку «поиска нет» (состояния
1 и 2), а не «гость»: он снимает растяжку крыльев, без которой логотип
уезжал бы к центру пустой шапки.

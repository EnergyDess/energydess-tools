...
def _сжатие(место):
    """Параметры сжатия ролика для секции места (`landing_defs.ВИДЕО_СЖАТИЕ`).
    Место не передано либо секция без своих параметров — путь ленты."""
    секция = (место or {}).get("section")
    return ld.ВИДЕО_СЖАТИЕ.get(секция, ld.ВИДЕО_СЖАТИЕ["feed"])


def сжать_ролик(вход, выход, разбор=None, место=None):
    ...
    Сторона, crf и режим кодера — от секции места (заход 342, A2)."""
    п_сж = _сжатие(место)
    д = п_сж["сторона"]
    ...
    команда = [ffmpeg(), "-hide_banner", "-nostdin", "-y", "-threads", "1", "-i", вход,
               ...
               "-c:v", "libx264", "-preset", "veryfast", "-crf", str(п_сж["crf"]),
               # ЛЁГКИЙ РЕЖИМ фона: без опережающего просмотра и B-кадров
               # пик памяти на 1920 — 136 МБ против 239 (замер в landing_defs)
...
def исходник_ролика(разбор, место=None):
    ...
    сторона = _сжатие(место)["сторона"]
    if max(разбор["width"], разбор["height"]) > сторона:
        return "сторона больше %d" % сторона
...
# ПЕРВЫЙ КАДР РОЛИКА — КАРТИНКОЙ (заход 342, блок B6). Пока ролик не приехал,
# страница показывает его первый кадр: чёрного фона не должно быть ни секунды.
# Снимается ИЗ ГОТОВОГО файла, а не из исходника: кадр обязан совпасть
# с тем, с чего начнёт играть страница, иначе при старте был бы скачок.
# Отдельного места под кадр нет — он принадлежит ролику и лежит рядом
# (`landing_store.имя_постера`).
ПОСТЕР_КАЧЕСТВО = 82


def снять_постер(ролик, выход, ширина=None):
    """webp первого кадра. None — не вышло (ролик без кадров): постер
    необязателен, страница тогда просто ждёт ролик, но это ГОВОРИТСЯ
    предупреждением, а не молчит. `ширина` — потолок ширины кадра
    с сохранением пропорции (`ВИДЕО_СЖАТИЕ[…]["постер_ширина"]`)."""
    кадр = выход + ".png"
    п = subprocess.run([ffmpeg(), "-hide_banner", "-nostdin", "-y", "-i", ролик,
                        "-frames:v", "1", кадр], capture_output=True, timeout=60)
    if п.returncode != 0 or not os.path.exists(кадр):
        return None
    try:
        with Image.open(кадр) as im:
            rgb = im.convert("RGB")
            if ширина and rgb.width > ширина:
                rgb = rgb.resize((ширина, round(rgb.height * ширина / rgb.width)),
                                 Image.LANCZOS)
            rgb.save(выход, "WEBP", quality=ПОСТЕР_КАЧЕСТВО, method=6)
    finally:
        os.remove(кадр)
    return выход
...
def обработать(место_id, временный, исходное_имя):
    ...
    if место["kind"] == ld.ВИДЕО:
        ...
        пик = сжать_ролик(временный, готовый, разбор, место)
        ...
        причина = исходник_ролика(разбор, место)
        ...
        итог_мб = _сжатие(место)["итог_мб"]
        if вес > итог_мб * 1024 * 1024:
            предупреждения.append("После сжатия %.1f МБ — для %s разумно до %d МБ"
                                  % (_мб(вес), "фона" if место["section"] == "bg" else "ленты",
                                     итог_мб))
        постер = снять_постер(готовый, os.path.join(каталог, "poster.webp"),
                              _сжатие(место).get("постер_ширина"))
        сведения["poster"] = постер
        if постер is None:
            предупреждения.append("Первый кадр снять не удалось — пока ролик грузится, "
                                  "на его месте будет пусто")
    else:

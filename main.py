...
def _апт_годен_до(поз):
    ...
    # ФОРМА, КОТОРАЯ ОТ ВСКРЫТИЯ НЕ ПОРТИТСЯ, СРОКА НЕ УКОРАЧИВАЕТ,
    # какая бы дата ни лежала в базе. Таблетки в блистере годны до срока
    # на пачке; поля вскрытия у них теперь и не показываются, но данные
    # у тех, кто успел их проставить, ОСТАЮТСЯ — и молча портить ответ
    # на главный вопрос инструмента они не должны (§5.8)
    if not _апт_опр.портится_после_вскрытия(поз.form, поз.unit):
        return упаковочный
...
def _апт_состояние(поз, сегодня):
    ...
    # ВСКРЫТИЕ ЗНАЧИТ ЧТО-ТО ТОЛЬКО ТАМ, ГДЕ ОНО МЕНЯЕТ СРОК. У таблеток
    # в блистере отметка «вскрыт» не сообщает ни о чём: пачка открыта,
    # таблетки годны до того же дня. Показывать по ней янтарный тон —
    # ложная тревога, порождённая полем, которого там быть не должно
    вскрыт = bool(поз.opened_on) and _апт_опр.портится_после_вскрытия(
        поз.form, поз.unit)
    if осталось < 0:
        return {"id": "expired",
...
def _апт_проверить(данные, db, user):
    ...
    if ош:
        return None, ош
    # Умолчание формы подставляется ТОЛЬКО когда поля не прислали вовсе.
    # Пустая строка означает «человек стёр число намеренно», и подставить
    # туда своё значило бы вернуть то, что он убрал
    if дней is None and "opened_days" not in данные:
        дней = форма["вскрытие_дней"]
...
async def _апт_найти_дозы(поз):
    ...
    if not _апт_дозы.числа(искомое["доза"]):
        return None, ("в карточке не указана дозировка — без неё нельзя "
                      "отличить таблетки 10 мг от капель 10 мг/мл")
    async with httpx.AsyncClient(timeout=ДОЗЫ_ТАЙМАУТ,
        ...
        if not адреса:
            return None, "в справочнике нет страницы с таким названием"
        подошли = []
        for u in адреса:
            try:
        ...
        if not подошли:
            return None, ("в справочнике не нашлось страницы, у которой "
                          "совпали бы и форма выпуска, и дозировка")
        if len(подошли) > 1:
...
async def medkit_page(request: Request, user=Depends(get_current_user),
    ...
    return templates.TemplateResponse(
        ...
        # Формы уезжают С ГОТОВЫМ ОТВЕТОМ «портится ли после вскрытия»
        # по каждой единице: правило живёт в `medkit_defs`, браузер его
        # только читает
        context={"user": user, "формы": _апт_опр.формы_для_экрана(),
...
async def medkit_create(request: Request, user=Depends(get_current_user),
                        db: Session = Depends(get_db)):
    if not user:
        return JSONResponse({"error": "Нужно войти"}, status_code=401)
    if not user_has_access(user, "medkit", db):
...
async def medkit_update(item_id: int, request: Request,
                        user=Depends(get_current_user),
                        db: Session = Depends(get_db)):
    if not user:
        return JSONResponse({"error": "Нужно войти"}, status_code=401)
...
async def medkit_take(item_id: int, request: Request,
    ...
    поз.qty_left = стало
    # «Вскрыто» ставится либо руками, либо АВТОМАТИЧЕСКИ при первом приёме
    # (постановка B.2). Дата — в поясе человека, а не процесса: на Fly
    # это UTC, и приём в час ночи по Москве датировался бы вчера (§5.0.6)
    if not поз.opened_on:
        поз.opened_on = _сегодня(user).isoformat()
        if поз.opened_days is None:
            поз.opened_days = форма.get("вскрытие_дней")
    поз.updated_at = datetime.utcnow()
    db.commit()
...
async def medkit_restore(item_id: int, request: Request,
    ...
    if вскрыто:
        ...
        # Пометку вскрытия, поставленную самим списанием, отмена обязана
        # снять: иначе «отменил» оставляло бы позицию вскрытой навсегда,
        # а у сиропа это ещё и меняет срок годности
        поз.opened_on = None
    поз.updated_at = datetime.utcnow()
    db.commit()
    return JSONResponse(_апт_ответ_сетки(request, db, user))

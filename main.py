...
def _апт_наружу(поз, сегодня, места_по_id, кат_по_позиции, кат_имена=None,
    ...
    return {
        ...
        "повтор_поможет": (True if not (поз.dosage_miss or "").strip()
                           else (True if not поз.dosage_miss_kind
                                 else поз.dosage_miss_kind
                                 in ДОЗЫ_ПОВТОР_ПОМОЖЕТ)),
...
def _апт_блоки_доз(поз):
    ...
    текст = (поз.dosage_text or "").strip()
...
async def medkit_dosage_pending(user=Depends(get_current_user),
    ...
    ПРИЗНАК — ПУСТАЯ `dosage_text`, И ЭТОГО ДОСТАТОЧНО.
    ...
    ждут = [{"id": п.id, "name": п.name}
            for п in позиции if not (п.dosage_text or "").strip()]

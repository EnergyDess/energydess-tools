...
def _апт_проверить(данные, db, user):
    ...
    код = re.sub(r"\D", "", строка("code_gtin", 40))
    if код and len(код) > 14:
        return None, "Штрих-код длиннее 14 цифр — это не GTIN"
    поля["code_gtin"] = код or None
...
def _апт_свести_черновик(черновик: dict, db, user) -> dict:
    ...
    # Штрих-код: не больше 14 цифр — это GTIN пачки, а не строка
    # со сканера целиком (§5.8)
    код = re.sub(r"\D", "", str(из_модели.get("code_gtin") or ""))
    if код and len(код) <= 14:

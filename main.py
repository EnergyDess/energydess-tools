...
def _апт_сгруппировать(позиции):
    ...
    for к in порядок:
        ...
        # ФОТО, МЕСТО И СОСТОЯНИЕ БЕРУТСЯ У ПЕРВОЙ ПАЧКИ — той, с которой
...
async def _апт_найти_дозы(поз):
    ...
    if not запросы:
        return None, "по названию этой позиции искать нечего", ДОЗЫ_ВИД_НЕЧЕГО_ИСКАТЬ
    искомое = {
    ...
    async with httpx.AsyncClient(timeout=ДОЗЫ_ТАЙМАУТ,
        ...
        for запрос in запросы:
            try:
                ...
                отказов += 1
                continue
            for u in _апт_дозы.ссылки_поиска(выдача):
                if u not in адреса:
                    адреса.append(u)
...
def _апт_записать_итог_доз(поз, итог, причина, вид=""):
...
async def _апт_дозы_фоном(user_id: int, item_id: int):
    ...
    try:
        ...
        if not поз:
            return
        итог, причина, вид = await _апт_найти_дозы(поз)
        ...
        _апт_записать_итог_доз(поз, итог, причина, вид)
        db.commit()
        if итог:

...
def _апт_места(db, user_id):
    return (db.query(MedkitPlace).filter(MedkitPlace.user_id == user_id)
            .order_by(MedkitPlace.name).all())


def _апт_категории(db, user_id):
    """Базовые (user_id IS NULL) плюс свои. Порядок: базовые в объявленном
    ...
    свои = (db.query(MedkitCategory).filter(MedkitCategory.user_id == user_id)
...
def _апт_данные(db, user):
    ...
    """
    сегодня = _сегодня(user)
    позиции = (db.query(MedkitItem).filter(MedkitItem.user_id == user.id)
               .order_by(MedkitItem.name).limit(АПТ_ПОРЦИЯ).all())
    места = _апт_места(db, user.id)
    места_по_id = {м.id: м.name for м in места}
    категории = _апт_категории(db, user.id)
    ...
    if место in ("", None, "0", 0):
        ...
        свой = (db.query(MedkitPlace)
                .filter(MedkitPlace.id == место,
                        MedkitPlace.user_id == user.id).first())
...
def _апт_категории_позиции(db, user, item_id, коды):
    ...
    """
    разрешено = {к.id for к in _апт_категории(db, user.id)}
    свои = []
    for к in коды or []:
        try:
            к = int(к)
...
def _апт_позиция(db, user, item_id):
    """Позиция пользователя или None. На чужую — 404, а не 403: отказ
    по правам подтвердил бы, что запись существует (§5.1)."""
    return (db.query(MedkitItem)
            .filter(MedkitItem.id == item_id,
                    MedkitItem.user_id == user.id).first())
...
def _апт_покупки_убрать_старое(db, user, сегодня):
    ...
    удалено = (db.query(MedkitBuyItem)
               .filter(MedkitBuyItem.user_id == user.id,
...
def _апт_покупки(db, user):
    ...
    места_по_id = {м.id: м.name for м in _апт_места(db, user.id)}
...
def _апт_свести_черновик(черновик: dict, db, user) -> dict:
    ...
    # никто не просил
    свои = _апт_категории(db, user.id)
...
{{"вступление":"одна фраза","подходят":[3,7],"почему":{{"3":"коротко, чем подходит"}},"нет":"","группа":""}}
...
4. НЕ пиши названий препаратов в тексте: их подставит сама программа рядом с остатком, сроком и местом. Твоё дело — «вступление» и короткое «почему».
...
def _апт_чат_текстом(тело):
    ...
    if вид == "заведение":
        ...
        return ("Собрал карточку: %s" % имя) if имя else "Собрал карточку."
    return ""
...
async def _апт_ответ_на_запрос(вопрос: str, db, user) -> dict:
    ...
    вступление = str(сырой.get("вступление") or "").strip()[:400]
    ...
    подложено = []
    for кусок in [вступление, нет, группа] + [str(з) for з in почему.values()]:
    ...
    if подложено:
        ...
        print("[апт-запрос] ОТБРОШЕНА формулировка модели: имена вне "
              "аптечки %r" % (подложено,))
        вступление = ("Ассистент назвал то, чего в вашей аптечке нет, — "
                      "его формулировку я показывать не стал. Вот что "
...
async def medkit_buy_add(request: Request, user=Depends(get_current_user),
    ...
    уже = next((с for с in db.query(MedkitBuyItem)
                .filter(MedkitBuyItem.user_id == user.id).all()
...
async def medkit_buy_check(buy_id: int, request: Request,
    ...
    строка = (db.query(MedkitBuyItem)
              .filter(MedkitBuyItem.id == buy_id,
                      MedkitBuyItem.user_id == user.id).first())
    ...
    return JSONResponse({**_апт_ответ_покупок(request, db, user),
                         ...
                         "строка": _апт_покупка_наружу(
                             строка, {м.id: м.name
                                      for м in _апт_места(db, user.id)})})
...
async def medkit_match(request: Request, user=Depends(get_current_user),
    ...
    искомый = _апт_ключ_группы({"name": имя, "substance": вещество,
                                "form": форма})
    сегодня = _сегодня(user)
    места = {м.id: м.name for м in _апт_места(db, user.id)}
    позиции = (db.query(MedkitItem)
               .filter(MedkitItem.user_id == user.id)
    ...
    for с in связи:
        кат_по_позиции.setdefault(с.item_id, []).append(с.category_id)
    кат_имена = {к.id: к.name for к in _апт_категории(db, user.id)}
...
async def medkit_create(request: Request, фон: BackgroundTasks,
    ...
    фон.add_task(_апт_дозы_фоном, user.id, новый)
...
async def medkit_update(item_id: int, request: Request, фон: BackgroundTasks,
    ...
    db.commit()
    if сопоставление_поехало:
        фон.add_task(_апт_дозы_фоном, user.id, item_id)
    return JSONResponse(_апт_ответ_сетки(request, db, user))
...
async def medkit_place_add(request: Request, user=Depends(get_current_user),
    ...
    есть = next((м for м in _апт_места(db, user.id)
...
async def medkit_category_add(request: Request,
    ...
    if not имя:
        return JSONResponse({"error": "Не заполнено название категории"},
                            status_code=400)
    есть = next((к for к in _апт_категории(db, user.id)

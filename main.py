...
def _апт_подпись_приёма(поз):
    """«Принял 1 таблетку». None — кнопки у этой формы нет.
    ...
    return "Принял %s %s" % (_апт_опр.число(доза), слово)
...
def _апт_наружу(поз, сегодня, места_по_id, кат_по_позиции, кат_имена=None,
    ...
    return {
        ...
        "подпись_приёма": _апт_подпись_приёма(поз),
...
async def medkit_by_code(request: Request, user=Depends(get_current_user),
    ...
    наружу = _апт_наружу(позиции[0], сегодня, места, кат_по_позиции, кат_имена)

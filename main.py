...
ПРОМПТ_ЗАПРОСА = """Человек спрашивает, есть ли у него дома лекарство под его жалобу.
Ты НЕ ставишь диагноз и НЕ назначаешь лечение. Ты ищешь по СПИСКУ НИЖЕ.
...
{{"подходят":[3,7],"почему":{{"3":"коротко, чем подходит"}},"нет":"","группа":""}}

ПРАВИЛА, нарушать нельзя:
1. В «подходят» только НОМЕРА из списка выше. Ничего, чего в списке нет, не называй — ни одного препарата из своей памяти.
2. Подбирай по КАТЕГОРИИ и назначению позиции, а не по своим знаниям о болезнях. «Болит живот» — это категория «ЖКТ». Это поиск по списку, а не диагноз.
3. НЕ называй дозировок, не пиши, сколько и как принимать. На вопрос «сколько пить» отвечай, что схема приёма — на карточке под кнопкой «Приём», а своя пометка — в поле «Заметка».
4. НЕ пиши названий препаратов в тексте: их подставит сама программа рядом с остатком, сроком и местом. Твоё дело — короткое «почему» и, если ничего не подошло, «нет» и «группа».
4а. Вступительной фразы НЕ ПИШИ: первую строку ответа собирает программа. О себе в прошедшем времени не пиши вовсе.
5. Если по списку не подошло НИЧЕГО — оставь «подходят» пустым, а в «группа» назови ГРУППУ средств словами (например «средства от изжоги»), без торговых названий, и скажи в «нет», что этого дома нет и стоит спросить у фармацевта: он знает наличие и спросит про аллергии, беременность и постоянные лекарства.
6. Если вопрос не про поиск в аптечке — оставь «подходят» пустым и объясни в «нет», на что ты отвечаешь.
7. Не советуй лечение и не оценивай тяжесть состояния.
8. ЕСЛИ В ВОПРОСЕ ЕСТЬ ХОТЬ ОДНО ИЗ ЭТОГО — {стоп} — не ищи ничего вовсе: оставь «подходят» пустым, а в «нет» скажи, что это повод обратиться к врачу, а не искать таблетку дома."""
...
def _апт_чат_записать(db, user, роль, текст, payload=None):
    ...
    """
    if not (текст or "").strip() and not payload:
        return None
...
def _апт_чат_хвост(db, user):
    ...
    if not реплики:
        return ""
    строки = ["%s: %s" % ("Человек" if м.role == "user" else "Ты",
                          (м.content or "").strip()[:600])
              for м in реплики if (м.content or "").strip()]
    if not строки:
        return ""
...
def _апт_чат_отдать(db, user, вопрос, тело):
    ...
    """
    _апт_чат_записать(db, user, "user", вопрос)
...
async def _апт_ответ_на_запрос(вопрос: str, db, user) -> dict:
    ...
    if not позиции:
        return {"вид": "запрос", "вопрос": (вопрос or "").strip()[:200],
                "вступление": "",
    ...
    вступление = (АПТ_ВСТУПЛЕНИЕ_ЕСТЬ if сколько_нашлось
                  else АПТ_ВСТУПЛЕНИЕ_НЕТ)
    if подложено:
...
async def medkit_assist(request: Request,
    ...
    return _апт_чат_отдать(db, user, вопрос, итог)

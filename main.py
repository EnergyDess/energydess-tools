...
{{"name":"торговое название","substance":"действующее вещество и его сила, например «Ибупрофен, 200 мг»","form":"один id из списка форм","unit":"один id единицы","qty_total":20,"qty_left":20,"expires_ym":"ГГГГ-ММ","code_gtin":"","категории":["Голова и боль"],"вопрос":""}}
...
7. Отвечай только про заведение карточки. Если просят посоветовать лечение — оставь поля пустыми, а в «вопрос» напиши, что подсказывать лечение ты не будешь.
...
async def _апт_спросить_модель(описание: str) -> tuple[dict, str, int]:
    """Черновик карточки, текст отказа и код ответа."""
    if not OPENROUTER_API_KEY:
    ...
    return _апт_очистить_черновик(сырой), "", 200
...
async def medkit_assist(request: Request,
    ...
    if вид.startswith("multipart/"):
        ...
        черновик, беда, код = await _апт_спросить_модель(описание)
        if беда:

...
async def _апт_ответ_на_запрос(вопрос: str, db, user) -> dict:
    ...
    if тип == "выбор":
        ...
        нет = АПТ_ВЫБОР_РАЗБОР if строки_выбора else ""
        группа = ""
    else:
        разбивка = []
...
АПТ_ВЫБОР_РАЗБОР = (
    "Чем они различаются, видно в строках выше: вещество, форма, "
    "сколько осталось и до какого срока. Что из этого подойдёт именно "
    "вам, зависит от причины, а причину по переписке не определить — "
    "это к фармацевту или врачу. Выбирать за вас я не стану.")

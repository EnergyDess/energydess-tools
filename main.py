...
def _апт_люди(db, ids, свой_id):
    ...
    ФОТОГРАФИИ УЧАСТНИКА НЕ ПОКАЗЫВАЕМ, ТОЛЬКО БУКВУ. `/avatar/{id}`
    намеренно отдаёт снимок ТОЛЬКО своему владельцу, и в шапке того
    обработчика написано, почему: «на сайте нет ни одного места, где
    один пользователь видит другого». Общая аптечка такое место
    заводит — и всё равно расширять эту дверь не стали. Лицо человека
    к вопросу «чья это пачка» отношения не имеет, а разрешение,
    выданное ради буквы на карточке, пришлось бы описывать в политике
    и сторожить отдельно. Эскиз рисует ровно буквы в кружках.
    """
    люди = {}

    ...
    async with httpx.AsyncClient(timeout=ДОЗЫ_ТАЙМАУТ,
        ...
        if not адреса:
            ...
            return None, ("справочник %s — про лекарственные средства, "
                          ...
                          % _апт_дозы.ИСТОЧНИК_ИМЯ)
        подошли, нашлись, мешает_доза = [], [], []
        for u in адреса:
            try:
            ...
            # ЕДИНСТВЕННОЕ, ЧТО НЕ СОШЛОСЬ, — ДОЗИРОВКА, И ТО ПОТОМУ,
            # ЧТО У НАС ЕЁ НЕТ. Это отдельный исход, а не мелкий разряд
            # общего отказа: форма и вещество совпали, то есть выпуск
            # ТОТ САМЫЙ, и человеку надо не «искать ещё», а дописать
            # число с упаковки (BACKLOG №178, A.4)
            if (not ок and детали["форма"] and детали["вещество"]
                    and not детали["доза"]
                    and not _апт_дозы.числа(искомое["доза"])):
                мешает_доза.append(_апт_дозы.чем_отличается(стр))
            if ок:
                подошли.append((u, стр))
        if not подошли and мешает_доза:
            # ИСХОД ЧЕТВЁРТЫЙ, И НАЙДЕН ОН ЗАМЕРОМ, А НЕ ЧТЕНИЕМ.
            #
            # «Солодки сироп»: у Видаля страница есть, форма сходится,
            # вещество сходится, а дозировка у него «4 г/100 г» и у нас
            # в карточке пусто. Прежний текст выдавал этот выпуск
            # за ЧУЖОЙ — «есть „Солодки“, но других выпусков: сироп
            # 4 г/100 г» — то есть говорил неправду про единственную
            # подходящую страницу.
            #
            # СХЕМУ ПРИ ЭТОМ НЕ ПОКАЗЫВАЕМ, и это не смягчение правила
            # A.2: у выпусков с разной концентрацией схема разная,
            # а какую держит человек — мы не знаем. Меняется ровно
            # текст отказа, и совет в нём выполним за десять секунд
            чем = _апт_выпуски_словами(мешает_доза, 3)
            return None, ("в справочнике есть подходящий выпуск%s, "
                          "но в карточке не указана дозировка — сверить "
                          "не с чем. Впишите её в поле «Действующее "
                          "вещество» так, как напечатано на упаковке"
                          % ((": " + чем) if чем else ""))
        if not подошли:
        ...
        if len(подошли) > 1:
            return None, ("под описание подошло несколько препаратов — "
                          "какой именно у вас в руках, решать не нам")
        u, стр = подошли[0]
        ...
        if not текст.strip():
            return None, "на странице препарата нет раздела о дозировании"
        if not блоки:
...
def _апт_записать_итог_доз(поз, итог, причина):
...
async def _апт_дозы_фоном(user_id: int, item_id: int):
    ...
    try:
        ...
        if not поз:
            return
        итог, причина = await _апт_найти_дозы(поз)
        ...
        _апт_записать_итог_доз(поз, итог, причина)
        db.commit()
        if итог:

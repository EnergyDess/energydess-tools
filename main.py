...
def _build_compact_dossier(profile: HHProfile) -> str:
    ...
    if profile.projects:
        titles = [p.get('title', '') for p in profile.projects[:5] if p.get('title')]
        if titles:
            parts.append(f"Проекты: {', '.join(titles)}")
...
relevant_portfolio_links — ТОЛЬКО ссылки из резюме/досье, реально релевантные этой вакансии. Пустой массив если ничего не подходит.
"""
    try:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ar = await client.post(
                ...
                json={"model": LETTER_MODEL, "messages": [{"role": "user", "content": analysis_prompt}],
...
Портфолио — только по релевантности. Если выше указаны «РЕЛЕВАНТНЫЕ ССЫЛКИ» — используй ТОЛЬКО их и НИЧЕГО больше. Если список пуст — не упоминай портфолио вообще. Упоминать портфолио или шоурил без реального URL — запрещено. Ссылки вставлять дословно, каждую на отдельной строке.
...
"""
    try:
        ...
        return JSONResponse({"letter": letter, "analysis": analysis})
    except httpx.TimeoutException:

...
def _блок_состава(logs, profile) -> dict | None:
    ...
    for l in logs:
        ...
        # «Возраст тела: 40 лет» без второго числа не говорит ничего:
        # сорок это много или мало, зависит от того, сколько человеку
        # на самом деле. Ставим рядом возраст из анкеты — и только его,
        # без вывода «вы старше своих лет»: вывод сделает человек
        if возраст:
            for с in строки:
                if с["поле"] == "body_age":
                    с["сравнение"] = f"в анкете — {возраст}"
        return {
            "дата": l.log_date,

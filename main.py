...
async def _апт_найти_дозы(поз):
    ...
    название = _апт_дозы.имя_для_поиска(поз.name)
    if not название:
    ...
    async with httpx.AsyncClient(timeout=ДОЗЫ_ТАЙМАУТ,
        ...
        try:
            выдача = await _апт_страница(
                кл, _апт_дозы.ИСТОЧНИК_ПОИСК % quote(название))
        except Exception as e:
            print("[medkit-doses] поиск не ответил: %r" % (e,))
            return None, "справочник не ответил — попробуйте позже"
        адреса = _апт_дозы.ссылки_поиска(выдача)
        if not адреса:
        ...
        for u in адреса:
            try:
                ...
                continue
            ок, _ = _апт_дозы.подходит(искомое, стр)

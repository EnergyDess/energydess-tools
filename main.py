...
{{"тип":"поиск","подходят":[3,7],"почему":{{"3":"коротко, чем подходит"}},"нет":"","группа":""}}
...
· "иное" — любой другой вопрос: можно ли принять ещё одну таблетку, можно ли с алкоголем, можно ли детям, сколько дней пить, что с чем сочетается, чем одно отличается от другого. В «подходят» тут ставь номера ТОЛЬКО если человек назвал конкретный препарат из списка, иначе оставь пустым.
...
4. НЕ пиши названий препаратов в тексте и НЕ пиши номеров позиций: их подставит сама программа рядом с остатком, сроком и местом. Твоё дело — короткое «почему» и, при необходимости, «нет» и «группа».
4а. Вступительной фразы НЕ ПИШИ: первую строку ответа собирает программа. О себе в прошедшем времени не пиши вовсе.
5. Тип "поиск", и по списку не подошло НИЧЕГО — оставь «подходят» пустым, а в «группа» назови ГРУППУ средств словами (например «средства от изжоги»), без торговых названий, и скажи в «нет», что этого дома нет и стоит спросить у фармацевта: он знает наличие и спросит про аллергии, беременность и постоянные лекарства.
...
async def _апт_ответ_на_запрос(вопрос: str, db, user) -> dict:
    ...
    почему = сырой.get("почему") if isinstance(сырой.get("почему"), dict) else {}
    ...
    вольное = [нет, группа] + [str(з) for з in почему.values()]
    ...
    подложено = []
    for кусок in [нет, группа] + [str(з) for з in почему.values()]:
    ...
    for н in номера:
        ...
        строка = {"id": н, "строка": _апт_строка_позиции(п),
                  "почему": str(почему.get(str(н)) or почему.get(н) or "")[:200],
                  "дозы": bool(п["дозы"])}
    ...
    if (тип == "иное" and not (нет or группа)
            and not any(п["почему"] for п in
                        (нашлось + просрочено + рецептурные))
            and not подложено and not дозы_в_тексте):

...
async def nut_history_month(month: str = None, user=Depends(get_current_user),
    ...
    Дни ДО заведения журнала своей нормы не имеют, и у них `goal: null`.
    Полоски у такой клетки нет вовсе — доля от подставленного числа была бы
    утверждением о прошлом, которого никто не проверял. Отсутствие полоски
    само требует подписи, иначе читается как «ничего не ел», — поэтому
    рядом едет `goals_unknown_days`, и месяц говорит «у 11 дней норма
    не записана, полоски нет»."""
    if not user:
        return JSONResponse({"error": "Не авторизован"}, status_code=401)
    сегодня = _сегодня(user)
    try:
    ...
    журнал = нормы_журнала(db, user.id)
    текущие = nut_goals(db.query(NutritionProfile).filter(
    ...
    for д, к, б, ж, у, н in строки:
        ...
        дни[д] = {"calories": round(к or 0), "protein": round(б or 0),
                  ...
                  # None — у клетки нет знаменателя, и полоски у неё не будет.
                  # Съеденное при этом на месте и видно в подсказке

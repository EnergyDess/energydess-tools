...
def _апт_годен_до(поз):
    ...
    # это упаковкой Креона, на которой НАПЕЧАТАНО «срок годности после
...
#
# ЗАПИСИ ОСТАЮТСЯ РАЗДЕЛЬНЫМИ. Две пачки Цетрина — это две строки базы,
...
def _апт_ключ_группы(п):
    ...
    Приводятся тем же `нормаль`, что и строка поиска: иначе «Цетрин»
    и «цетрин  » оказались бы разными препаратами (§6.0.7 — один способ
...
def _апт_сгруппировать(позиции):
    ...
    for к in порядок:
        ...
        # «Цетрин, 2 упаковки» БЕЗ КАРТИНКИ при живом снимке. Разбор:
        ...
        # «Экстракт корня лопуха» 05.2027 против 01.2029 — то же самое.
        #
...
async def _апт_найти_дозы(поз):
    ...
    # как напечатано на коробке — «Цетрин таб. п/о 10мг №20», — а
    # справочник по такой строке не находит НИЧЕГО. Замер: запрос
    # целиком даёт ноль страниц, «Цетрин» — три
    # ЗАПРОСОВ БЫВАЕТ ДВА, И ВТОРОЙ НЕ ЗАПАСНОЙ, А РАВНОПРАВНЫЙ.
    # Слово формы бывает частью самого названия («Солодки сироп»),
    ...
    искомое = {
        ...
        # поставили «Креон 25000» на карточку «Креон 10000»
        "название": поз.name,
        "имена": запросы,
        "форма": поз.form,
    ...
    # на «Фослюган комплекс для кишечника» как поломку — и не мог понять,
    ...
    try:
      async with httpx.AsyncClient(timeout=ДОЗЫ_ТАЙМАУТ,
        ...
        for запрос in запросы:
            ...
            # «Нимесулида» 54 ссылки, первые шесть — гель, таблетки
        ...
        # построению: у «Солодки сироп» первые шесть мест занимает
        # выдача по «Солодки», и страница сиропа не попала бы
        ...
        if not адреса:
            ...
            #
            # Владелец получил его на «Фослюган комплекс для кишечника»
            # и прочитал как поломку. Замер по трём запросам подряд
            # («Фослюган», полное название, заведомая бессмыслица) — все
        ...
        for u in адреса:
            ...
            #
            # На её место встал исход, который замер нашёл на «Солодки
            # сиропе» владельца: название совпало, форма совпала,
            # а вещество у нас записано «Солодки корней экстракт густой»
            # против «солодка» в справочнике. Прежний текст объявлял
            # эту страницу ЧУЖИМ ВЫПУСКОМ («есть „Солодки сироп“,
            ...
            if (not ок and детали["название"] and детали["форма"]
                ...
                # и совет вышел бессмысленным: замер на «Солодки сиропе»
                # дал «действующее вещество там названо иначе: „сироп;
                # солодка“», где «сироп» это форма, а не вещество.
        ...
        if not подошли and мешает_вещество:
            ...
            #
            # «Солодки сироп» владельца: у Видаля страница ЕСТЬ,
            # название сходится, форма сходится — а действующее вещество
            # записано у нас «Солодки корней экстракт густой» против
            # «солодка» в справочнике, и по границе слова это разные
            # строки. Прежний текст выдавал такую страницу за ЧУЖОЙ
            # ВЫПУСК («есть „Солодки сироп“, но других выпусков: сироп»),
        ...
        if not подошли:
            ...
            #
            # Замер на Необутине владельца: у Видаля есть ТРИ страницы —
...
def _апт_выпуски_словами(описания, сколько):
    ...
    ПОВТОР ТУТ НЕ ТЕОРЕТИЧЕСКИЙ. У «Солодки сиропа» шесть страниц
...
СТОП_СИМПТОМЫ = (
    ...
    ("принятое лекарство не помогло",
     ...
     ("не помог", "не помогл", "не проход", "не действ",
      ...
      # пример постановки «выпила необутин, всё равно болит»
...
def _апт_позиции_для_запроса(db, user):
    ...
    и ответ «Цетрин, годен до 07.2027, полка в ванной» про сводку группы
...
# осталось им же. Два — это русское падежное окончание: «Цетрина»,
# «Цетрином», «Нурофенов». Три уже пропускают «Цетринакс», то есть
...
def _апт_имя_знакомо(слово: str, разрешённые: set) -> bool:
    ...
    ПОЧЕМУ НЕ ТОЧНОЕ РАВЕНСТВО. Модель склоняет: «две упаковки Цетрина».
...
#
#   «А как принимать необутин?»       → прислал список препаратов заново
...
· "схема" — спрашивают, КАК ПРИНИМАТЬ конкретный препарат: «как принимать нурофен», «нурофен до или после еды», «покажи схему приёма цетрина». В «подходят» поставь НОМЕРА тех позиций из списка, про которые спрашивают, и больше ничего не пиши: схему покажет сама программа с карточки.
...
async def medkit_update(item_id: int, request: Request, фон: BackgroundTasks,
    ...
    поправь человек «Необутин» на «Тримедат» — и на карточке остался бы
    ...
    # значимой. Строка «правка карточки — Цетрин» верна при любом
...
async def medkit_restore(item_id: int, request: Request,
    ...
    #
    # Окно отмены — шесть секунд, и «Зина — приём: Цетрин» рядом
    # с «Зина — отмена приёма: Цетрин» это не история, а шум: соседу
...
# о дозировании у Видаля из замеренных (Цетрин, 2584 знака вместе
...
async def medkit_substance_from_ref(item_id: int, request: Request,
    ...
    четыре добавки он не знает вовсе, а у «Ренгалина» страница есть,
    но блока «Активное вещество» на ней нет.

    Зато нашёлся исход, где справочник вещество ЗНАЕТ, а у нас оно
    записано иначе: «Солодки корней экстракт густой» против «солодка»,

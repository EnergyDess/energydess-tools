...
async def _апт_ответ_на_запрос(вопрос: str, db, user) -> dict:
    ...
    # ── ТОТ ЖЕ ОТВЕТ ДВА РАЗА ПОДРЯД — ЗНАЧИТ ВОПРОС НЕ ПОНЯТ (A.2) ─
    #
    # СЧИТАЕТСЯ ПОСЛЕ ВСЕГО ОСТАЛЬНОГО, включая подстановку выше:
    # предмет сравнения — то, что человек ПРОЧИТАЕТ, а не то, что
    # прислала модель. Посчитай подпись раньше — и подставленное
    # сервером объяснение в неё не попало бы.
    if _апт_подпись_ответа(тело) == _апт_прошлый_ответ(db, user):
        print("[апт-запрос] ответ ДОСЛОВНО повторяет прошлый — "
              "говорим, что вопрос не понят")
...
def _апт_подпись_ответа(тело) -> str:
    ...
    return "|".join([str(тело.get("вступление") or ""),
                     str(тело.get("нет") or ""),

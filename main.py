...
async def medkit_dosage(item_id: int, request: Request,
    ...
    поз.dosage_source = _апт_дозы.ИСТОЧНИК_ИМЯ
    поз.dosage_url = адрес
    # Разметка — только рядом с текстом и только сошедшаяся. Пустой
    # список пишем как NULL: «блоков нет» и «блоков ноль» это одно
    # и то же, а две записи одного факта разошлись бы
    поз.dosage_blocks = _json.dumps(блоки, ensure_ascii=False) if блоки else None
    поз.dosage_fetched_at = datetime.utcnow()
    db.commit()
    return JSONResponse(_апт_ответ_сетки(request, db, user))

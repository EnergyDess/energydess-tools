...
# и живут столько же, сколько процесс. Ровно так же устроен кеш
# страниц рядом.
_апт_клиенты = {}


def _апт_клиент(источник):
    """Живой HTTP-клиент этого источника, общий на процесс."""
    кл = _апт_клиенты.get(источник.домен)
    if кл is None or кл.is_closed:
        кл = httpx.AsyncClient(
            timeout=getattr(источник, "таймаут", ДОЗЫ_ТАЙМАУТ),
            follow_redirects=True, verify=False)
        _апт_клиенты[источник.домен] = кл
    return кл

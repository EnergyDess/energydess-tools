...
Портфолио — только по релевантности. Прикрепляй ТОЛЬКО ссылки, реально связанные с этой конкретной вакансией: если вакансия про AI-видео — ссылку на разработку не давать; если про разработку — ссылку на видеомонтаж не давать; если релевантно и то, и другое — дать обе с коротким пояснением. Упоминать портфолио или шоурил без реального URL из резюме — запрещено. Ссылки вставлять дословно из текста резюме, каждую на отдельной строке.

Инструменты. Не выдумывай инструменты и программы, которых нет в опыте пользователя. Опирайся на резюме, а в будущем — на досье пользователя и на custom_context, если он передан. Не заменяй один инструмент на похожий по смыслу.

Call to action в финале. Конкретное предложение: созвон, тестовое задание или показ процесса вживую. Никаких пустых оборотов перед подписью: «жду вашего ответа», «буду рад», «надеюсь на сотрудничество», «Надеюсь на…», «В заключение хочу». Подпись «С уважением, [имя]» — допустима как стандарт делового письма.
...
Объём: 100–180 слов. Абзацы разделять пустой строкой. Структура: первый абзац — заход + суть совпадения; второй-третий — конкретика и факты; финал — портфолио (если релевантно) + call to action. Без эмодзи. Без маркированных списков внутри письма — только связный текст.
"""

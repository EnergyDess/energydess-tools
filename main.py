...
async def robots_txt():
    ...
    Пустой Disallow — это не заготовка, а принятая форма записи «запретов
    нет»: строка обязана присутствовать, иначе группа User-agent считается
    незавершённой у части роботов.

    Здесь СТОЯЛ `Disallow: /botamin`, и он убран сознательно. Причин две.

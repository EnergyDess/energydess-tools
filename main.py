...
async def _апт_ответ_на_запрос(вопрос: str, db, user) -> dict:
    ...
    for н in номера:
        ...
        строка = {"id": н, "строка": _апт_строка_позиции(п),
                  ...
                  "показание": _апт_показание_по_вопросу(п, основы),
                  "показания_есть": bool((п.get("показания") or "").strip()),
                  "дозы": bool(п["дозы"])}
    ...
    def _ступень(с):
        if с.get("показание"):
            return 0
        return 1 if с.get("показания_есть") else 2

    for куча in (нашлось, просрочено, рецептурные):
        куча.sort(key=_ступень)
    # СКОЛЬКО СОШЛОСЬ ДОСЛОВНО — числом, а не на глаз. По нему экран
    # решает, ставить ли подзаголовок «про эти сказать нечего»
    все_строки = нашлось + просрочено + рецептурные
    по_показанию = sum(1 for с in все_строки if с.get("показание"))
...
async def medkit_assist(request: Request,
    ...
    if вид.startswith("multipart/"):
        ...
        if ждём:
            сырой_у, беда, код = await _апт_спросить_модель(

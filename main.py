    ...
    try:
      async with httpx.AsyncClient(timeout=ДОЗЫ_ТАЙМАУТ,
        ...
        for запрос in запросы:
            ...
            if not _апт_дозы.справочник_знает(выдача, искомое["форма"],
                                              искомое["доза"]):
                continue
        ...
        return (текст, _апт_дозы.ИСТОЧНИК_БАЗА + u, блоки,
                п_текст, п_блоки), "", "", _апт_подсказки_страницы(стр)
...
def _апт_подсказки_страницы(страница_html):
    ...
    """
    плоско = _апт_дозы.без_тегов(страница_html or "")
    опис = _апт_дозы.описание(плоско)
    код, слово = _апт_дозы.форма_страницы(опис)
    return {
        "вещество": _апт_дозы.вещество_страницы(страница_html) or "",
        "форма": код or "",

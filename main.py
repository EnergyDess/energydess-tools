...
def _nut_chat_system(date: str, logs: list, water: int, profile,
    ...
    `нормы=None` означает, что за этот день нормы у нас НЕТ. Тогда блока
    с числами в промпте нет вовсе, и модели прямо сказано, что остаток
    считать не от чего. Подставить сюда сегодняшнюю значило бы дать модели
    число, которого экран рядом не показывает, — и ассистент назвал бы
    остаток до нормы там, где кольцо стоит пустым.
...
- вода: {цели['water_ml']} мл""" if цели else
        f"НОРМ ЗА {date} У ПРИЛОЖЕНИЯ НЕТ. Нормы записываются с того дня, "
        f"как завели их историю, а {date} этой записи не застал. Значит, "
        f"остаток до нормы за этот день считать НЕ ОТ ЧЕГО — так и скажи, "
        f"если спросят, и НЕ подставляй сегодняшнюю: экран рядом её тоже "
        f"не показывает. Съеденное за день ниже — оно известно точно.")
...
async def nut_diary(date: str = None, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    нормы, с_какого = норма_на_день(журнал, d, _сегодня(user).strftime("%Y-%m-%d"),
                                    nut_goals(profile))
    # `goals: null` у дня без записанной нормы — форма ответа, а не забывчивость.
    # Верни мы сюда сегодняшнюю норму «на всякий случай», и любой вызывающий,
    # забывший посмотреть на признак, молча нарисовал бы долю от чужого числа.
    # Тот же довод, по которому гейт почты сделан одним middleware (§5.3):
    # правильное поведение не должно зависеть от того, вспомнил ли о нём
    # следующий
    diary["goals"] = нормы
    diary["goals_meta"] = {
        "known": нормы is not None,
        "from": с_какого,
...
async def nut_history_month(month: str = None, user=Depends(get_current_user),
    ...
    текущие = nut_goals(_анкета) if _анкета else None
...
async def nut_ai_chat(request: Request, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    нормы_дня, _с = норма_на_день(
        ...
        _сегодня(user).strftime("%Y-%m-%d"), nut_goals(profile))
    system = _nut_chat_system(date, logs, water, profile, нормы_дня)

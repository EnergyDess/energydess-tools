...
class СбойСервисаМоделей(RuntimeError):
    """ОТВЕТ ЧУЖОГО СЕРВИСА, КОТОРЫЙ НЕЛЬЗЯ ПОКАЗЫВАТЬ КОМУ ПОПАЛО.

    BACKLOG №244, C.2. Владелец увидел на экране аптечки:

        Ошибка ИИ (402): This request requires more credits, or fewer
        max_tokens… visit https://openrouter.ai/settings/credits

    Постороннему человеку это не говорит ничего, кроме того, что
    у владельца сайта кончились деньги, и зовёт его в чужой кабинет.

    ОТДЕЛЬНЫЙ КЛАСС, А НЕ ПРИЗНАК В ТЕКСТЕ. Наши собственные
    сообщения — «Снимок весит 24 МБ при потолке 5», «разбор оборвался
    по лимиту» — человеку НУЖНЫ и прятать их нельзя.
    Отличить их от чужого ответа по строке нечем: разбирать текст
    исключения значило бы завести признак, который развалится
    от первой же правки формулировки (§6.0.7, довод `повтор_поможет`).
    Класс отвечает на этот вопрос по построению: его поднимает ровно
    то место, которое пересказывает ЧУЖОЙ ответ.
    """


def _сбой_наружу(e: Exception, user=None) -> str:
    """ЧТО ИЗ СБОЯ УВИДИТ ЧЕЛОВЕК — одно место на весь проект.

    BACKLOG №244, C.2. Владельцу (`is_admin`) — полный текст: чинить
    это ему, и «сервис недоступен» отняло бы у него код ответа
    и причину. Всем остальным — фраза, из которой следует, что делать.

    Наши сообщения проходят КАК ЕСТЬ: они и написаны для человека.
    """
    if isinstance(e, СбойСервисаМоделей):
        if user is not None and getattr(user, "is_admin", False):
            return str(e)
        return ("Ассистент временно недоступен — попробуйте позже. "
                "Данные аптечки на месте.")
    return str(e)
...
async def _сеть(db, корутина):
    """Поход в СЕТЬ С ОСВОБОЖДЁННЫМ СОЕДИНЕНИЕМ к базе.

    Третья сестра `_пауза` и `_в_потоке`, и понадобилась по той же
    причине — только держали соединение не сном и не счётом, а ЧУЖИМ
    СЕРВИСОМ, который отвечает десятки секунд.

    ЗАМЕР 2026-09-07, из-за которого это отдельная функция. Заглушка
    вместо OpenRouter, `POST /medkit/api/assist` идёт 35 с, в это же
    время открывается `GET /medkit`:

        GET /medkit   500 «database is locked» через 32 858 мс

    Трасса SQL по соединениям показала механизм целиком:

        [соединение А] DELETE FROM chat_messages … created_at < ?
        [соединение А] SELECT chat_messages …
        …  35 секунд ожидания ответа модели  …
        [соединение Б] DELETE FROM medkit_buy_items …   ← ждёт и падает

    Ленивая уборка старых реплик — ЗАПИСЬ, она открывает в SQLite
    транзакцию записи, и та живёт до конца обработчика. Второй писатель
    ждёт `busy_timeout` (30 с) и получает `database is locked`. То есть
    пока ассистент думает, сайт в соседней вкладке отдаёт пятисотку.

    ПОЧЕМУ ЭТО НЕ ЛЕЧИТСЯ РАСШИРЕНИЕМ `busy_timeout`. Ждать дольше
    значит не «перестать падать», а «падать позже и молча тормозить
    всё это время»: у SQLite один писатель по построению, и ждущий
    держит поток. Лечится тем, что писателя на время сети нет вовсе.

    БЕДА ШИРЕ ОДНОГО УПАВШЕГО ЗАПРОСА, и это НАШЁЛ ЗАМЕР, а не
    постановка. Ждущий блокировку запрос ждёт её СИНХРОННО внутри
    async-обработчика, то есть стоит ВЕСЬ ЦИКЛ СОБЫТИЙ (§6.0.5). Видно
    это по самому виновнику: в контрольном прогоне ассистент, которому
    заглушка отвечает за 12 с, шёл 35 с — лишние 23 он простоял, пока
    соседи досиживали свои тридцать секунд. То есть один поход в сеть
    под открытой транзакцией останавливает САЙТ ЦЕЛИКОМ, включая
    `/health`, а не только тот запрос, который упал.

    ЦЕНА ТА ЖЕ, ЧТО У СЕСТЁР, и правило на вызывающей стороне то же:
    `db.close()` ОТКАТЫВАЕТ незакоммиченное и ОТЦЕПЛЯЕТ объекты ORM.
    Значит значение, нужное после, берётся в переменную ДО; объект,
    который будут менять, — из базы заново; а запись, которая обязана
    пережить поход в сеть, коммитится ДО него. Соблюдение первого
    сторожит `check_detached.py` (проверка 15), второго —
    `check_db_hold.py` (проверка 27).

    Передаётся КОРУТИНА, а не функция с аргументами: у сетевых вызовов
    проекта аргументы разной формы (позиционные, именованные, целые
    словари), и обёртка «функция плюс аргументы» заставила бы каждое
    место переписывать вызов. `_сеть(db, f(a, b=1))` читается как
    исходный `await f(a, b=1)` и правится в одну строку.

    СЕССИИ МОЖЕТ НЕ БЫТЬ, и это законно — ровно как у `send_email`,
    который зовут и без неё. Отпускать тогда нечего; ронять на этом
    падением значило бы требовать сессию там, где её по смыслу нет.
    """
    if db is not None:
        db.close()
...
async def send_email(to: str, subject: str, html: str, text: str = None,
    ...
    if error:
        ...
        if text:
            письмо["text"] = text
        # СОЕДИНЕНИЕ К БАЗЕ ОТПУСКАЕМ ДО ПОХОДА В СЕТЬ (см. `_сеть`).
        # Отпускается ЗДЕСЬ, а не у каждого из семи вызывающих: заслон,
        # который надо не забыть дописать, следующий эндпоинт не получит —
        # та же причина, по которой суточный запас адреса считается тоже
        # здесь. Незакоммиченного к этому месту нет ни у одного вызывающего
        # (замерено обходом дерева), а объекты после этого ОТЦЕПЛЕНЫ —
        # сторожит `check_detached.py`.
        if db is not None:
            db.close()
        try:
            async with httpx.AsyncClient(timeout=10) as client:
...
@app.get("/version")
async def version():
    """Какой коммит СЕЙЧАС на машине.

    ═══════════════════════════════════════════════════════════════════
    НА ПРОДЕ ЭТО ОТВЕЧАЛО «unknown», И ОСЬ ПРОВЕРКИ БЫЛА СЛЕПА

    BACKLOG №245, E.5. Здесь стоял ТОЛЬКО вызов `git rev-parse`,
    а `.git/` исключён из образа `.dockerignore` — то есть на Fly
    git не находил репозитория и ответ был «unknown» ВСЕГДА, с первого
    дня. `check_deploy.py` (проверка «доехал ли код до прода») из-за
    этого честно печатала «спросить нечем» по оси коммита — то есть
    самая нужная её ось не работала ни разу.

    ТЕПЕРЬ ОТПЕЧАТОК ВПЕКАЕТСЯ В ОБРАЗ на сборке: `deploy.yml` передаёт
    `--build-arg GIT_COMMIT=${{ github.sha }}`, `Dockerfile` кладёт его
    в переменную окружения. Прочитать его дороже, чем строку из env,
    не нужно — и подпроцесса на каждый запрос больше нет.

    GIT ОСТАЁТСЯ ЗАПАСНЫМ ПУТЁМ, и это не подстраховка «на всякий»:
    локально репозиторий на месте, переменной нет, и без запасного
    пути разработчик получал бы «unknown» у себя — то есть проверка
    выкатки не прогонялась бы и на машине, где её пишут.
    """
    впечатано = (os.getenv("GIT_COMMIT") or "").strip()
    if впечатано and впечатано != "unknown":
        # ОТВЕТ КОРОТКИМ ОТПЕЧАТКОМ, как у `git rev-parse --short`:
        # рядом с ним печатается вывод локального git, и два разных
        # формата одного и того же пришлось бы сверять глазами
        return JSONResponse({"commit": впечатано[:7], "откуда": "образ"})
    import subprocess
    ...
    try:
        ...
        commit, откуда = "unknown", "нечем"
    return JSONResponse({"commit": commit, "откуда": откуда})
...
async def forgot_post(request: Request, email: str = Form(...), db: Session = Depends(get_db)):
    ...
    # ЗАПИСЬ ТОКЕНА И ОТПРАВКА ПИСЬМА РАЗВЕДЕНЫ ПО ДВУМ БЛОКАМ, и это
    # не оформление. `send_email` отпускает соединение к базе на время
    # похода в сеть (см. `_сеть`), а `db.close()` ОТЦЕПЛЯЕТ объекты ORM:
    # держи мы запись и отправку в одном блоке, следующая правка легко
    # поставила бы присваивание в `user` после письма — и оно не доехало
    # бы до базы молча. Разведённые блоки делают это невозможным
    # по построению, и ровно так это видит `check_detached.py`.
    link, кому = None, None
    if user:
        rtok = generate_token()
        ...
        db.commit()
        link, кому = f"{BASE_URL}/reset-password/{rtok}", user.id

    if link:
        await send_email(
            db=db, user_id=кому, kind="reset",
            to=email,
...
def _енш_каталог_наружу(db):
    """Каталог сетов с числом отметок. ОДНО место на два потребителя —
    страницу и `/admin/api/enshrouded/rows`: две копии сборки разошлись
    бы порядком строк, и таблица после правки перестала бы совпадать
    с первым кадром."""
...
def _енш_подпись_категории(cid):
    """«blacksmith» → «Кузнец». Незнакомая категория показывается КАК
    ЕСТЬ: категория, выпавшая из показа молча, — ровно та дыра, которую
    ищет проверка 24."""
    return _енш_опр.КОРОТКО.get(cid, cid)


def _енш_состав_словами(с):
    """Состав сета словами — тем же `enshrouded_defs.словами`, которым
    он назывался в скрипте. Второй реализации не заводится (§6.0.7)."""
    свой = с.get("custom") or []
    if свой:
        return "свой: " + ", ".join(к.get("label", "") for к in свой)
    куски = с.get("pieces") or []
    if куски:
        return _енш_опр.словами(куски)
    return "полный (5)"
...
async def admin_enshrouded_page(request: Request, user=Depends(get_current_user),
    ...
    return templates.TemplateResponse(
        ...
        context={"user": user, "sets": данные,
                 ...
                 "подпись_категории": _енш_подпись_категории,
                 "состав_словами": _енш_состав_словами,
                 "max_w": ENS_MAX_W, "max_h": ENS_MAX_H})


@app.get("/admin/api/enshrouded/rows")
async def admin_enshrouded_rows(request: Request,
                                user=Depends(get_current_user),
                                db: Session = Depends(get_db)):
    """Строки каталога ГОТОВОЙ РАЗМЕТКОЙ — тем же партиалом, что рисует
    первый кадр.

    Нужен затем, чтобы построитель остался ОДИН: после правки, создания
    и удаления сета таблицу надо перерисовать, и собери её скрипт —
    построителей стало бы двое, а разошлись бы они молча (§6.0.15,
    тот же довод, что у `/medkit/api/grid`)."""
    if not _admin_guard(user):
        return JSONResponse({"error": "нет прав"}, status_code=403)
    данные = _енш_каталог_наружу(db)
    return templates.TemplateResponse(
        request=request, name="_ens_rows.html",
        context={"sets": данные,
                 "подпись_категории": _енш_подпись_категории,
                 "состав_словами": _енш_состав_словами})
...
"""
    try:
        ...
        if response.status_code != 200:
            # ЧУЖОЙ ТЕКСТ — ЧЕРЕЗ ОДНО МЕСТО (BACKLOG №244, C.2).
            # Здесь он уходил наружу ЦЕЛИКОМ, даже без обрезки
            return JSONResponse({"error": _сбой_наружу(
                СбойСервисаМоделей(
                    "Ошибка OpenRouter: %s" % response.text[:300]),
                user)}, status_code=500)
...
def _upright_jpeg(content: bytes, max_dim: int = 1920,
    ...
    try:
        ...
        # с обычного Android просили +383 МБ при 256 МБ у машины
        # (с 2026-09-05 машина 512 — `fly.toml`, BACKLOG №242).
...
async def _call_vision(b64: str, mime: str, prompt: str, max_tokens: int = VISION_MAX_TOKENS) -> str:
    ...
    if resp.status_code != 200:
        raise СбойСервисаМоделей(
            f"Ошибка ИИ ({resp.status_code}): {resp.text[:200]}")
...
async def nut_search(q: str = "", user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    off_results, сбой = await _сеть(db, _off_search(q))
...
async def nut_barcode(code: str, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    result = await _сеть(db, _off_barcode(code))
    if result:
...
async def nut_ai_chat_log_foods(request: Request, user=Depends(get_current_user),
    ...
    if записанные:
        перечень = ", ".join(
            ...
            for r in записанные)
        текст = f"Записано в дневник за {date}: {перечень}."
    else:
...
{{"name":"название блюда","brand":"заведение или производитель (пустая строка, если неизвестно)","calories":150,"protein":10,"fat":5,"carbs":20,"estimated_grams":300,"note":"краткое пояснение"}}"""
    ...
    try:
        ...
        return JSONResponse(
            {"error": "Не удалось распознать: %s" % _сбой_наружу(e, user)},
            status_code=500)
...
{{"name":"название блюда","brand":"заведение или производитель (пустая строка, если неизвестно)","calories":150,"protein":10,"fat":5,"carbs":20,"estimated_grams":300}}"""
    ...
    # РЕПЛИКА ЧЕЛОВЕКА ДОБАВЛЯЕТСЯ ПОСЛЕ СЕТИ, а не до, и это НЕ вкус.
    # `_сеть` отдаёт соединение через `db.close()`, а тот ОТЦЕПЛЯЕТ всё
    # несохранённое: `db.add(...)` до похода в сеть был бы откачен молча —
    # сообщение человека исчезло бы из переписки без единой строки
    # в журнале (§6.0.5, третий случай того же класса за три дня).
    # Порядок реплик от переноса не меняется: обе уезжают одним коммитом
    # ниже, а `created_at` у них проставляется здесь же.
    моя_реплика = ChatMessage(user_id=user.id, role="user",
                              content=user_text or "[фото блюда]",
                              image_path=токен)
...
async def nut_transcribe(file: UploadFile = File(...),
    ...
    # ── ЧТО ВИДИТ ЧЕЛОВЕК, КОГДА РЕЧЬ НЕ РАСПОЗНАЛАСЬ (№245, E.3) ──
    #
    # ЗДЕСЬ СТОЯЛ СЫРОЙ ТЕКСТ ИСКЛЮЧЕНИЯ: «Не удалось распознать речь:
    # HTTPStatusError: Client error 402 …». Две беды разом.
    #
    # ПЕРВАЯ — ЭТО ЧУЖОЙ ОТВЕТ. Тело от OpenRouter уходило постороннему
    # человеку целиком, ровно как в задаче 244 (C.2), где владелец
    # увидел на экране код 402 и ссылку на чужой кабинет оплаты.
    # Правило оттуда закрывало пять точек и НЕ закрывало эту.
    #
    # ВТОРАЯ — ФРАЗА НЕ ГОВОРИТ, ЧТО ДЕЛАТЬ. «HTTPStatusError» человеку
    # не значит ничего, а причин у этой ветки ровно две, и советы
    # у них РАЗНЫЕ: сеть не ответила — повтор помогает; сервис ответил
    # отказом — повтор не поможет никогда.
    #
    # ЧТО ЗАМЕР НЕ ПОДТВЕРДИЛ, И ЭТО СКАЗАНО ПРЯМО: живой случай
    # владельца воспроизвести НЕ УДАЛОСЬ. Пять краевых входов —
    # запись 0.3 с, mp4, незнакомый content-type, тип с параметрами
    # codecs и пустой файл — дали HTTP 200 (пустой даёт свой 400
    # «Пустая запись»). То есть формат записи тут ни при чём;
    # остаётся отказ самого сервиса, и его текст теперь человеческий.
    сеть = isinstance(last_error, httpx.HTTPError) and not isinstance(
        last_error, httpx.HTTPStatusError)
    if сеть:
        текст = ("Не удалось связаться с сервисом распознавания речи. "
                 "Попробуйте ещё раз через минуту — или наберите текстом.")
        код = 503
    else:
        текст = ("Сервис распознавания речи вернул ошибку — это сбой "
                 "на нашей стороне, а не в вашей записи. Повтор, скорее "
                 "всего, не поможет: наберите текстом.")
        код = 502
    # ВЛАДЕЛЬЦУ — ПОДРОБНОСТИ, ОСТАЛЬНЫМ НЕТ, тем же местом, что
    # и у всех прочих пересказов чужого ответа
    if user and getattr(user, "is_admin", False) and last_error:
        текст += " (%s: %s)" % (type(last_error).__name__,
                                str(last_error)[:200])
    return JSONResponse({"error": текст}, status_code=код)
...
async def workout_page(request: Request, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request=request, name="workout.html", context={
        ...
        # ПРИЧИНУ ПРОПУСКА ЧЕЛОВЕК ГОВОРИТ О СЕБЕ, значит слово
        # согласуется с ЕГО родом. Пол не указан — нейтральное «Нет
        # сил»: оно не хуже, а «Устал» женщине просто неверно.
        "wk_причина_усталости": по_полу(
            getattr(user, "gender", None), "Устал", "Устала", "Нет сил"),
    })
...
async def workout_generate_program(user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return JSONResponse({"error": _сбой_наружу(
                СбойСервисаМоделей(
                    f"Ошибка OpenRouter: {response.text[:300]}"),
                user)}, status_code=500)
...
async def exercise_preview(youtube_id: str, db: Session = Depends(get_db)):
    ...
    if когда and time.time() - когда < PREVIEW_RETRY_SEC:
        ...
        данные = await _сеть(db, _скачать_превью(youtube_id))
...
def по_полу(пол, мужской, женский, нейтрально):
    """Слово, согласованное с полом человека. Пол не указан —
    НЕЙТРАЛЬНАЯ формулировка, и это законное состояние, а не «ещё
    не заполнил» (BACKLOG №215, G.6).

    ОДНО ПРАВИЛО НА ВЕСЬ ПРОЕКТ. До 2026-09-02 согласование жило
    одним словарём в аптечке, и второе такое же место (причина
    пропуска тренировки) завело бы второй экземпляр правила,
    расходящийся молча (§6.0.7). Слова у мест РАЗНЫЕ — общее
    у них не слово, а РЕШЕНИЕ: чей род, и что делать, когда род
    неизвестен.

    УМОЛЧАНИЕ НЕЙТРАЛЬНОЕ И ЭТО ВАЖНО: место, забывшее передать пол,
    скажет нейтрально, а не назовёт человека мужчиной. Ошибка в эту
    сторону незаметна, в обратную — обращение не к тому человеку.
    """
    if пол == "m":
        return мужской
...
def _апт_подпись_приёма(поз, пол=None):
    ...
    глагол = по_полу(пол, АПТ_ГЛАГОЛ_ПРИЁМА["m"],
                     АПТ_ГЛАГОЛ_ПРИЁМА["f"], АПТ_ГЛАГОЛ_ПРИЁМА[None])
    return "%s %s %s" % (глагол, _апт_опр.число(доза), слово)


def _апт_остаток_строкой(поз):
    """«Таблетки · 18 из 20», «Капли · 348 из 350 кап · во флаконе 10 мл».

    ОБЪЁМ ПРИПИСЫВАЕТСЯ ХВОСТОМ И ОТДЕЛЬНЫМ ЗВЕНОМ (BACKLOG №238, A.1).
    Он не участвует в счёте, и на экране это обязано быть видно: «348
    из 350 кап» — то, что убывает от кнопки, «во флаконе 10 мл» — то,
    что не двигается никогда. Слепи мы их в одно число, вернулась бы
    ровно та неправда, ради которой поле и заводилось.

    Собирается ЗДЕСЬ, а не в шаблоне: строку показывают карточка, окно
    упаковок и сводка группы, и три сборки разошлись бы молча.
    """
    форма = _апт_опр.ФОРМА_ПО_ID.get(поз.form)
    имя_формы = форма["имя"] if форма else поз.form
    объём = _апт_опр.объём_словами(поз.volume, поз.volume_unit)
    # ОБЪЁМ НЕ ПОКАЗЫВАЕТСЯ, КОГДА ОН ПОВТОРЯЕТ ОСТАТОК. Нашёл это
    # ЗАМЕР строк на живой странице, а не чтение: «Эмульсия · 80 из
    # 100 мл · во флаконе 100 мл» — вторая половина не добавляет
    # ничего, потому что остаток уже считается в миллилитрах.
    #
    # Приписка нужна ровно там, где единицы РАЗНЫЕ: «348 из 350 кап ·
    # во флаконе 10 мл» — два разных факта. У форм со шкалой она нужна
    # ВСЕГДА: числа там нет вовсе, и «половина» без объёма не говорит,
    # половина чего.
    дубль = (форма and форма.get("счёт") == "точный"
             and поз.unit == поз.volume_unit)
    приписка = (" · во флаконе %s" % объём) if (объём and not дубль) else ""
    if форма and форма["счёт"] == "шкала":
        ш = _апт_опр.ШКАЛА_ПО_ID.get(поз.scale)
        начало = ("%s · %s" % (имя_формы, ш["имя"].lower())) if ш else имя_формы
        return начало + приписка
    ...
    if поз.qty_left is None:
        return имя_формы + приписка
    if поз.qty_total:
        return "%s · %s из %s%s%s" % (имя_формы, _апт_опр.число(поз.qty_left),
                                      _апт_опр.число(поз.qty_total), хвост,
                                      приписка)
    return "%s · %s%s%s" % (имя_формы, _апт_опр.число(поз.qty_left), хвост,
                            приписка)
...
def _апт_наружу(поз, сегодня, места_по_id, кат_по_позиции, кат_имена=None,
    ...
    return {
        ...
        "dose": поз.dose,
        # ОБЪЁМ УПАКОВКИ — ОТДЕЛЬНОЙ СТРОКОЙ И БЕЗ СВЯЗИ СО СЧЁТОМ
        # (BACKLOG №238, A.1). Готовой строкой, а не парой чисел:
        # склеивать её в шаблоне значило бы завести второе место,
        # где известно, как называется единица объёма
        "volume": поз.volume,
        "volume_unit": поз.volume_unit or "",
        "объём_словами": _апт_опр.объём_словами(поз.volume, поз.volume_unit),
        ...
        #
        # «Источник: Видаль» говорит, откуда ТЕКСТ. Оно не отвечает
        # на вопрос, почему мы уверены, что страница про ЭТУ пачку, —
        # а вся цена ошибки здесь именно в подмене выпуска: чужая
        # схема хуже её отсутствия.
        #
        # Считает СЕРВЕР и по тем же полям, по которым шло само
        # сопоставление (`medkit_dosage.подходит`). Вещество названо
        # только там, где оно у нас заполнено: у пустого поля сверять
        # было нечем, и написать «сверено по веществу» значило бы
        # соврать ровно в том месте, ради которого строка и заводится.
        "сверено": (["названию", "форме выпуска"]
                    + (["действующему веществу"]
                       if (поз.substance or "").strip() else [])),
        # МЕТКА НАЗЫВАЕТ ДОМЕН, А НЕ НАШЕ СЛОВО (BACKLOG №207, E.1).
        # Источников стало два, и человек обязан видеть, чей текст
        # читает. В базе при этом лежит ИМЯ источника, включая
        # записи, снятые до появления второго: перевод делает
        # `_апт_ист.по_имени`, и старые строки читаются как прежде
        "дозы_источник": _апт_домен_источника(поз.dosage_source),
        ...
        #
        # Та же тройка, что у схемы: справочник — своё — заметка врача,
        # и метка источника на экране обязательна. Участвует в группах
        # ответа ассистента НАРАВНЕ со справочными показаниями: позиция
        # без «от чего» в группу не попадает по построению, а вкладыш
        # у человека в руках есть.
        "своё_от_чего": (поз.own_indications_text or "").strip(),
        "своё_от_чего_дата": (
            _апт_дата_словом(поз.own_indications_at.date().isoformat())
            if поз.own_indications_at else ""),
        ...
        # ОТКУДА ВЗЯЛОСЬ ВЕЩЕСТВО — код и готовая подпись (№234, A.3).
        #
        # ПОДПИСЬ СОБИРАЕТ СЕРВЕР, а не браузер: второй словарь имён
        # источников разошёлся бы с первым молча, и на карточке встало
        # бы «прочитано с упаковки» там, где код говорит другое
        # (§6.0.7). Пустое вещество источника не имеет по построению,
        # и подписи у него нет — говорить не о чем.
        "вещество_откуда": (поз.substance_src or "") if поз.substance else "",
        "вещество_откуда_подпись": (
            _апт_опр.источник_вещества(поз.substance_src) or ""
            if поз.substance else ""),
        # ── ЧТО СПРАВОЧНИК ГОВОРИТ О ФОРМЕ (BACKLOG №239, блок B) ────
        #
        # ТРИ ПОЛЯ, И КАЖДОЕ ОТВЕЧАЕТ НА СВОЁ:
        #   `форма_подсказка`       — КОД нашей формы, если он выведен
        #                             однозначно. Пусто — предложить
        #                             нечего, выбирать за человека
        #                             мы не будем
        #   `форма_подсказка_слово` — СЛОВО справочника ДОСЛОВНО.
        #                             Есть и тогда, когда кода нет:
        #                             показать сказанное источником
        #                             можно всегда
        #   `форма_подсказка_подпись` — как ЭТА форма зовётся у нас,
        #                             чтобы человек видел, что
        #                             подставится, ДО нажатия (B.2)
        "форма_подсказка": (поз.dosage_hint_form or "").strip(),
        "форма_подсказка_слово": (поз.dosage_hint_form_word or "").strip(),
        "форма_подсказка_подпись": (
            (_апт_опр.ФОРМА_ПО_ID.get(поз.dosage_hint_form) or {}).get("имя", "")
            if поз.dosage_hint_form else ""),
        # ── РАЗЛИЧАЕТСЯ ЛИ ПОДСКАЗКА С ТЕМ, ЧТО ЗАПИСАНО ────────────
        #
        # BACKLOG №244, A.5. Решает СЕРВЕР, и это не удобство:
        # у него уже есть `medkit_defs.нормаль` — то самое приведение,
        # которым сравниваются строки во всём разделе. Второе мнение
        # в браузере пришлось бы держать в согласии с первым вручную,
        # и разошлись бы они молча (§6.0.7).
        #
        # ЗАМЕР, ИЗ-ЗА КОТОРОГО ЭТО ЗАВЕДЕНО (боевая база, 2026-09-07):
        # подсказка формы стояла у 15 позиций, а код формы РАСХОДИЛСЯ
        # ровно у ОДНОЙ — то есть в четырнадцати случаях экран
        # предлагал заменить «Таблетки» на «Таблетки». Причина была
        # не в сравнении, а в том, что сравнивать было НЕ С ЧЕМ:
        # браузер читал ключи `вещество` и `форма`, которых в этом
        # ответе нет вовсе (есть `substance` и `form`), получал пустую
        # строку и честно находил различие с ней.
        "вещество_подсказка_иначе": _апт_подсказка_иначе(поз)[0],
        "форма_подсказка_иначе": _апт_подсказка_иначе(поз)[1],
        # ОТКУДА ВЗЯЛАСЬ ФОРМА — тем же словарём, что у вещества
        "форма_откуда": (поз.form_src or ""),
        "форма_откуда_подпись": (
            _апт_опр.источник_вещества(поз.form_src) or ""),
        ...
        # ── ПО ЧЕМУ ИМЕННО СОВПАЛО (BACKLOG №244, E.2) ──────────────
        #
        # Отдельными полями, потому что ПОРЯДОК выдачи зависит от того,
        # где нашлось: название весит больше вещества, вещество больше
        # категории. Одной склеенной строкой этого не выразить —
        # в ней все поля равны.
        #
        # ЗАМЕР, ИЗ-ЗА КОТОРОГО ЭТО ЗАВЕДЕНО (боевая база, 2026-09-07):
        # запрос «парацетамол» давал 3 совпадения — 1 в названии
        # и 2 только в веществе, — а шли они в порядке сетки, то есть
        # по сроку годности. Сам препарат оказывался третьим.
        # Запрос из одной буквы «п» давал 50 карточек из 77 при двух,
        # чьё название с неё начинается: человек, помнивший «средство
        # на П», получал полсписка и не находил ничего.
        #
        # СЕРВЕР ОТДАЁТ ПОЛЯ, А СТУПЕНЬ СЧИТАЕТ ЭКРАН — и это не
        # второе мнение: перечень полей, по которым ищем, по-прежнему
        # живёт ЗДЕСЬ, браузер лишь сравнивает готовые строки.
        "искать_имя": _апт_опр.нормаль(поз.name or ""),
        "искать_вещество": _апт_опр.нормаль(поз.substance or ""),
    }
...
def _апт_проверить(данные, db, user):
    ...
    # ── ОТКУДА ВЗЯЛОСЬ ВЕЩЕСТВО (BACKLOG №234, A.3) ──────────────────
    #
    # БЕЛЫЙ СПИСОК, А НЕ ДОВЕРИЕ КЛИЕНТУ. Незнакомый код даёт None —
    # «неизвестно»; принять его значило бы показать на карточке
    # подпись «прочитано с упаковки» по слову запроса, собранного мимо
    # экрана. Пустое вещество источника не имеет по построению.
    поля["substance_src"] = (
        строка("substance_src", 20)
        if (поля["substance"]
            and строка("substance_src", 20) in _апт_опр.ИСТОЧНИК_ВЕЩЕСТВА)
        else None)
    ...
    # ── ОБЪЁМ УПАКОВКИ (BACKLOG №238, A.1) ──────────────────────────
    #
    # СПРАВОЧНОЕ ПОЛЕ, В СЧЁТЕ НЕ УЧАСТВУЕТ. «Во флаконе 10 мл» рядом
    # с «350 капель»: пересчёта между ними нет и не заводится — разбор
    # в шапке `medkit_defs.ЕДИНИЦЫ_ОБЪЁМА`.
    #
    # ЕДИНИЦА БЕЗ ЧИСЛА И ЧИСЛО БЕЗ ЕДИНИЦЫ НЕ ЗАПИСЫВАЮТСЯ ОБА:
    # «10» без «мл» на карточке — не объём, а загадка, а «мл» без числа
    # не говорит ничего. Пара пишется целиком либо не пишется вовсе.
    объём, ош = _апт_число(данные.get("volume"), "объём упаковки",
                           минимум=0.01, обязательно=False)
    if ош:
        return None, ош
    ед_объёма = строка("volume_unit", 4) or None
    if ед_объёма and ед_объёма not in _апт_опр.ЕДИНИЦА_ОБЪЁМА_ПО_ID:
        return None, ("Объём упаковки измеряется в %s"
                      % " или ".join(и for _, и in _апт_опр.ЕДИНИЦЫ_ОБЪЁМА))
    if объём is not None and not ед_объёма:
        return None, "У объёма упаковки не выбрана единица — мл или г"
    поля["volume"] = объём if ед_объёма else None
    поля["volume_unit"] = ед_объёма if объём is not None else None
...
# ═══════════════════════════════════════════════════════════════════
# ПОТОЛОК КЕША — В МЕГАБАЙТАХ, А НЕ ТОЛЬКО В ЗАПИСЯХ (BACKLOG №241)
# ═══════════════════════════════════════════════════════════════════
#
# БОЕВОЙ ДЕФЕКТ: обход всей аптечки УБИВАЛ ПРОД. Ядро печатало
# «Out of memory: Killed process», машина уходила в перезагрузку,
# а обход в браузере вставал молча посреди двадцати четырёх позиций.
#
# ПОТОЛОК В ЗАПИСЯХ БЕЗОПАСЕН РОВНО ПОКА ЗАПИСИ ОДНОГО ВЕСА, и до
# второго источника так и было. Замер 2026-09-04 С БОЕВОЙ МАШИНЫ,
# размер страницы В ПАМЯТИ:
#
#     Видаль  выдача    124 КБ      второй  выдача    849 КБ
#     Видаль  карточка  663 КБ      второй  карточка 2480 КБ
#
# Домены тут НЕ НАЗВАНЫ намеренно: адрес чужого сайта на месте вызова
# нельзя ни грепнуть, ни поменять решением владельца, и это сторожит
# тест. Речь здесь про ИСТОЧНИКИ, а не про их адреса
#
# Карточка второго источника ВЧЕТВЕРО тяжелее. Полный кеш из 400
# записей весил 154 МБ на одном Видале и стал весить 402 МБ на двух
# источниках — при 212 МБ, которые машина имеет ВСЕГО.
#
# Отсюда арифметика, и она сошлась с местом падения:
#
#     только Видаль   память кончается на 478-й странице → УБОРКА
#                     на 400-й успевает раньше, дефекта НЕТ
#     два источника   память кончается на 183-й          → УБОРКА
#                     не успевает ВДВОЕ, процесс убит
#
# Обход владельца прошёл 18 позиций (~180 страниц) и умер на 19-й.
#
# ЧИСЛО ВЫВЕДЕНО ЗАМЕРОМ, А НЕ НАЗНАЧЕНО, И ПЕРЕМЕРЕНО 2026-09-05
# ПОСЛЕ ПОДЪЁМА МАШИНЫ 256 -> 512 МБ (BACKLOG №242).
#
# Замер `/proc` на проде, кеш пуст: MemTotal 469 892 kB = 458.9 МБ
# (было 212 228 kB = 207.3), uvicorn 107.4 МБ, hallpass и init 24.9.
# СВОБОДНО 326.6 МБ против прежних 75.
#
# ПОЧЕМУ 96, А НЕ ПРЕЖНИЕ 24. Замер «сколько держит процесс при
# налитом до потолка кеше», страницы боевого веса (2.42 МБ):
#
#     потолок  24 МБ ->  9 страниц, процесс держит 108 МБ
#     потолок  96 МБ -> 39 страниц, процесс держит 181 МБ
#     потолок 144 МБ -> 59 страниц, процесс держит 230 МБ
#
# ДЕВЯТИ СТРАНИЦ НЕ ХВАТАЕТ ДАЖЕ НА ОДНУ ПОЗИЦИЮ: запросов на неё
# до шестнадцати — две выдачи и до шести карточек у каждого из двух
# источников, — то есть при 24 МБ кеш вытеснял сам себя ВНУТРИ
# позиции, и выдача, нужная второму имени поиска, уходила в сеть
# заново. При 96 в кеш помещается полный обход аптечки (116 страниц
# на 24 позиции весили 82.6 МБ — замер 2026-09-04).
#
# ЗАПАС СЧИТАЕТСЯ ВМЕСТЕ С РАЗБОРОМ ФОТО, и это не перестраховка:
# снимок могут прислать во время обхода, а пик разбора 50 Мпикс —
# 97 МБ (замер §5.8). Занято 107.4 + 24.9 + 96 + 97 = 325.3
# из 458.9, запас 133.6 МБ.
#
# НА ПРЕЖНЕЙ МАШИНЕ ЭТА СУММА НЕ ВЛЕЗАЛА ВОВСЕ — и это ответ на
# вопрос, зачем поднимали: даже со СТАРЫМ потолком 24 обход плюс
# одновременный разбор фото требовали 253.3 МБ при 207.3 доступных.
# Потолок кеша спасал от OOM ровно до первого снимка.
#
# ПОЧЕМУ НЕ ОТКАЗАТЬСЯ ОТ КЕША ВОВСЕ. В пределах ОДНОЙ позиции
# страница запрашивается дважды: имён поиска бывает два, и выдача
# у них общая. Плюс повторное открытие карточки человеком не должно
# ходить в чужую сеть заново.
#
# ПЕРЕОПРЕДЕЛЯЕТСЯ ОКРУЖЕНИЕМ, и это не украшение: мерки
# (`check_medkit_all`, `check_medkit_manual`) гоняются на машине
# разработчика, где памяти на порядок больше, и им кеш нужен ЦЕЛИКОМ —
# два прогона обязаны видеть одну выдачу (§5.8).
REF_CACHE_MB = float(os.getenv("MEDKIT_REF_CACHE_MB", "96"))
# Сколько помнить, что источник не ответил на базовую линию.
# Короче общего срока: разбор — у `_апт_базовая_линия`
БАЗА_ОТКАЗ_TTL = int(os.getenv("MEDKIT_BASE_FAIL_TTL", "300"))
_реф_кеш = {}
...
# БАЗОВАЯ ЛИНИЯ ИСТОЧНИКА, снятая один раз на процесс (BACKLOG №207).
# Кешируется тем же сроком, что и страницы: три лишних запроса
# на КАЖДУЮ позицию обхода — это 150 обращений вместо трёх, а набор
# за час не меняется (замер: три выдуманных имени дают один и тот же
# ответ подряд)
_реф_база_кеш = {}
...
async def _апт_страница(клиент, адрес, хост=None):
    ...
    # СЧЁТ ПО ФАКТИЧЕСКОМУ ХОСТУ, а не по имени первого источника
    # (BACKLOG №207): источников стало два, и запись всех обращений
    # на «Видаль» сделала бы счётчик неправдой ровно в том месте,
    # по которому принимают решение о потолке (E.2 задачи 201)
    _реф_счётчик(хост or _апт_дозы.ИСТОЧНИК_ИМЯ)
...
def _реф_кеш_вес():
    """Сколько БАЙТ занимают лежащие в кеше страницы."""
    return sum(sys.getsizeof(v[1]) for v in _реф_кеш.values())


def _реф_кеш_подрезать():
    """Вытеснить СТАРЕЙШИЕ, пока кеш не влезет в оба потолка.

    ПО ОДНОМУ, А НЕ `clear()`, и это отмена прежнего решения.
    Здесь стояло «проще и честнее, чем вытеснять по одному» — верно
    ровно пока потолок считался в ЗАПИСЯХ и срабатывал раз в четыреста
    страниц. Потолок в 24 МБ упирается каждые ~десять страниц wer.ru,
    и `clear()` выбрасывал бы выдачу, по которой мы прямо сейчас
    идём: в пределах одной позиции страница запрашивается дважды.

    СТАРЕЙШАЯ — ПЕРВАЯ ПО ПОРЯДКУ ВСТАВКИ: словарь Python его хранит,
    заводить рядом вторую очередь незачем.
    """
    if not _реф_кеш:
        return
    потолок = int(REF_CACHE_MB * 1024 * 1024)
    вес = _реф_кеш_вес()
    while _реф_кеш and (вес > потолок or len(_реф_кеш) > REF_CACHE_MAX):
        старейший = next(iter(_реф_кеш))
        вес -= sys.getsizeof(_реф_кеш.pop(старейший)[1])
...
# ПОЗИЦИЯ НЕ УЛОЖИЛАСЬ В СВОЙ ПОТОЛОК ВРЕМЕНИ (BACKLOG №241, блок C).
# Повтор ПОМОЖЕТ: источник, отвечавший минуту назад по полминуты,
# через час отвечает за секунду — замер 2026-09-04 дал у одного
# и того же адреса 0.9 с утром и 54.3 с днём.
ДОЗЫ_ВИД_ВРЕМЯ = "время"
...
# ═══════════════════════════════════════════════════════════════════
# ПОТОЛОК ВРЕМЕНИ НА ОДНУ ПОЗИЦИЮ (BACKLOG №241, блок C)
# ═══════════════════════════════════════════════════════════════════
#
# Потолок был у КАЖДОГО ЗАПРОСА (20 с у Видаля, 90 с у wer.ru) и НЕ БЫЛ
# у позиции целиком. Запросов на позицию до шестнадцати — две выдачи
# и до шести карточек у каждого из двух источников, — то есть позиция
# имела право идти до семнадцати минут, ничего об этом не сказав.
#
# ЧТО ЭТО ДАВАЛО ЧЕЛОВЕКУ. Обход в браузере ждёт ответа сервера
# `fetch`-ем, у которого своего таймаута НЕТ ВОВСЕ. Плашка «Смотрю
# 19 из 24» стояла и не двигалась — ни хода, ни ошибки, ни отказа.
# Немой отказ в чистом виде (§6.0): экран исправен, а работа стоит.
#
# ЧИСЛО ВЫВЕДЕНО ИЗ ЗАМЕРА. Позиции на проде 2026-09-04 шли
# 0.9–4.3 с, на копии 1.0–6.8 с — то есть шестьдесят секунд это
# ДЕСЯТИКРАТНЫЙ запас к худшему наблюдённому. Верхний край
# наблюдённого разброса у второго источника — 54.3 с на ОДНУ выдачу,
# и позиция, которой хватает одного такого запроса, ещё успевает.
#
# ЦЕНА НАЗВАНА: позиция, где медленны ДВА запроса подряд, будет
# пропущена, хотя схема у неё, возможно, нашлась бы. Ошибка тут
# в безопасную сторону — пропуск НАЗЫВАЕТСЯ на экране и чинится
# повтором, тогда как молчаливое ожидание не чинится ничем.
ДОЗЫ_ПОТОЛОК_ПОЗИЦИИ = float(os.getenv("MEDKIT_DOSAGE_ITEM_TIMEOUT", "60"))


# ═════════════════════════════════════════════════════════════════════
# ОБХОД ИСТОЧНИКОВ: ПОРЯДОК ЖЁСТКИЙ, ВТОРОЙ — ТОЛЬКО ЕСЛИ ПЕРВЫЙ МОЛЧИТ
# ═════════════════════════════════════════════════════════════════════
#
# BACKLOG №207, правило D.1. Порядок объявлен в `medkit_sources.ПОРЯДОК`
# и здесь не повторяется: два места молча разошлись бы (§6.0.7).
#
# ═════════════════════════════════════════════════════════════════════
# РАСХОЖДЕНИЯ МЕЖДУ ИСТОЧНИКАМИ НЕ НАСТУПАЕТ ПО ПОСТРОЕНИЮ
# ═════════════════════════════════════════════════════════════════════
#
# Постановка спрашивала, что делать, когда Видаль сказал одно, а wer.ru
# другое. ОТВЕТ: этого случая нет, и не потому, что он безопасен, —
# второй источник СПРАШИВАЕТСЯ ТОЛЬКО ТОГДА, когда первый не дал схемы.
# Сравнивать нечего: у нас всегда ровно один ответ.
#
...
# выбрать`). Замер, сколько позиций дали бы РАЗНЫЕ схемы, спроси мы
# обоих, печатает `check_wer_gain.py --расхождения`: это свойство
# чужих сайтов, и на боевом пути оно не проявляется.
#
# ЦЕНА НАЗВАНА: у позиции, чью схему Видаль знает неверно, wer.ru
# не спросится никогда. Это осознанный размен — Видаль справочник
# лекарственных средств, wer.ru аптечный каталог с перепечаткой,
# и при разногласии вернее первый.


def _апт_ист_по_адресу(адрес):
    """Какой источник дал эту страницу — по её собственному адресу.

    BACKLOG №207. Второго мнения не заводится: адрес и текст приезжают
    ОДНИМ кортежем из одного источника, и спрашивать «а кто это был»
    у отдельной переменной значило бы завести величину, способную
    разойтись с фактом (§6.0.7).

    Незнакомый адрес даёт первый источник порядка, и это осознанно:
    так читаются записи, снятые до появления второго.
    """
    for и in _апт_ист.ПОРЯДОК:
        if (адрес or "").startswith(и.база):
            return и
    return _апт_ист.ПОРЯДОК[0]


def _апт_домен_источника(имя):
    """Имя источника из базы → ДОМЕН для метки на экране (E.1).

    Незнакомое имя отдаётся КАК ЕСТЬ, а не подменяется первым
    источником: в базе может лежать метка, снятая версией, о которой
    этот код не знает, и выдать её за домен первого значило бы соврать
    ровно в том месте, ради которого метка и заводится.

    ЛИТЕРАЛА ДОМЕНА ЗДЕСЬ НЕТ, И ЭТО СТОРОЖИТ ТЕСТ: адрес чужого сайта
    на месте вызова нельзя ни грепнуть, ни поменять решением владельца.
    Домены объявлены в `medkit_sources`, по одному на переходник.
    """
    if not имя:
        return ""
    и = _апт_ист.по_имени(имя)
    return и.домен if и else имя


# ═════════════════════════════════════════════════════════════════════
# КЛИЕНТ НА ИСТОЧНИК, А НЕ НА ПОЗИЦИЮ: TLS-РУКОПОЖАТИЕ СТОИТ ДОРОГО
# ═════════════════════════════════════════════════════════════════════
#
# ЗАМЕР 2026-09-04 НА ПРОДЕ, и он опровергает вывод, который напрашивался
# весь заход. «Источник медленный» — НЕПРАВДА: пофазный разбор показал,
# что DNS занимает 0.14 с, TCP 0.01 с, а РУКОПОЖАТИЕ TLS — от 2.8
# до 29.4 с. У Видаля на том же прогоне 29.44 с.
#
# ПРИЧИНА НЕ В ЧУЖОМ САЙТЕ, А В НАШЕЙ МАШИНЕ: у неё ОДИН vCPU
# (`os.cpu_count()` = 1), рукопожатие это криптография, и под нагрузкой
# приложения оно ползёт. Тот же запрос из ЛЁГКОГО процесса, без импорта
# приложения, приходит за 0.2–0.6 с; после `import main` — за 43.8
# и 96.6 с.
#
# ОТСЮДА КЛИЕНТ ЖИВЁТ В ПРОЦЕССЕ. Свой клиент на каждый вызов означал
# бы новое рукопожатие на КАЖДУЮ позицию: обход аптечки из 49 позиций
# по двум источникам — до 98 рукопожатий по 3–30 с, то есть десятки
# минут ТОЛЬКО на криптографию. С переиспользованием их два на весь
# обход, дальше keep-alive.
#
# ЗАКРЫВАТЬ ЕГО НЕКОМУ, И ЭТО НЕ УТЕЧКА: клиентов ровно столько,
# сколько источников (два), они держат по одному соединению
# и живут столько же, сколько ЦИКЛ СОБЫТИЙ. Ровно так же устроен кеш
# страниц рядом.
#
# ═════════════════════════════════════════════════════════════════════
# КЛЮЧ ВКЛЮЧАЕТ ЦИКЛ СОБЫТИЙ, И НАШЁЛ ЭТО ЗАМЕР, А НЕ ЧТЕНИЕ
# ═════════════════════════════════════════════════════════════════════
#
# Первая версия держала клиента ПО ДОМЕНУ, на процесс. На бою это
# верно — у приложения один цикл на всё время жизни. А МЕРКИ зовут
# `asyncio.run()` на КАЖДУЮ позицию, то есть цикл у них каждый раз
# новый, и клиент из прошлого цикла мёртв.
#
# ЗАМЕР ПО ВСЕЙ АПТЕЧКЕ: нашлось 18 из 49 против 25 до подключения
# второго источника — то есть «улучшение» оказалось РЕГРЕССОМ,
# и в журнале 39 строк `RuntimeError('Event loop is closed')`.
# Ни ошибки наружу, ни признака: отказ приходит в тот же `except`,
# что и сетевой, и читается как «источник не ответил».
_апт_клиенты = {}


def _апт_клиент(источник):
    """Живой HTTP-клиент этого источника — на источник И на цикл.

    Мёртвые записи чужих циклов убираются здесь же: их не может быть
    много (мерка создаёт цикл на позицию), но копиться им незачем.
    """
    # КЛЮЧОМ — САМ ЦИКЛ, А НЕ ЕГО `id()`. CPython переиспользует адреса
    # памяти: у закрытого и вновь созданного цикла `id()` СОВПАДАЕТ,
    # и клиент из мёртвого цикла числился бы живым. Поймал это тест,
    # а не чтение — он для того и написан
    try:
        цикл = asyncio.get_running_loop()
    except RuntimeError:
        цикл = None
    ключ = (источник.домен, цикл)
    кл = _апт_клиенты.get(ключ)
    if кл is None or кл.is_closed or (цикл is not None and цикл.is_closed()):
        кл = httpx.AsyncClient(
            timeout=getattr(источник, "таймаут", ДОЗЫ_ТАЙМАУТ),
            follow_redirects=True, verify=False)
        # МЁРТВЫЕ ЦИКЛЫ УБИРАЮТСЯ ЗДЕСЬ ЖЕ: словарь держит ссылку
        # на объект цикла, и без уборки он копился бы с каждой позицией
        # прогона мерки
        for старый in [к for к in _апт_клиенты
                       if к[1] is not цикл
                       and (к[1] is None or к[1].is_closed())]:
            _апт_клиенты.pop(старый, None)
        _апт_клиенты[ключ] = кл
    return кл


async def _апт_базовая_линия(кл, источник):
    """АДРЕСА, КОТОРЫЕ ИСТОЧНИК ОТДАЁТ НА ЛЮБОЙ ЗАПРОС.

    BACKLOG №207, блок A. wer.ru на выдуманное имя отвечает HTTP 200
    и ДВУМЯ карточками, причём на обеих стоит настоящий раздел «Способ
    применения, курс и дозировка» (замер 2026-09-04). Без вычитания
    выдуманное название показало бы человеку схему приёма ЧУЖОГО
    препарата — а признака ошибки не было бы никакого.
    ...
    а не про сайт. Замер: все три дают ОДИН И ТОТ ЖЕ набор из двух
    адресов.

    НЕ ОТВЕТИЛ НИ ОДИН — отдаём `None`, и вызывающий обязан
    ОСТАНОВИТЬСЯ. Пустая базовая линия при живом источнике означала бы
    «вычитать нечего», то есть ровно ту дыру, ради которой всё это
    и написано (§6.0.1: «спросить не удалось» и «ничего нет» — разные
    факты).
    """
    if not источник.базовая_линия_нужна:
        return frozenset()
    попал = _реф_база_кеш.get(источник.домен)
    if попал and time.time() - попал[0] < REF_CACHE_TTL:
        # `None` в кеше — ПОМНИМ ОТКАЗ, а не «вычитать нечего».
        # Разница не косметическая: пустое множество означало бы
        # «источник ответил, реклама не показана», и мы пошли бы
        # спрашивать его без вычитания (блок A)
        return попал[1]
    набор, ответили, прошлый = set(), 0, None
    for бред in _апт_дозы.КОНТРОЛЬ_ВЫДУМАННЫЕ:
        try:
            выдача = await _апт_страница(кл, источник.адрес_поиска(бред),
                                         источник.домен)
        except ПределСправочника:
            raise
        except Exception as e:
            print("[medkit-doses] базовая линия %s: %r не ответил: %r"
                  % (источник.домен, бред, e))
            continue
        ответили += 1
        свой = frozenset(источник.карточки(выдача))
        набор |= свой
        # ДВА СОВПАВШИХ ОТВЕТА — ДОВОЛЬНО, И ЭТО НЕ ЭКОНОМИЯ РАДИ
        # ЭКОНОМИИ. Спрашиваем мы РАЗНЫМИ выдуманными именами затем,
        # чтобы убедиться: набор один и тот же, то есть он про САЙТ,
        # а не про конкретную бессмыслицу. Два совпавших это уже
        # показали; третий ответ ничего не добавит, а стоит он
        # до 90 с — столько же, сколько сама выдача (замер: 54.3 с
        # с прода в тот же день, когда утром она приходила за секунды)
        if прошлый is not None and свой == прошлый:
            break
        прошлый = свой
    if not ответили:
        # ОТКАЗ КЕШИРУЕТСЯ ТОЖЕ, И ЭТО НАШЁЛ ПРОГОН, А НЕ ЧТЕНИЕ.
        #
        # Первая версия кешировала только УДАЧУ, и обход аптечки
        # с недоступным источником вставал намертво: три попытки
        # по таймауту НА КАЖДУЮ позицию — при 49 позициях это 147
        # ожиданий вместо трёх. Замер: прогон не уложился в 600 с
        # и ушёл в фон.
        #
        # СРОК КОРОТКИЙ И СВОЙ: источник, лежавший минуту назад, мог
        # подняться, и держать отказ час значило бы выключить источник
        # на час из-за одной сетевой заминки. Пять минут — столько,
        # чтобы один обход аптечки не платил за отказ дважды.
        _реф_база_кеш[источник.домен] = (
            time.time() - REF_CACHE_TTL + БАЗА_ОТКАЗ_TTL, None)
        return None
    готово = frozenset(набор)
    _реф_база_кеш[источник.домен] = (time.time(), готово)
    print("[medkit-doses] базовая линия %s: %d адресов (опрошено %d имён)"
          % (источник.домен, len(готово), ответили))
    return готово


async def _апт_найти_дозы(поз):
    """Схема приёма ЭТОГО препарата — С ПОТОЛКОМ ВРЕМЕНИ НА ПОЗИЦИЮ.

    Потолок стоит ЗДЕСЬ, а не на месте вызова, и это то же правило,
    что у всего остального в этом файле: потребителей три — фон при
    заведении, фон при правке и кнопка окна, — и место, где его
    забыли бы поставить, ничем не отличалось бы от места, где он
    не нужен (§6.0.7).

    ОТМЕНА ЗАДАЧИ ЗАКРЫВАЕТ И ЗАПРОС: `wait_for` отменяет корутину,
    httpx на отмене закрывает соединение сам. Клиент при этом живёт
    дальше — он общий на источник и на цикл (`_апт_клиент`).
    """
    try:
        return await asyncio.wait_for(_апт_найти_дозы_обходом(поз),
                                      timeout=ДОЗЫ_ПОТОЛОК_ПОЗИЦИИ)
    except asyncio.TimeoutError:
        print("[medkit-doses] позиция пропущена по времени: дольше %.0f с"
              % ДОЗЫ_ПОТОЛОК_ПОЗИЦИИ)
        # БЕЗ СКЛОНЕНИЯ ЧИСЛИТЕЛЬНОГО: «за 3 секунд» не по-русски,
        # а второй набор правил русского счёта рядом с тем, что лежит
        # в `medkit_defs`, здесь заводить незачем — тот же довод, что
        # у текста предела обращений выше
        return None, ("справочник не ответил за %.0f с — эта позиция "
                      "пропущена. Попробуйте ещё раз позже: источник "
                      "отвечает неровно." % ДОЗЫ_ПОТОЛОК_ПОЗИЦИИ
                      ), ДОЗЫ_ВИД_ВРЕМЯ, ""


async def _апт_найти_дозы_обходом(поз):
    """Обход источников по порядку — БЕЗ потолка, его ставит вызывающий.

    Возвращает то же, что и раньше: `(итог, причина, вид, подсказка)`.
    Первый источник, давший схему, выигрывает; следующий спрашивается,
    ТОЛЬКО если предыдущий не дал.

    ОТКАЗ ОТДАЁТСЯ ОТ ПЕРВОГО ИСТОЧНИКА, а не от последнего, и это
    решение: причина «в справочнике есть N, но других выпусков» от
    Видаля выполнима — там перечислены настоящие выпуски; та же
    причина от аптечного каталога говорила бы про ассортимент магазина.
    Исключение одно и названо ниже: если первый не ответил по СЕТИ,
    а второй ответил внятно, человеку полезнее внятное.
    """
    первый_отказ = None
    for источник in _апт_ист.ПОРЯДОК:
        # КЛИЕНТ ПЕРЕИСПОЛЬЗУЕТСЯ МЕЖДУ ПОЗИЦИЯМИ — разбор и замер
        # у `_апт_клиент`. Своим он был до 2026-09-04, и обход аптечки
        # платил за рукопожатие TLS на КАЖДОЙ позиции
        кл = _апт_клиент(источник)
        try:
            база = await _апт_базовая_линия(кл, источник)
        except ПределСправочника as e:
            return None, ("%s. Схема приёма в этом месяце больше "
                          "не ищется; впишите её с упаковки." % e
                          ), ДОЗЫ_ВИД_ПРЕДЕЛ, ""
        if база is None:
            # БАЗОВАЯ ЛИНИЯ НЕ СНЯЛАСЬ — ИСТОЧНИК ПРОПУСКАЕТСЯ
            # ЦЕЛИКОМ. Спросить его без вычитания значило бы
            # рискнуть показать схему чужого препарата ради одной
            # позиции (блок A), и «источник не ответил» тут честнее
            print("[medkit-doses] %s пропущен: базовая линия не снялась"
                  % источник.домен)
            continue
        итог, причина, вид, подсказка = await _апт_искать_в_источнике(
            кл, поз, источник, база)
        if итог:
            return итог, причина, вид, подсказка
        if первый_отказ is None or (первый_отказ[2] == ДОЗЫ_ВИД_СЕТЬ
                                    and вид != ДОЗЫ_ВИД_СЕТЬ):
            первый_отказ = (итог, причина, вид, подсказка)
    return первый_отказ or (None, "по названию этой позиции искать нечего",
                            ДОЗЫ_ВИД_НЕЧЕГО_ИСКАТЬ, "")


def _апт_чем_отличается(страница_html, источник=None):
    """ЧТО ЗА ВЫПУСК ОПИСАН НА ЭТОЙ СТРАНИЦЕ — короткой строкой человеку.

    У Видаля описание устроено как «… – описание Citramon таблетки
    100 мг: 10, 15 шт. (44740)», и хвост после слова «описание» и есть
    ответ; разбор — `medkit_dosage.чем_отличается`.

    У wer.ru такой строки НЕТ ВОВСЕ: форма и количество стоят
    в заголовке карточки вперемешку с производителем. Пересказывать
    её нечем и не надо — отдаётся сам заголовок, дословно и целиком.
    Правило дословности не знает исключений даже в тексте отказа.
    """
    if источник is None or источник is _апт_ист.ВИДАЛЬ:
        return _апт_дозы.чем_отличается(страница_html or "")
    return (источник.описание(страница_html or "") or "")[:100]


async def _апт_искать_в_источнике(кл, поз, источник, базовая=frozenset()):
    """(текст, адрес) раздела о дозировании ЭТОГО препарата — В ОДНОМ ИСТОЧНИКЕ.
    ...
    try:
        ...
        for запрос in запросы:
            try:
                выдача = await _апт_страница(
                    кл, источник.адрес_поиска(запрос), источник.домен)
            except ПределСправочника:
            ...
            # у `ссылки_поиска`
            # ЗАСЛОН СУЩЕСТВОВАНИЯ СТОИТ ЗДЕСЬ, И ОБОЙТИ ЕГО НЕЛЬЗЯ
            # ПО ПОСТРОЕНИЮ (BACKLOG №205).
            #
            # `справочник_знает` и `ссылки_поиска` — ОДИН И ТОТ ЖЕ
            # разбор: подтверждение существования и список того, что
            # открывать, выводятся из одной выдачи. Разведи их —
            # и заслон начал бы пропускать имена, по которым нечего
            # показать, либо отвергать те, по которым есть, и разошлись
            # бы они молча (§6.0.7).
            #
            # Отсюда свойство, ради которого заслон и заводился:
            # НИ ОДНА СТРАНИЦА НЕ ОТКРЫВАЕТСЯ, пока справочник
            # не подтвердил имя. Не «мы не забываем проверить»,
            # а «проверять нечего — открывать нечего».
            свои = [u for u in источник.карточки(выдача, искомое["форма"],
                                                 искомое["доза"])
                    if u not in базовая]
            if not свои:
                continue
        ...
        if not адреса:
            ...
            return None, ("справочник %s — про лекарственные средства, "
                          ...
                          % источник.имя), ДОЗЫ_ВИД_НЕТ_НАЗВАНИЯ, ""
        подошли, нашлись, мешает_вещество = [], [], []
        for u in адреса:
            try:
                стр = await _апт_страница(кл, источник.адрес_страницы(u),
                                          источник.домен)
            except Exception:
                continue
            ок, детали = _апт_дозы.подходит(искомое, стр, источник)
            нашлись.append(_апт_чем_отличается(стр, источник))
        ...
        if not подошли:
            ...
            # ── ПОДСКАЗКА ФОРМЫ У ЭТОГО ИСХОДА (№239, блок B) ───────
            #
            # ЗДЕСЬ ОНА НУЖНЕЕ ВСЕГО, и первая версия правки её тут
            # не собирала: подсказки брались со страницы, ВЫБРАННОЙ
            # сопоставлением, а здесь сопоставление не выбрало ни одной.
            # То есть у трёх боевых позиций с неверной формой
            # предложить было нечего — ровно у тех, ради которых
            # блок и заводился (замер C.2).
            #
            # ЕДИНОГЛАСИЕ ОБЯЗАТЕЛЬНО. Страниц тут несколько, и форма
            # у них бывает разная: «таблетки 100 мг» и «гранулы
            # 25 мг/пак» под одним именем. Разошлись — предложения
            # НЕТ: выбрать за человека, какую пачку он держит, нечем
            # (тот же довод, что у ничьей в `форма_страницы`).
            формы = {_апт_дозы.форма_страницы(о)[0] for о in нашлись}
            формы.discard(None)
            слова = {_апт_дозы.форма_страницы(о)[1] for о in нашлись}
            слова.discard("")
            подсказка_выпуска = {"вещество": "", "форма": "",
                                 "форма_слово": ""}
            if len(формы) == 1:
                единственная = формы.pop()
                # ПРЕДЛАГАЕМ ТОЛЬКО ОТЛИЧНОЕ от записанного: совпавшее
                # предложение — шум, который учит не читать раздел
                if единственная != поз.form:
                    подсказка_выпуска["форма"] = единственная
                    подсказка_выпуска["форма_слово"] = (
                        sorted(слова, key=len)[-1] if слова else "")
            ...
            return None, ("в справочнике есть «%s», но других выпусков%s. "
                          ...
                          % (" ".join((поз.name or "").split()) or название,
                             (": " + чем) if чем else "")
                          ), ДОЗЫ_ВИД_ВЫПУСКИ, подсказка_выпуска
        if len(подошли) > 1:
        ...
        u, стр = подошли[0]
        заголовок, кусок = источник.раздел_доз(стр)
        ...
        _, п_кусок = источник.раздел_показаний(стр)
        if п_кусок:
            п_текст, п_блоки = _апт_дозы.разметить(п_кусок)
            п_текст = п_текст.strip()
        # ── ЧТО СПРАВОЧНИК ГОВОРИТ О САМОМ ВЫПУСКЕ (№239, блок B) ──
        #
        # Отдаётся И ПРИ УДАЧЕ, а не только при отказе, и вот почему:
        # схема находится и при ПУСТОМ у нас веществе — сверка его
        # тогда не проверяет вовсе (§5.8, «не заполнено у нас — не
        # проверяем»). Замер по боевой аптечке: вещество пусто у семи
        # позиций из 49, и до этой правки предложить им было нечего.
        #
        # Со СТРАНИЦЫ, УЖЕ ВЫБРАННОЙ сопоставлением, а не отдельным
        # запросом: второй выбор страницы разошёлся бы с первым молча
        # (§6.0.7) — тот же довод, по которому показания берутся
        # оттуда же
        return (текст, источник.адрес_страницы(u), блоки,
                п_текст, п_блоки), "", "", _апт_подсказки_страницы(
                    стр, источник)
    except ПределСправочника as e:
...
def _апт_подсказки_страницы(страница_html, источник=None):
    """ЧТО СПРАВОЧНИК ГОВОРИТ О ВЫПУСКЕ — вещество и форма, дословно.

    BACKLOG №239, блок B. Возвращает словарь, а не строку, и это
    решение: полей стало три, и четвёртый элемент кортежа пришлось бы
    расширять до пятого, шестого — то есть заводить перечень там, где
    нужен один предмет (§6.0.7). Словарь читает ОДНО место —
    `_апт_записать_итог_доз`.

    СВОЕГО РАЗБОРА ЗДЕСЬ НЕТ НИ СТРОКИ: и вещество, и форма берутся
    у `medkit_dosage` — того же модуля, который делает сопоставление.
    Свой разбор дал бы предложение про поведение, которого на бою нет.
    """
    if источник is None:
        источник = _апт_ист.ВИДАЛЬ
    опис = источник.описание(страница_html or "")
    код, слово = _апт_дозы.форма_страницы(опис)
    return {
        "вещество": источник.вещество(страница_html) or "",
        "форма": код or "",
        "форма_слово": слово or "",
    }
...
def _апт_подсказки_разобрать(подсказка):
    """Подсказка приходит СТРОКОЙ либо СЛОВАРЁМ — приводим к одному виду.

    BACKLOG №239, блок B. Отказные пути отдают одну строку (вещество,
    названное иначе, — №199 B.1), удачный путь отдаёт словарь из трёх
    полей. Разбор ОДИН и лежит здесь: два разбора разошлись бы молча,
    и «форма не предложена» было бы неотличимо от «предложение
    потеряно по дороге» (§6.0.7).

    Незнакомый тип даёт пустые подсказки, а не исключение: предложение —
    удобство, и ронять на нём запись итога поиска нельзя.
    """
    if isinstance(подсказка, dict):
        return ((подсказка.get("вещество") or "").strip(),
                (подсказка.get("форма") or "").strip(),
                (подсказка.get("форма_слово") or "").strip())
    return ((подсказка or "").strip() if isinstance(подсказка, str) else "",
            "", "")
...
def _апт_записать_итог_доз(поз, итог, причина, вид="", подсказка=""):
    ...
    if итог:
        ...
        # ИСТОЧНИК ЗАПИСЫВАЕТСЯ ФАКТИЧЕСКИЙ, А НЕ ПЕРВЫЙ ИЗ СПИСКА
        # (BACKLOG №207). Определяется он по АДРЕСУ выдержки —
        # второго мнения о том, кто её дал, не заводится: адрес
        # и текст пришли одним кортежем из одного источника
        поз.dosage_source = _апт_ист_по_адресу(адрес).имя
        ...
        # ПОДСКАЗКИ ПИШУТСЯ И ПРИ УДАЧЕ (BACKLOG №239, блок B).
        #
        # ЗДЕСЬ СТОЯЛО `dosage_hint_substance = None`, и это было верно
        # ровно пока подсказка отвечала на вопрос «почему не нашли».
        # Теперь она отвечает на другой — «что справочник говорит
        # о выпуске», — и у найденной схемы он осмыслен: вещество
        # могло быть у нас ПУСТЫМ, и сверка его тогда не проверяла
        # вовсе (§5.8).
        в, ф, фс = _апт_подсказки_разобрать(подсказка)
        поз.dosage_hint_substance = в[:200] or None
        поз.dosage_hint_form = ф[:20] or None
        поз.dosage_hint_form_word = фс[:120] or None
    ...
    # ПОДСКАЗКИ (BACKLOG №199 B.1, №239 блок B) — предложение, а не
    # запись поверх человека: в `substance` и `form` наш код не пишет
    # НИКОГДА. Разбор один на оба пути — `_апт_подсказки_разобрать`
    в, ф, фс = _апт_подсказки_разобрать(подсказка)
    поз.dosage_hint_substance = в[:200] or None
    поз.dosage_hint_form = ф[:20] or None
    поз.dosage_hint_form_word = фс[:120] or None
...
def _апт_забыть_дозы(поз):
    ...
    # ПОДСКАЗКИ ФОРМЫ — ТОЖЕ ИЗ СПРАВОЧНИКА, значит уходят вместе
    # с остальным: они были получены со страницы ПРЕЖНЕГО препарата
    поз.dosage_hint_form = None
    поз.dosage_hint_form_word = None
    поз.indications_text = None
    поз.indications_blocks = None


def _апт_подсказка_иначе(поз):
    """РАЗЛИЧАЕТСЯ ЛИ ПОДСКАЗКА С ТЕМ, ЧТО ЗАПИСАНО. Пара «вещество,
    форма».

    BACKLOG №244, A.5. Именем, а не выражением на месте: потребителей
    два — отдача позиции наружу и мерка `check_ref_offer`, — и второй
    экземпляр разошёлся бы с первым молча (§6.0.7). Приведение строк
    берётся у `medkit_defs.нормаль`, то есть у того же места, которым
    сравнивается всё остальное в разделе.
    """
    в = bool((поз.dosage_hint_substance or "").strip()
             and (_апт_опр.нормаль(поз.dosage_hint_substance)
                  != _апт_опр.нормаль(поз.substance or "")))
    ф = bool((поз.dosage_hint_form or "").strip()
             and (поз.dosage_hint_form or "").strip() != (поз.form or ""))
    return в, ф


def _апт_подстановка_из_справочника(поз):
    """ЧТО ДЕЛАТЬ С ВЫДЕРЖКОЙ, КОГДА ЧЕЛОВЕК ВПИСАЛ СЛОВО СПРАВОЧНИКА.

    BACKLOG №244, блок A. Одно место на оба действия — «вписать
    вещество» и «вписать форму»: решение у них общее, а две копии
    разошлись бы при первой же правке и МОЛЧА (§6.0.7).

    ═══════════════════════════════════════════════════════════════════
    ПРАВИЛО ОДНОЙ ФРАЗОЙ
    ═══════════════════════════════════════════════════════════════════

    Подсказка и выдержка сняты с ОДНОЙ И ТОЙ ЖЕ страницы — значит
    вписать подсказку нельзя так, чтобы выдержка перестала относиться
    к этой пачке. Выдержка не трогается; поиск перезапускается ровно
    там, где выдержки НЕТ.

    ═══════════════════════════════════════════════════════════════════
    ПОЧЕМУ ЭТО НЕ ОСЛАБЛЕНИЕ ПРАВИЛА «ПРАВКА ЧИСТИТ ВЫДЕРЖКУ» (§5.8)
    ═══════════════════════════════════════════════════════════════════

    То правило написано про ПРОИЗВОЛЬНУЮ правку человеком: поменял
    название — и на карточке остался бы раздел о дозировании ДРУГОГО
    препарата, дословный, с источником и датой, то есть выглядящий
    совершенно достоверно. Оно в полной силе и здесь не трогается
    ни на знак.

    Подстановка — не произвольная правка. Значение записал СЕРВЕР
    из той страницы, которую выбрало сопоставление (`_апт_подсказки_
    страницы` зовётся там же, где сохраняется `dosage_text`, одним
    кортежем). После неё поля карточки СБЛИЖАЮТСЯ со страницей,
    а не расходятся: сверка по веществу, которая раньше молчала при
    пустом поле, начинает совпадать.

    ЗАМЕР, ИЗ-ЗА КОТОРОГО ЭТО ЗАВЕДЕНО (боевая база, 2026-09-07):
    позиций, где нажатие «Вписать» стирало НАЙДЕННУЮ схему, — 15.
    Владелец жал, схема пропадала целиком, и вернуть её можно было
    только повторным поиском. Правка данных, молча уничтожающая
    результат работы.

    ═══════════════════════════════════════════════════════════════════
    ИСПОЛНЕННАЯ ПОДСКАЗКА СНИМАЕТСЯ, И ЭТО ВТОРАЯ ПОЛОВИНА
    ═══════════════════════════════════════════════════════════════════

    Оставь мы её — предложение «вписать» висело бы над полем, куда
    уже вписано ровно это. Снимается ТОЛЬКО исполненная: подсказка
    соседнего поля отвечает на свой вопрос и остаётся.

    Возвращает True, если поиск надо перезапустить.
    """
    выдержка_есть = bool(_апт_выдержка(поз))
    if not выдержка_есть:
        # Схемы нет — подстановка затем и нужна, чтобы поиск нашёл её
        # теперь (замер задачи 239: было 5 из 10, стало 7 из 10).
        # Прежнее состояние справочника при этом уходит целиком:
        # причина отказа отвечала на вопрос о ПРЕЖНИХ полях
...
class _АптИскомое:
    """ПОЛЯ ПОЗИЦИИ, НУЖНЫЕ ПОИСКУ, — снимок вместо объекта ORM.

    BACKLOG №207. Соединение к базе отдаётся ПЕРЕД походом в чужую
    сеть (§6.0.6), а объект ORM после этого отцеплен: читать его поля
    можно, но поведение зависит от того, что успело подгрузиться, —
    и отказ был бы молчаливым. Простой предмет с теми же именами полей
    таких вопросов не задаёт.

    ПОЛЯ ТОЛЬКО ТЕ, ЧТО ЧИТАЕТ ПОИСК: `name`, `substance`, `form`, `id`.
    Дать сюда весь объект значило бы вернуть ту же неопределённость,
    только под другим именем.
    """

    __slots__ = ("id", "name", "substance", "form")

    def __init__(self, поз):
        self.id = поз.id
        self.name = поз.name
        self.substance = поз.substance
        self.form = поз.form
...
async def _апт_дозы_фоном(user_id: int, item_id: int):
    ...
    """
    # ═══════════════════════════════════════════════════════════════
    # СОЕДИНЕНИЕ ОТДАЁТСЯ ПЕРЕД ПОХОДОМ В ЧУЖУЮ СЕТЬ
    # ═══════════════════════════════════════════════════════════════
    #
    # BACKLOG №207. Здесь стояла ОДНА сессия на всю задачу, открытая
    # до поиска и закрытая после записи, — то есть соединение к базе
    # было занято ВСЁ ВРЕМЯ похода к чужому сайту. Ровно то, что
    # запрещает §6.0.6: под WAL это мешает контрольной точке, а два
    # таких обработчика подряд дают `database is locked`.
    #
    # НАШЁЛ ЭТО ПРОГОН, А НЕ ЧТЕНИЕ: в журнале стенда 2026-09-04 три
    # строки «фоном для N оборвалось: OperationalError: database is
    # locked» — фоновая задача не смогла записать СОБСТВЕННЫЙ итог.
    #
    # ДЕФЕКТ ПРЕДСУЩЕСТВУЮЩИЙ, но этот заход сделал его тяжелее в разы:
    # у второго источника таймаут 90 с против 20 у первого, и держать
    # соединение стало можно минутами, а не секундами.
    #
    # ЛЕЧИТСЯ ТЕМ ЖЕ ПРИЁМОМ, ЧТО `_пауза` (§6.0.5): читаем нужное,
    # ЗАКРЫВАЕМ сессию, идём в сеть, открываем заново и переспрашиваем
    # позицию. Переспрашивать всё равно было надо — между запросом
    # и записью человек мог позицию поправить или удалить.
    db = SessionLocal()
    try:
        ...
        if not поз:
            return
        # СНИМОК ПОЛЕЙ, А НЕ ОТЦЕПЛЕННЫЙ ОБЪЕКТ: после `close()` объект
        # ORM отцеплен, и читать его поля — то самое молчаливое
        # поведение, которое §6.0.5 разбирает на записи. Простой
        # предмет с теми же именами полей ведёт себя предсказуемо
        искомое = _АптИскомое(поз)
    finally:
        db.close()
    итог, причина, вид, подсказка = await _апт_найти_дозы(искомое)
    db = SessionLocal()
    try:
...
def _апт_отсеять_домыслы(черновик: dict, источник: str) -> tuple[dict, list]:
    """Поле, не оставившее следа в источнике, — ДОМЫСЕЛ. Выбрасываем.

    ═══════════════════════════════════════════════════════════════════
    ЗАЧЕМ, И ЭТО ЗАМЕР, А НЕ ОСТОРОЖНОСТЬ
    ═══════════════════════════════════════════════════════════════════

    Замер 2026-09-01, три прогона подряд (`py check_medkit_entry.py
    --повторов 3`): по фотографии упаковки, на которой действующее
    вещество НЕ НАПЕЧАТАНО, модель заполняла его **3 раза из 3** —
    «Ацетилсалициловая кислота + Парацетамол + Кофеин», из собственной
    памяти. На экране такое поле неотличимо от прочитанного, а ляжет
    оно в базу и будет годами показываться как факт.

    ПОСТАНОВКА ЗАХОДА СЧИТАЛА, ЧТО ДЕФЕКТ В ТЕКСТОВОЙ ВЕТКЕ («по фото
    модель читает упаковку, это честно»). ЗАМЕР ЭТО ОПРОВЕРГ: текст
    дал 0 домыслов из 3 и даже переспросил про вещество, а дополняла
    модель именно ФОТО — там наша же подсказка перечисляет поля,
    которые надо «прочитать», и модель заполняет их все.

    ═══════════════════════════════════════════════════════════════════
    ПОЧЕМУ КОДОМ, А НЕ ПРАВИЛОМ В ПРОМПТЕ
    ═══════════════════════════════════════════════════════════════════

    Промпт — просьба, и правило «не выдумывай» стояло там с самого
    начала (пункт 1). Оно и не сработало. Тот же довод, по которому
    белый список полей держится здесь, а не в тексте промпта.

    ЧЕРНЫЙ СПИСОК ПОЛЕЙ ЗДЕСЬ НЕ ГОДИТСЯ: «вещество» отсеивать всегда
    нельзя — с упаковки оно читается законно и нужно для сверки схемы
    приёма. Отсев идёт по ИСТОЧНИКУ значения, а не по имени поля.

    ═══════════════════════════════════════════════════════════════════
    ЧЕГО ОН НЕ ЗАКРЫВАЕТ, И ЭТО НАЗВАНО
    ═══════════════════════════════════════════════════════════════════

    Источник для фотографии — ВЫПИСКА САМОЙ МОДЕЛИ (поле `видно`).
    То есть заслон ловит «дописала поле, которого не выписала»,
    и не ловит выдумку, повторённую и в выписке. Второе дороже
    для модели и заметнее человеку: выписка показывается рядом.
    Пустая выписка — отдельный исход, разбор у места вызова.
    """
    оставлено, отсеяно = {}, []
    for ключ, зн in черновик.items():
        if ключ in ("категории_словами", "вопрос"):
            оставлено[ключ] = зн
            continue
        варианты = _апт_опр.человеческое(ключ, зн)
        if any(_апт_опр.след(в, источник) for в in варианты):
            оставлено[ключ] = зн
        else:
            отсеяно.append(ключ)
    if отсеяно:
        print("[апт-заведение] ОТСЕЯНО как домысел (следа в источнике нет): "
              "%s" % ", ".join(отсеяно))
    return оставлено, отсеяно


def _апт_дополнить_черновик(прежний: dict, сырой: dict, ответ: str,
                            db, user) -> dict:
    """ЧЕРНОВИК ПЛЮС ОТВЕТ ЧЕЛОВЕКА (BACKLOG №238, B.1).

    ═══════════════════════════════════════════════════════════════════
    ЗАСЛОН ДОМЫСЛОВ НЕ ОСЛАБЛЯЕТСЯ НИ НА ЗНАК
    ═══════════════════════════════════════════════════════════════════

    Поля здесь ДВУХ происхождений, и проверяются они по-разному:

      · ПРЕЖНИЕ уже прошли отсев тогда, когда их разбирали, — источником
        служило исходное описание. Гонять их через отсев ещё раз нечем:
        того описания сейчас под рукой нет, а по ответу «90» они следа
        не оставят и будут выброшены ЦЕЛИКОМ. То есть повторный отсев
        не усилил бы заслон, а стёр бы верно разобранную карточку;

      · НОВЫЕ проходят отсев ПО ОТВЕТУ ЧЕЛОВЕКА, тем же
        `_апт_отсеять_домыслы` и той же функцией, что на бою. Ответ
        «90» не содержит ни названия, ни вещества — значит модель,
        дописавшая их «заодно», получит их обратно вычеркнутыми.

    ЧЕРНОВИК ПЕРЕЗАПИСЫВАТЬ НОВЫМИ ЗНАЧЕНИЯМИ НЕЛЬЗЯ, и промпт просит
    об этом (правило 2), но просьба — не гарантия: слияние идёт
    «прежнее сильнее», и поле, которое модель решила «уточнить»,
    до карточки не доедет. Человек правит поля руками в форме;
    переписывать их из второго ответа модели значило бы менять то,
    что он уже видел и принял.
    """
    новые = _апт_очистить_черновик(сырой.get("поля") or {})
    новые, отсеяно = _апт_отсеять_домыслы(новые, ответ)
    если_было = dict(прежний.get("поля") or {})
    добавлено = []
    for ключ, зн in новые.items():
        if ключ in ("категории_словами",):
            continue
        # ПРЕЖНЕЕ СИЛЬНЕЕ — разбор выше
        if если_было.get(ключ) in (None, "", []):
            если_было[ключ] = зн
            добавлено.append(ключ)
    # КАТЕГОРИИ ДОБИРАЮТСЯ, А НЕ ЗАМЕНЯЮТСЯ: их у позиции несколько
    # по построению (MedkitItemCategory — многие-ко-многим), и вторая
    # реплика вполне может назвать ещё одну
    словами = (сырой.get("поля") or {}).get("категории") or []
    if словами:
        было = set(если_было.get("категории") or [])
        для_поиска = {"категории_словами": словами}
        сведённые = _апт_свести_черновик(для_поиска, db, user)
        было |= set(сведённые["поля"].get("категории") or [])
        если_было["категории"] = sorted(было)
    вопрос = str(сырой.get("вопрос") or "").strip()
    итог = dict(прежний)
    итог["поля"] = если_было
    итог["вопрос"] = вопрос
    итог["вид"] = "заведение"
    # ЧТО ЗАПОЛНИЛ АССИСТЕНТ — СПИСОК РАСТЁТ, А НЕ ПЕРЕПИСЫВАЕТСЯ:
    # пометка у поля из первой реплики обязана пережить вторую,
    # иначе на экране «заполнил ассистент» исчезнет у полей, которых
    # человек не касался
    итог["заполнено"] = sorted(set(прежний.get("заполнено") or [])
                               | set(добавлено))
    # СРОК, ПОДСТАВЛЕННЫЙ АССИСТЕНТОМ, ТРЕБУЕТ ПОДТВЕРЖДЕНИЯ И ЗДЕСЬ:
    # он мог приехать как первой репликой, так и этой
    итог["срок_подтвердить"] = "expires_ym" in если_было
    if добавлено:
        print("[апт-заведение] ответ дополнил черновик: %s"
              % ", ".join(добавлено))
    elif отсеяно:
        print("[апт-заведение] ответ ничего не добавил, отсеяно: %s"
              % ", ".join(отсеяно))
    return итог
...
def _апт_свести_черновик(черновик: dict, db, user) -> dict:
    ...
    if форма:
        # ЕДИНИЦА ВНЕ ФОРМЫ БОЛЬШЕ НЕ ЗАМЕНЯЕТСЯ ПЕРВОЙ РАЗРЕШЁННОЙ
        # (BACKLOG №238, A.3). Здесь стояло `из_модели["unit"] =
        # форма["единицы"][0]`, и до расширения списка это было почти
        # безобидно: единица у формы была одна, подставлялась она же.
        #
        # С приходом форм, где единиц НЕСКОЛЬКО (капли — капля или мл,
        # сироп — мл или ложка), та же строка стала выдумкой: модель
        # сказала «мл», мы молча пишем «капля», и человек видит
        # заполненное поле, которого никто не выбирал. Постановка A.3
        # называет это прямо: «не читается с упаковки — поле пустое
        # или „не выбрано“, НИКОГДА не наугад».
        #
        # ЕДИНСТВЕННАЯ ЕДИНИЦА ФОРМЫ — НЕ ДОГАДКА, А ОПРЕДЕЛЕНИЕ:
        # выбрав «таблетки», человек выбрал и «штуки»; спрашивать его
        # об этом значило бы показать выбор из одного варианта.
        if из_модели.get("unit") not in форма["единицы"]:
            if len(форма["единицы"]) == 1:
    ...
    # ОБЪЁМ УПАКОВКИ: пара пишется целиком либо не пишется вовсе —
    # то же правило, что на сервере (`_апт_проверить`). «10» без «мл»
    # на карточке не объём, а загадка
    if из_модели.get("volume_unit") not in _апт_опр.ЕДИНИЦА_ОБЪЁМА_ПО_ID:
        из_модели.pop("volume_unit", None)
        из_модели.pop("volume", None)
    if из_модели.get("volume") in (None, ""):
        из_модели.pop("volume", None)
        из_модели.pop("volume_unit", None)
...
ЕГО АПТЕЧКА (номер — название — вещество — форма — категории — ОТ ЧЕГО):
{аптечка}

Верни ТОЛЬКО JSON без ```json и без пояснений:
{{"тип":"поиск","подходят":[3,7],"группы":[{{"имя":"Головная боль","цитата":"дословный кусок из ОТ ЧЕГО","ids":[3,7]}}],"нет":"","группа":""}}
...
2. При типе "поиск" подбирай по полю ОТ ЧЕГО и по категории позиции, а не по своим знаниям о болезнях. Это поиск по списку, а не диагноз.
2б. ПРИ ТИПЕ "поиск" РАЗЛОЖИ НАЙДЕННОЕ ПО ГРУППАМ — это поле «группы». Группа — это одно состояние, про которое написано в ОТ ЧЕГО у нескольких позиций сразу: «Головная боль», «Спазмы и колики», «Температура». Имя группы — 1–4 слова про СОСТОЯНИЕ, без торговых названий и без слов о свойствах препарата.
2в. У КАЖДОЙ ГРУППЫ ОБЯЗАТЕЛЬНА «цитата» — кусок текста ОТ ЧЕГО одной из её позиций, СКОПИРОВАННЫЙ ЗНАК В ЗНАК из списка выше. Ничего не дописывай, не сокращай и не пересказывай: программа сверяет цитату с сохранённым текстом побуквенно и группу с непроверенной цитатой не покажет.
2г. Позицию, у которой в ОТ ЧЕГО написано «не записано», в группы НЕ помещай — про неё сказать нечего, и программа скажет это сама. В «подходят» её тоже не добавляй, если она не подходит по категории.
2д. Отбирать в «подходят» по ДОСЛОВНОМУ совпадению слов НЕ НАДО: «головная боль» и «боль в висках» — про одно и то же. Смотри на смысл текста ОТ ЧЕГО.
...
6. Тип "схема", и такой позиции в списке НЕТ — оставь «подходят» пустым и скажи в «нет», что такой пачки в аптечке нет.
...
# ── СКОЛЬКО ПОКАЗАНИЙ УЕЗЖАЕТ В ПРОМПТ НА ПОЗИЦИЮ (BACKLOG №245, B.1)
#
# ДО ЭТОГО ЗАХОДА ПОКАЗАНИЙ В ПРОМПТЕ НЕ БЫЛО ВОВСЕ: модель получала
# название, вещество, форму и категории — и раскладывать по смыслу
# ей было НЕ ПО ЧЕМУ. Отбор при этом делал сервер, дословным
# совпадением основ слов вопроса с текстом показаний, и живой прогон
# показал, чего это стоит: на «болит голова в висках» сошлись ДВЕ
# карточки, а средство, у которого в показаниях стоит «головная боль,
# в т.ч. мигренозного характера», ушло в кучу «дословно не сошлись».
# Про то же самое, другими словами.
#
# ЧИСЛО ВЫВЕДЕНО ЗАМЕРОМ по боевой базе (77 позиций, показания
# у 40 из них): полный текст даёт 15 293 знака и вместе с мета-строками
# упирается в `АПТ_СПИСОК_ЗНАКОВ`, то есть часть позиций перестала бы
# доезжать до модели — ровно тот немой отказ, ради которого список
# режется по строкам. 220 знаков — медиана показаний (340) с запасом
# вниз: этого хватает, чтобы понять, о каком состоянии речь, и не
# хватает, чтобы список раздулся.
#
# ОБРЕЗОК ОСТАЁТСЯ ДОСЛОВНЫМ ПРЕФИКСОМ, и это условие проверки цитаты:
# сверка идёт с ПОЛНЫМ сохранённым текстом, а префикс — его подстрока
# по построению.
АПТ_ПОКАЗАНИЕ_В_ПРОМПТ = int(os.getenv("MEDKIT_IND_PROMPT", "220"))
...
def _апт_промпт_запроса(вопрос: str, позиции, кат_имена,
    ...
    for п in позиции:
        ...
        # ПОКАЗАНИЯ ИДУТ ДОСЛОВНЫМ ПРЕФИКСОМ, а не пересказом: цитату
        # модели мы потом сверяем с ПОЛНЫМ сохранённым текстом побуквенно,
        # и пересказанный кусок не сошёлся бы ни разу
        куски = [" ".join(т.split())[:АПТ_ПОКАЗАНИЕ_В_ПРОМПТ]
                 for т, _м in _апт_тексты_от_чего(п)]
        пок = " | ".join(куски) if куски else "не записано"
        строки.append("%d — %s — %s — %s — %s — ОТ ЧЕГО: %s"
                      % (п["id"], п["name"], п["substance"] or "вещество не записано",
                         п["форма_имя"], имена, пок))
    влезло, длина = [], 0
...
def _апт_ждём_ответа(db, user):
    """ЧЕРНОВИК, ПО КОТОРОМУ АССИСТЕНТ ЗАДАЛ ВОПРОС И ЖДЁТ ОТВЕТА.

    ═══════════════════════════════════════════════════════════════════
    ЗАЧЕМ, И ЭТО ЖИВОЙ СЛУЧАЙ, А НЕ ПРЕДОСТОРОЖНОСТЬ
    ═══════════════════════════════════════════════════════════════════

    Владелец завёл позицию текстом. Ассистент собрал карточку и спросил:
    «Сколько доз в упаковке?». Владелец ответил «90». Ассистент:
    «Не понял вопрос. Если хотите завести упаковку — назовите препарат…».

    Воспроизведено замером до правки, дословно: три поля черновика
    («Стрепсилс», срок, категория) после ответа «24» пережили НОЛЬ,
    вид ответа — «непонятно».

    ПРИЧИНА: заведение не получало НИЧЕГО, кроме текущей реплики.
    `_апт_промпт(описание)` собирается из одного текста, а «24» само
    по себе упаковкой не является — значит `_апт_есть_что_заводить`
    честно отвечает «нет», и срабатывает третий исход задачи 197.
    То есть вопрос задавался, а состояние диалога не держалось нигде.

    ═══════════════════════════════════════════════════════════════════
    СОСТОЯНИЕ БЕРЁТСЯ ИЗ ПЕРЕПИСКИ, А НЕ ЗАВОДИТСЯ ОТДЕЛЬНО
    ═══════════════════════════════════════════════════════════════════

    Черновик УЖЕ хранится — `chat_messages.payload`, структурная часть
    реплики (BACKLOG №178, C). Заводить рядом второе хранилище «что мы
    сейчас спрашиваем» значило бы получить два состояния одного
    разговора, расходящихся молча: панель показывает один черновик,
    сервер помнит другой, и какой из двух настоящий — не узнать
    никогда (§6.0.7, довод про `sessionStorage`).

    ПОСЛЕДНЯЯ РЕПЛИКА, А НЕ ПОИСК ПО ИСТОРИИ. Вопрос живёт ровно до
    следующей реплики человека: ответил — состояние закрыто, сменил
    тему — тоже. Искать «последний незакрытый вопрос» глубже значило бы
    воскрешать разговор, который человек уже бросил.
    """
    реплики = _апт_чат_реплики(db, user)
    if not реплики:
        return None, ""
    последняя = реплики[-1]
    if последняя.role == "user" or not последняя.payload:
        return None, ""
    try:
        тело = _json.loads(последняя.payload) or {}
    except ValueError:
        return None, ""
    if тело.get("вид") != "заведение":
        return None, ""
    вопрос = str(тело.get("вопрос") or "").strip()
    # НЕТ ВОПРОСА — НЕТ И ОЖИДАНИЯ. Карточка, собранная целиком,
    # ответа не ждёт: следующая реплика человека это новая тема
    return (тело if вопрос else None), вопрос


# ПРОМПТ УТОЧНЕНИЯ — ОТДЕЛЬНЫЙ, И ЭТО РЕШЕНИЕ (BACKLOG №238, B.1).
#
# Дописать хвост переписки в `ПРОМПТ_АПТЕЧКИ` было бы короче на десять
# строк и неверно: тот промпт разбирает УПАКОВКУ, у него своя приёмка
# на двадцати двух формах, и правка его поведения — это правка того,
# что уже замерено (урок задачи 176: дописанная задача сдвинула форму
# «Фервекс» с powder на sachet 3 прогона из 3).
#
# Здесь задача другая и узкая: перед моделью ОДИН вопрос, ОДИН ответ
# и уже собранные поля. Она либо дополняет черновик, либо говорит,
# что человек сменил тему.
ПРОМПТ_УТОЧНЕНИЯ = """Ты ведёшь заведение карточки лекарства в домашней аптечке.

Ты уже собрал часть полей и задал ОДИН вопрос. Человек ответил.

УЖЕ СОБРАНО: {черновик}
ТВОЙ ВОПРОС БЫЛ: {вопрос}
ЧЕЛОВЕК ОТВЕТИЛ: {ответ}

Верни ТОЛЬКО JSON без ```json и без пояснений:
{{"продолжение":true,"поля":{{}},"вопрос":"","вид":"заведение"}}

ФОРМЫ ВЫПУСКА, только эти id: {формы}
ЕДИНИЦЫ, только эти id: {единицы}
СОЧЕТАНИЯ «форма → допустимые единицы»: {сочетания}
КАТЕГОРИИ, только эти названия: {категории}

ПРАВИЛА, нарушать нельзя:
1. "продолжение": true, если ответ человека отвечает на ТВОЙ вопрос — хотя бы частично. false, если человек сменил тему: назвал другой препарат, спросил, что есть дома, или заговорил о другом.
2. При "продолжение": true в "поля" клади ТОЛЬКО то, что следует из ответа человека. Уже собранное не повторяй и не меняй.
3. Ничего не выдумывай. В ответе «90» нет ни названия, ни срока — значит там только одно число, и понять, к какому полю оно относится, можно лишь по твоему вопросу.
4. Если человек ответил «пропусти», «не знаю», «неважно» — "продолжение": true, "поля" пустые, "вопрос" пустой: поле останется незаполненным, и это нормально.
5. Если после этого ответа для карточки не хватает чего-то ВАЖНОГО — задай следующий ОДИН короткий вопрос в "вопрос". Хватает — оставь "вопрос" пустым.
6. Про разовую дозу и срок после вскрытия не спрашивай никогда: дозу назначает врач, а срок после вскрытия печатают на упаковке.
7. Названия полей те же, что при заведении: name, substance, form, unit, qty_total, qty_left, volume, volume_unit, expires_ym, code_gtin, категории.

ОТВЕТ ЧЕЛОВЕКА:
{ответ}"""


def _апт_промпт_уточнения(черновик: dict, вопрос: str, ответ: str) -> str:
    """Списки форм и единиц — ИЗ ТОГО ЖЕ ИСТОЧНИКА, что у заведения.

    Второй перечень словами разошёлся бы с первым молча, и уточняющий
    шаг принимал бы формы, которых сервер не знает (§6.0.7).
    """
    поля = {к: в for к, в in (черновик.get("поля") or {}).items()
            if к != "категории"}
    return ПРОМПТ_УТОЧНЕНИЯ.format(
        черновик=_json.dumps(поля, ensure_ascii=False) or "{}",
        вопрос=вопрос[:300],
        ответ=ответ[:СТРАНИЦА_ЗНАКОВ],
        сочетания="; ".join(
            "%s → %s" % (ф["id"], "/".join(ф["единицы"]))
            for ф in _апт_опр.ФОРМЫ if len(ф["единицы"]) > 1),
        формы=", ".join("%s (%s)" % (ф["id"], ф["имя"])
                        for ф in _апт_опр.ФОРМЫ),
...
async def _апт_ответ_на_запрос(вопрос: str, db, user) -> dict:
    ...
    сырой, беда, код = await _сеть(
        db, _апт_спросить_модель(промпт, очищать=False))
    if беда:
    ...
    нашлось, просрочено, рецептурные = [], [], []
    # ЧТО ИСКАТЬ В ПОКАЗАНИЯХ — основы слов ЖАЛОБЫ (BACKLOG №244, D.1).
    # Считается один раз на ответ: пересчёт в цикле дал бы то же самое
    # по каждой позиции
    основы = _апт_основы_вопроса(вопрос)
    # ГРУППЫ ПО СМЫСЛУ ПОКАЗАНИЙ (BACKLOG №245, B.1). Разбор и три
    # заслона — в шапке `_апт_группы_из_ответа`; здесь важно, что
    # проверка идёт ПОСЛЕ разбора номеров и ПО ТОМУ ЖЕ набору
    группы, группа_по_id = _апт_группы_из_ответа(сырой, по_id, разрешённые)
    for н in номера:
        п = по_id[н]
        # СЧИТАЕТСЯ ОДИН РАЗ: внутри разбор предложений и проверка
        # дословности, и второй вызов дал бы ту же работу впустую
        пок_текст, пок_откуда = _апт_показание_с_меткой(п, основы)
        строка = {"id": н, "строка": _апт_строка_позиции(п),
                  ...
                  #
                  # Владелец узнаёт пачку ПО ВИДУ, а не по названию,
                  # и без снимка выходил из ассистента искать её
                  # в списке. Адрес отдаёт то же место, что и карточке;
                  # своего пути к файлу здесь не заводится
                  "photo": п.get("photo") or "",
                  # ── ДОСЛОВНАЯ СТРОКА ПОКАЗАНИЙ, ГДЕ СОШЛОСЬ (D.1) ──
                  #
                  # Пусто означает РОВНО ОДНО из двух: показаний
                  # у пачки нет вовсе либо они есть и с жалобой
                  # не сошлись. Различает эти два случая поле
                  # `показания_есть` — без него «сказать нечего»
                  # читалось бы как «не подходит» (D.2)
                  # ПОКАЗАНИЕ ПОД КАРТОЧКОЙ: та строка, где сошлось
                  # с жалобой, иначе — начало раздела «от чего».
                  # Совпало с цитатой группы — не печатаем дважды
                  "показание": пок_текст,
                  "показание_откуда": пок_откуда,
                  "группа": группа_по_id.get(н, ""),
                  "показания_есть": bool(_апт_тексты_от_чего(п)),
                  "дозы": bool(п["дозы"]),
                  # СКОЛЬКО УПАКОВОК СВЕРНУЛОСЬ В ЭТУ КАРТОЧКУ (B.4).
                  # Единица — обычный случай; двойка и больше означают,
                  # что строк в базе несколько, а карточка одна
                  "упаковок": 1}
    ...
    # ── ОДНА КАРТОЧКА НА ПРЕПАРАТ, СКОЛЬКО БЫ ПАЧЕК НИ БЫЛО (B.4) ──
    #
    # ЗАМЕР НА ЖИВОМ ОТВЕТЕ: на «болит голова в висках» сошлись две
    # карточки — и обе оказались ОДНИМ средством в двух упаковках,
    # с одинаковым текстом. Список от этого вдвое длиннее, а нового
    # в нём ноль.
    #
    # КЛЮЧ БЕРЁТСЯ У `_апт_ключ_группы` — того же, по которому пачки
    # схлопываются в сетке карточек. Второе правило «это один препарат»
    # разошлось бы с первым молча, и ассистент показывал бы одну
    # карточку там, где список показывает две (§6.0.7).
    #
    # ВЫЖИВАЕТ БЛИЖАЙШАЯ ПО СРОКУ, и это не вкус: `id` карточки уезжает
    # в кнопку «Приём», а списывать надо с той пачки, которая испортится
    # раньше, — ровно как в свёрнутой группе сетки (§5.8, D).
    #
    # ФОТО БЕРЁТСЯ У ТОЙ ПАЧКИ, У КОТОРОЙ ОНО ЕСТЬ (C.1). Вторая
    # упаковка заводится действием «Ещё упаковка», а оно снимок
    # НЕ ПЕРЕНОСИТ намеренно; окажись она ближе по сроку — карточка
    # осталась бы без картинки при живом снимке, и это читалось бы
    # как «фото потеряно». Тот же довод, что у сводки группы в сетке.
    def _свернуть(куча):
        порядок, по_ключу = [], {}
        for с in куча:
            п = по_id[с["id"]]
            ключ = _апт_ключ_группы(п)
            if ключ not in по_ключу:
                по_ключу[ключ] = [с]
                порядок.append(ключ)
            else:
                по_ключу[ключ].append(с)
        итог = []
        for ключ in порядок:
            пачки = по_ключу[ключ]
            пачки.sort(key=lambda с: (по_id[с["id"]].get("expires_ym")
                                      or "9999-99"))
            главная = dict(пачки[0])
            главная["упаковок"] = len(пачки)
            if not главная.get("photo"):
                главная["photo"] = next(
                    (с["photo"] for с in пачки if с.get("photo")), "")
            главная["дозы"] = any(с.get("дозы") for с in пачки)
            итог.append(главная)
        return итог

    нашлось = _свернуть(нашлось)
    просрочено = _свернуть(просрочено)
    рецептурные = _свернуть(рецептурные)
    # ЦИТАТА ГРУППЫ И ПОКАЗАНИЕ КАРТОЧКИ НЕ ПЕЧАТАЮТСЯ ДВАЖДЫ
    цитаты = {г["имя"]: _апт_пробелы(г["цитата"]) for г in группы}
    for с in нашлось + просрочено + рецептурные:
        если_та_же = цитаты.get(с.get("группа") or "")
        if если_та_же and _апт_пробелы(с.get("показание") or "") == если_та_же:
            с["показание"] = ""
    ...
    # ── ПОРЯДОК: СОШЛОСЬ С ЖАЛОБОЙ — ВВЕРХ (BACKLOG №244, D.1, D.2) ──
    #
    # ТРИ СТУПЕНИ, И СРЕДНЯЯ ОБЯЗАТЕЛЬНА. «Показания есть, но с этой
    # жалобой не сошлись» и «показаний у пачки нет вовсе» — разные
    # факты с разными выводами: про первое мы можем сказать, что
    # смотрели, про второе — только что смотреть было нечего.
    # Свести их значило бы объявить «не подходит» там, где мы просто
    # не знаем (§6.0.1).
    #
    # СОРТИРОВКА УСТОЙЧИВАЯ: внутри ступени порядок остаётся тем,
    # который назвала модель, — переставлять его нам не на чем.
    # СТУПЕНЕЙ ПО-ПРЕЖНЕМУ ТРИ, НО ПЕРВАЯ ТЕПЕРЬ — «ПОПАЛО В ГРУППУ»,
    # а не «дословно совпало с жалобой» (BACKLOG №245, B.1, B.3).
    #
    # ТРЕТЬЯ СТУПЕНЬ ОБЯЗАТЕЛЬНА И ОНА ПРО ДРУГОЕ: «показаний в
    # справочнике нет вовсе» — это НЕ «показания есть, но не подошли».
    # Свести их значило бы сказать «не подходит» там, где мы просто
    # не знаем (§6.0.1). Замер по боевой базе: показаний нет
    # у 37 позиций из 77 — почти половина, и назвать их «не подошли»
    # было бы неправдой про половину аптечки.
    место_группы = {г["имя"]: i for i, г in enumerate(группы)}

    def _ступень(с):
        if с.get("группа"):
            return (0, место_группы.get(с["группа"], 0))
        return (1 if с.get("показания_есть") else 2, 0)

    for куча in (нашлось, просрочено, рецептурные):
        куча.sort(key=_ступень)
    все_строки = нашлось + просрочено + рецептурные
    # ЧИСЛА ОТБОРА СЧИТАЕТ СЕРВЕР, и признак у него один на весь ответ
    по_показанию = sum(1 for с in все_строки if с.get("группа"))
    без_показаний = sum(1 for с in все_строки if not с.get("показания_есть"))
    ...
    if тип == "выбор":
        ...
        # ── КАРТОЧКИ НЕ ПОВТОРЯЮТСЯ (BACKLOG №245, B.5) ──────────
        #
        # Они УЖЕ на экране — их показал прошлый ответ, и «это»
        # в вопросе «что из этого выбрать» означает именно их.
        # Прежде ответ присылал тот же список второй раз, с теми же
        # выдержками, и владелец сказал про это прямо: «не понимаю,
        # что поменялось». Он был прав — не менялось ничего.
        #
        # ПОРЯДОК ЧАСТЕЙ ОБЯЗАТЕЛЕН: сперва ответ на заданный вопрос
        # («выбрать за вас нельзя, и вот почему»), потом различия
        # как дополнение. Наоборот — это ответ не на тот вопрос.
        различия = _апт_чем_различаются(строки_выбора, по_id)
        куски = [АПТ_ВЫБОР_ГЛАВНОЕ]
        if строки_выбора:
            куски.append(АПТ_ВЫБОР_РАЗЛИЧИЯ if различия
                         else АПТ_ВЫБОР_НЕЧЕМ)
            куски += ["— " + с for с in различия]
        нет = "\n".join(куски) if строки_выбора else ""
        нашлось, просрочено, рецептурные = [], [], []
        группы, разбивка = [], []
        группа = ""
    else:
    ...
    тело = {"вид": "запрос", "тип": тип,
            "вопрос": (вопрос or "").strip()[:200],
            # ЧИСЛА ОТБОРА (BACKLOG №244, D.1, D.2). Считает СЕРВЕР:
            # признак «сошлось с жалобой» живёт в одном месте, и второе
            # мнение в браузере разошлось бы с ним молча (§6.0.7)
            "по_показанию": по_показанию,
            "без_показаний": без_показаний,
            # ГРУППЫ ПО СМЫСЛУ ПОКАЗАНИЙ (BACKLOG №245, B.1). Каждая
            # несёт ЦИТАТУ, проверенную сервером на дословность:
            # браузер её не проверяет и проверять не должен — второе
            # мнение о дословности разошлось бы с первым молча
            "группы": группы,
            ...
            "оговорка": ("Просмотрена не вся аптечка: позиций %d, взято "
...
АПТ_ВСТУПЛЕНИЕ_СХЕМА = ("Схема приёма — на карточке этой пачки, "
                        "под кнопкой «Приём».")
АПТ_ВСТУПЛЕНИЕ_СХЕМА_НЕТ = "Такой пачки в аптечке нет."
...
АПТ_НЕ_ПОНЯЛ_СНОВА = (
    "Снова не получается понять — видимо, я отвечаю не на то. "
        "Напишите одним "
...
АПТ_ПОВТОР = (
    ...
    "не получилось понять. Переспросите иначе: назовите жалобу («что у меня "
...
#
# BACKLOG №245, B.5, решение владельца. ЗДЕСЬ СТОЯЛО ОБРАТНОЕ: сперва
# «вот чем различаются», и следом тот же список карточек, что и
# в прошлом ответе. Владелец сказал про это прямо: «не понимаю, что
# поменялось» — и был прав, потому что не менялось ничего.
#
# ПОРЯДОК ВАЖЕН И ОН ЕСТЬ СУТЬ ПРАВКИ. Без первой части вторая
# читается как ответ не на тот вопрос: человек спросил «что выбрать»,
# а получил перечень форм и сроков. Первая часть отвечает на заданный
# вопрос честно — выбрать нельзя и почему, — и только после неё
# различия читаются как то, чем они и являются: ДОПОЛНЕНИЕ.
АПТ_ВЫБОР_ГЛАВНОЕ = (
    "От чего каждое средство — написано выше, под его карточкой, "
    "дословно из справочника. А вот что из этого подойдёт именно вам, "
    "по переписке не определить: это зависит от причины, от того, что "
    "вы уже принимаете, и от противопоказаний. Такой выбор — "
    "к фармацевту в аптеке или к врачу; перед приёмом сверьтесь "
    "с упаковкой и инструкцией. Выбирать за вас я не стану.")
АПТ_ВЫБОР_РАЗЛИЧИЯ = "Чем эти пачки различаются по карточкам:"
АПТ_ВЫБОР_НЕЧЕМ = (
    "По карточкам эти пачки ничем не различаются — ни формой, "
    "ни веществом, ни сроком, ни остатком.")
АПТ_ВСТУПЛЕНИЕ_ВЫБОР = "Отвечаю на «что из этого выбрать»."
...
def _апт_срок_словами(ym: str) -> str:
    """'2027-06' → '06.2027'. Пусто — пусто, а не выдуманная дата."""
    if not ym or len(ym) < 7:
        return ""
    return "%s.%s" % (ym[5:7], ym[:4])


def _апт_чем_различаются(строки, по_id) -> list:
    """Различия пачек — ИЗ КАРТОЧЕК, а не из памяти модели.

    Модель тут не участвует ни одним словом: каждая строка собрана
    из полей записи, которые владелец завёл сам либо сервер снял
    со страницы справочника. Ровно тот же довод, по которому подпись
    под карточкой собирает сервер (задача 203).

    ПЕЧАТАЕТСЯ ТОЛЬКО ТО, ЧТО ДЕЙСТВИТЕЛЬНО РАЗЛИЧАЕТСЯ. Строка
    «формы: таблетки — все три» различием не является и место
    занимает; такой перечень читался бы как повтор карточек, ради
    отсутствия которого блок B.5 и написан.
    """
    if len(строки) < 2:
        return []
    имя = {с["id"]: (по_id[с["id"]].get("name") or "").strip()
           for с in строки if с["id"] in по_id}
    итог = []

    def по_признаку(ключ, заголовок):
        куч = {}
        for с in строки:
            п = по_id.get(с["id"]) or {}
            з = (п.get(ключ) or "").strip()
            if not з:
                continue
            куч.setdefault(з, []).append(имя.get(с["id"], ""))
        if len(куч) > 1:
            итог.append("%s %s." % (заголовок, "; ".join(
                "%s — %s" % (з, ", ".join(и)) for з, и in куч.items())))

    по_признаку("форма_имя", "Формы разные:")
    по_признаку("substance", "Действующие вещества разные:")

    сроки = [(по_id[с["id"]].get("expires_ym") or "", имя.get(с["id"], ""))
             for с in строки if с["id"] in по_id]
    сроки = sorted((с for с in сроки if с[0]), key=lambda с: с[0])
    if len(сроки) > 1 and сроки[0][0] != сроки[-1][0]:
        итог.append("Раньше всех истекает %s — до %s; позже всех %s, до %s."
                    % (сроки[0][1], _апт_срок_словами(сроки[0][0]),
                       сроки[-1][1], _апт_срок_словами(сроки[-1][0])))

    остатки = []
    for с in строки:
        п = по_id.get(с["id"]) or {}
        if п.get("qty_left") is not None and п.get("qty_total"):
            остатки.append((float(п["qty_left"]), имя.get(с["id"], ""),
                            п["qty_left"], п["qty_total"]))
    остатки.sort()
    if len(остатки) > 1 and остатки[0][0] != остатки[-1][0]:
        # ФРАЗА БЕЗ ПАДЕЖА, и это не вкус: после «у» нужен родительный
        # («у Анальгина»), а склонять торговые названия нечем — тот же
        # довод, по которому подпись кнопки приёма считает СЕРВЕР
        # по готовым формам, а дату в §5.0.10 не согласуют с родом.
        # Поймал СНИМОК приёмки: на кадре стояло «осталось у Анальгин».
        итог.append("Меньше всего осталось: %s — %g из %g."
                    % (остатки[0][1], остатки[0][2], остатки[0][3]))
    return итог
...
# ── ОТБОР ПО ТЕКСТУ ПОКАЗАНИЙ, А НЕ ПО КАТЕГОРИИ (№244, D.1) ───────
#
# Владелец спросил «болит голова в висках, что выпить» и получил ВЕСЬ
# список категории «Голова и боль» без единого признака отбора; на
# «что из этого конкретно выбрать» — ТОТ ЖЕ список с приписками.
# Его слова: «мой вопрос не решается, он просто говорит — на, сам
# изучи ту же информацию».
#
# ЭТО ПОИСК ПО ТЕКСТУ, А НЕ СУЖДЕНИЕ МОДЕЛИ. Совпадение ищется
# в ДОСЛОВНОМ тексте показаний, снятом со страницы справочника,
# и рядом печатается ТА САМАЯ строка, где оно нашлось. Модель этот
# текст не читает, не пересказывает и в отборе не участвует: правило
# D.6 («не приписывать свойств, которых нет в извлечённом тексте»)
# не ослабляется ни на знак — наоборот, отбор теперь ОПИРАЕТСЯ
# на извлечённый текст, а не на память модели.

# Слова, которые есть в любой жалобе и ничего не сужают. Перечень тут
# законен: он закрывает ИСКЛЮЧЕНИЯ, а множество жалоб открыто (§6.0.7)
АПТ_СЛОВА_НЕ_В_СЧЁТ = {
    "болит", "боль", "болей", "болью", "очень", "сильно", "немного",
    "что", "чего", "выпить", "принять", "можно", "нужно", "надо",
    "мне", "меня", "себя", "есть", "дома", "аптечке", "лекарство",
    "таблетки", "какой", "какая", "какие", "лучше", "помочь", "помогает",
    "второй", "день", "дня", "уже", "после", "перед", "около",
}

# СКОЛЬКО ЗНАКОВ ОСНОВЫ. Пять — потому что русское окончание короче:
# «голова», «головная», «головы» сходятся на «голов». Четыре давали бы
# «горл» и «горе» одной основой, шесть теряли бы «висок»/«виски».
АПТ_ОСНОВА = 5


def _апт_основы_вопроса(вопрос: str) -> list:
    """ЧТО ИСКАТЬ В ТЕКСТЕ ПОКАЗАНИЙ — основы слов жалобы.

    Морфология здесь нужна самая грубая и НАМЕРЕННО грубая: «болит
    голова» обязано найти «головная боль», а склонять по-настоящему
    нечем и незачем. Основа — первые `АПТ_ОСНОВА` букв слова.
    """
    слова = re.findall(r"[^\W\d_]{3,}", (вопрос or "").lower(), re.UNICODE)
    основы = []
    for с in слова:
        if с in АПТ_СЛОВА_НЕ_В_СЧЁТ:
            continue
        о = с[:АПТ_ОСНОВА]
        if len(о) >= 4 and о not in основы:
            основы.append(о)
    return основы


def _апт_показание_по_вопросу(п, основы) -> str:
    """ТА САМАЯ СТРОКА ПОКАЗАНИЙ, где сошлось с вопросом, — или пусто.

    Цитата берётся ДОСЛОВНО из сохранённого текста и проверяется тем же
    инвариантом, что и остальные выдержки: не сошлось — молчим (§6.0.1).
    Предложение отдаётся целиком, а не обрезком вокруг слова: обрубок
    «...ой боли различного...» не говорит человеку ничего.
    """
    полный = (п.get("показания") or "").strip()
    if not полный or not основы:
        return ""
    for кусок in re.split(r"(?<=[;.])\s+", полный):
        низ = кусок.lower()
        if any(о in низ for о in основы):
            цитата = кусок.strip()
            if len(цитата) > АПТ_ПОКАЗАНИЕ_ЗНАКОВ:
                цитата = _апт_дозы.выдержка_показаний(
                    цитата, АПТ_ПОКАЗАНИЕ_ЗНАКОВ)
            if not _апт_дозы.выдержка_дословна(цитата, полный):
                print("[апт-запрос] цитата показаний позиции %s разошлась "
...
def _апт_показание_с_меткой(п, основы) -> tuple:
    """(текст «от чего», откуда он) — или пустая пара.

    ОТКУДА — ЧАСТЬ ОТВЕТА, А НЕ УКРАШЕНИЕ (BACKLOG №245, D.2). Запись
    с вкладыша и выдержка справочника выглядят одинаково и означают
    разное: у первой источник — коробка в руках владельца, у второй —
    страница, снятая такого-то числа. Без метки первая читалась бы
    как вторая — ровно то, что запретила задача 177.

    Справочник СПРАШИВАЕТСЯ ПЕРВЫМ: у него текст с датой снятия
    и с именем источника, то есть проверяемый. Своя запись — запасной
    путь ровно там, где справочник промолчал.
    """
    for текст, метка in _апт_тексты_от_чего(п):
        проба = dict(п)
        проба["показания"] = текст
        готовое = (_апт_показание_по_вопросу(проба, основы)
                   or _апт_показание_кратко(проба))
        if готовое:
            return готовое, метка
    return "", ""
...
#
def _апт_пробелы(текст: str) -> str:
    """Схлопнутые пробелы и нижний регистр — для сверки цитаты.

    ЭТО НЕ ОСЛАБЛЕНИЕ ДОСЛОВНОСТИ, и разница важная. Дословность
    запрещает менять СЛОВА и ЧИСЛА; переносы строк и двойные пробелы
    словами не являются, а в промпт текст уезжает уже схлопнутым
    (иначе одна позиция съедала бы полстроки списка). Сравнивай мы
    посимвольно с сохранённым — цитата не сошлась бы НИ РАЗУ, и
    группы не показывались бы никогда: немой отказ ровно того вида,
    ради которого сверка и заводится.
    """
    return " ".join((текст or "").split()).lower()


def _апт_тексты_от_чего(п) -> list:
    """ВСЕ тексты «от чего» у позиции: справочник и запись с вкладыша.

    ДВА ИСТОЧНИКА, И ОНИ РАВНОПРАВНЫ В ГРУППИРОВКЕ (BACKLOG №245, D.2).
    Различает их МЕТКА на экране, а не право участвовать: вкладыш
    у человека в руках, и его запись не менее дословна, чем страница
    справочника, — просто переписал её он, а не мы.

    Возвращает пары (текст, метка): метка нужна там, где выдержку
    показывают, — без неё запись человека читалась бы как выдержка
    справочника, ровно тот случай, что запретила задача 177.
    """
    пары = []
    справ = (п.get("показания") or "").strip()
    своё = (п.get("своё_от_чего") or "").strip()
    if справ:
        пары.append((справ, "справочник"))
    if своё:
        пары.append((своё, "вкладыш"))
    return пары


# ── ГРУППЫ ПО СМЫСЛУ ПОКАЗАНИЙ (BACKLOG №245, блок B) ────────────────
#
# ЧТО ЭТО МЕНЯЕТ. До этого захода ответ на жалобу СТРОИЛСЯ ОТБОРОМ:
# сервер искал основы слов вопроса в тексте показаний, и позиция,
# где написано то же самое другими словами, уезжала в кучу «дословно
# не сошлись». Живой прогон на аптечке владельца: на «болит голова
# в висках» сошлись ДВЕ карточки — и обе одна и та же пачка в двух
# упаковках, — а два десятка средств, среди которых были прямо
# от головной боли, оказались в куче «не сошлись».
#
# СЛОВАРЯ СИНОНИМОВ ЗАВОДИТЬ НЕЛЬЗЯ (решение владельца): под каждую
# жалобу — голова, живот, спазм, горло, спина — пришлось бы держать
# свой список форм слова и вечно его править. Это ровно тот перечень
# на открытом множестве, против которого написан §6.0.7.
#
# ЧТО РАЗРЕШЕНО МОДЕЛИ И ПОЧЕМУ ЭТО НЕ ОТМЕНА ЗАДАЧИ 203. Там ей
# запрещено ПРИПИСЫВАТЬ пачке свойства («спазмолитик при вздутии») —
# то есть добавлять к карточке то, чего в ней нет. Здесь она НИЧЕГО
# НЕ ДОБАВЛЯЕТ: раскладывает ДОСЛОВНЫЙ текст показаний, который сам
# же и получила, и на каждую группу обязана предъявить ЦИТАТУ из него.
#
# ТРИ ЗАСЛОНА, И КАЖДЫЙ СНИМАЕТ СВОЙ КЛАСС ВЫДУМКИ:
#
#   ЦИТАТА     — обязана найтись в показаниях позиции, ПОПАВШЕЙ В ЭТУ
#                группу. Не нашлась — группы нет, её позиции идут
#                общим списком. Это и есть доказательство, что группа
#                выведена из текста, а не из памяти модели;
#   ИМЯ        — проходит те же признаки, что вольный текст ответа:
#                след дозировки и чужое имя. Группа «Нурофен и аналоги»
#                отбрасывается ровно так же, как такая же фраза
#                в поле «нет»;
#   НОМЕРА     — только из посланного набора, как везде в этом ответе.
#
# ЧЕГО ЗАСЛОНЫ НЕ ЗАКРЫВАЮТ, И ЭТО НАЗВАНО: имя группы — слова модели,
# и совпадения имени с цитатой мы НЕ требуем. Требовать значило бы
# отвергать «Простуда и грипп» над текстом про ОРВИ, то есть верное
# обобщение своими словами. Цена ошибки тут мала: имя стоит НАД
# цитатой, из которой видно, о чём речь на самом деле.
def _апт_группы_из_ответа(сырой, по_id, разрешённые) -> tuple[list, dict]:
    """Проверенные группы и карта «id позиции → имя группы»."""
    сырые = сырой.get("группы")
    ...
    for г in сырые[:8]:
        if not isinstance(г, dict):
            continue
        имя = str(г.get("имя") or "").strip()[:60]
        цитата = str(г.get("цитата") or "").strip()[:400]
        ids = г.get("ids") if isinstance(г.get("ids"), list) else []
        номера = []
        for н in ids[:20]:
            try:
                н = int(н)
                ...
                continue
            # Позиция уже в другой группе — во вторую не кладём: одна
            # пачка в двух кучах читалась бы как две упаковки
            if н in по_id and н not in номера and н not in по_позиции:
                номера.append(н)
        if not имя or not номера:
            continue
        if _апт_след_дозы([имя]):
            print("[апт-группы] ОТБРОШЕНА группа %r: дозировка в имени" % имя)
            continue
        чужие = _апт_чужие_имена(имя, разрешённые)
        if чужие:
            print("[апт-группы] ОТБРОШЕНА группа %r: имена вне аптечки %r"
                  % (имя, чужие))
            continue
        # ЦИТАТА СВЕРЯЕТСЯ С ПОКАЗАНИЯМИ ЧЛЕНОВ ЭТОЙ ЖЕ ГРУППЫ.
        # Не «хоть какой-нибудь позиции»: цитата из чужого текста
        # доказывала бы ровно ничего — она подтверждала бы, что модель
        # умеет копировать, а не что группа выведена из этих пачек.
        низ = _апт_пробелы(цитата)
        # ЦИТАТА ИЩЕТСЯ И В ЗАПИСИ С ВКЛАДЫША (D.2): она такой же
        # дословный текст, просто переписал его человек.
        # ОТКУДА ОНА ВЗЯТА — ЗАПОМИНАЕТСЯ ЗДЕСЬ ЖЕ. Метка «Из
        # справочника» над цитатой, переписанной человеком с вкладыша,
        # была бы ровно той неправдой, которую запретила задача 177;
        # поймал это СНИМОК приёмки, а не чтение.
        свой, откуда = None, ""
        for н in номера:
            if not низ:
                break
            for текст, метка in _апт_тексты_от_чего(по_id[н]):
                if низ in _апт_пробелы(текст):
                    свой, откуда = н, метка
                    break
            if свой:
                break
        if not свой:
            print("[апт-группы] ОТБРОШЕНА группа %r: цитата не найдена "
                  "в показаниях её позиций" % имя)
            continue
        группы.append({"имя": имя, "цитата": цитата, "ids": номера,
                       "источник": свой, "откуда": откуда})
        for н in номера:
            по_позиции[н] = имя
    return группы, по_позиции
...
Верни ТОЛЬКО JSON без ```json и без пояснений:
{{"видно":"дословно всё, что напечатано на упаковке или сказано человеком","name":"торговое название","substance":"действующее вещество и его сила, например «Ибупрофен, 200 мг»","form":"один id из списка форм","unit":"один id единицы","qty_total":20,"qty_left":20,"volume":"","volume_unit":"","expires_ym":"ГГГГ-ММ","code_gtin":"","категории":["Голова и боль"],"вопрос":"","вид":"заведение"}}
...
ПРАВИЛА, нарушать нельзя:
0. Поле «видно» заполняй ПЕРВЫМ и дословно: выпиши подряд всё, что напечатано на упаковке (а если описание текстовое — что сказал человек). Ничего не добавляй от себя и ничего не переводи. Это выписка, а не пересказ.
1. Поля, которых в описании НЕТ, не выдумывай — оставь пустыми или убери из JSON. Пустое поле лучше выдуманного. Ты знаешь состав многих лекарств наизусть — это знание сюда не переносится: если вещество не напечатано на упаковке и не названо человеком, поле «substance» остаётся ПУСТЫМ, даже когда препарат тебе знаком.
...
5а. Форму выпуска бери ПО СЛОВУ из описания, если оно там названо прямо («порошок» — это powder, даже если порошок расфасован в пакетики). Догадываться о форме по упаковке можно только тогда, когда словом её не назвали. Если ни одна форма из списка не подходит к тому, что написано, — оставь «form» ПУСТЫМ и спроси в «вопрос». Ближайшую по смыслу подставлять нельзя: «капсулы» вместо жидкости для полоскания — выдумка, а не приближение.
5б. ЕДИНИЦА — ЭТО ТО, ЧЕМ ОТМЕРЯЮТ ОДИН ПРИЁМ, а не то, в чём измерен объём упаковки. У капель это drop (капля), у спрея dose (впрыскивание), у сиропа ml или spoon (мерная ложка), у таблеток tablet. Единица обязана быть из списка, разрешённого этой форме: {сочетания}
5в. qty_total — В ЕДИНИЦАХ ПРИЁМА. Если на упаковке названо и то и другое («флакон 10 мл, 350 капель»), то qty_total=350, unit=drop, а объём напиши отдельно: volume=10, volume_unit=«ml». Объём — справочная запись, в счёте он не участвует. Названо только «10 мл» и ничего про капли — тогда qty_total=10, unit=ml, volume оставь пустым.
...
def _апт_промпт(описание: str) -> str:
    # СОЧЕТАНИЯ «ФОРМА → ЕДИНИЦЫ» СОБИРАЮТСЯ ИЗ ТОГО ЖЕ `ФОРМЫ`,
    # что сторожит `_апт_проверить` (BACKLOG №238, A.3). Перечень
    # словами в тексте промпта разошёлся бы с проверкой молча,
    # и модель просили бы не о том, что принимает сервер: единица
    # вне формы там не отвергается, а МОЛЧА ЗАМЕНЯЕТСЯ первой
    # разрешённой — то есть выдумка проехала бы в карточку
    сочетания = "; ".join(
        "%s → %s" % (ф["id"], "/".join(ф["единицы"]))
        for ф in _апт_опр.ФОРМЫ if len(ф["единицы"]) > 1)
...
async def medkit_assist(request: Request,
    ...
    if вид.startswith("multipart/"):
        ...
        # а у машины 512 МБ (`fly.toml`, до 2026-09-05 было 256);
        # принимать произвольный объём
        ...
        try:
            сырой_фото = _extract_json(текст)
            черновик = _апт_очистить_черновик(сырой_фото)
        except Exception:
            print(f"[апт-ассистент] фото: ответ не разобрался: {текст[:200]!r}")
            return JSONResponse(
                {"error": "Ассистент ответил не так, как мы ожидаем."},
                status_code=502)
        # ── ЗАСЛОН ДОМЫСЛОВ (BACKLOG №232, A.2) ─────────────────────
        #
        # ИСТОЧНИК — ВЫПИСКА МОДЕЛИ ПЛЮС СЛОВА ЧЕЛОВЕКА. Своего OCR
        # у сервера нет, и выдумать его неоткуда; выписка же обходится
        # даром — она уже в том же ответе.
        #
        # ВЫПИСКА ПУСТА — ОТСЕВА НЕТ, И ЭТО СКАЗАНО В ЖУРНАЛ.
        # Отсеять всё при молчащей модели значило бы сломать
        # работающий путь ради её заминки; молча пропустить —
        # завести немой отказ внутри заслона. Замер частоты этого
        # исхода печатает `check_medkit_entry.py`
        выписка = str(сырой_фото.get("видно") or "").strip()
        if выписка:
            черновик, _ = _апт_отсеять_домыслы(
                черновик, выписка + " " + подпись)
        else:
            print("[апт-заведение] фото: выписки НЕТ — отсев домыслов "
                  "не делался, поля идут как есть")
        ...
        if адрес:
            описание, беда = await _сеть(db, _апт_текст_страницы(адрес))
            if беда:
                return JSONResponse({"error": беда}, status_code=400)
            if текст_ввода:
        ...
        # ── ОТВЕТ НА ЗАДАННЫЙ ВОПРОС (BACKLOG №238, B.1) ────────────
        #
        # Если прошлая реплика ассистента была черновиком С ВОПРОСОМ,
        # эта читается как ОТВЕТ на него. Без этой ветки «90» уходило
        # разбираться как самостоятельное описание упаковки — и честно
        # не разбиралось: замер до правки дал «вид: непонятно», а три
        # уже собранных поля терялись все три.
        #
        # ССЫЛКА СЮДА НЕ ПОПАДАЕТ: адрес страницы аптеки — это новое
        # описание упаковки, а не ответ на вопрос о прошлой.
        ждём, прошлый_вопрос = (
            _апт_ждём_ответа(db, user) if not адрес else (None, ""))
        if ждём:
            ...
            # НОВАЯ ТЕМА — НЕ ОТВЕТ, И РЕШАЕТ ЭТО МОДЕЛЬ (B.4).
            # `продолжение: false` роняет нас в обычный маршрут ниже:
            # человек, назвавший посреди диалога другой препарат,
            # обязан получить разбор ЭТОГО препарата, а не поле
            # к прошлой карточке.
            if сырой_у.get("продолжение") is not False:
                return _апт_чат_отдать(
                    db, user, текст_ввода,
                    _апт_дополнить_черновик(ждём, сырой_у, текст_ввода,
                                            db, user))
            print("[апт-заведение] ответ читается как НОВАЯ ТЕМА — "
                  "разбираем его с нуля")
        ...
        # ── ЗАСЛОН ДОМЫСЛОВ (BACKLOG №232, A.2) ─────────────────────
        #
        # У ТЕКСТА И ССЫЛКИ ИСТОЧНИК ЕСТЬ ЦЕЛИКОМ — это `описание`,
        # и выписка модели тут не нужна вовсе: сервер держит ввод
        # у себя. Замер до правки: домыслов по тексту 0 из 3, то есть
        # заслон здесь сторожит будущее, а не чинит настоящее, —
        # и стоит он в ОБЕИХ ветках именно поэтому. Ветка, оставленная
        # без него «потому что там и так чисто», разошлась бы с первой
        # на первой же правке промпта (§6.0.7)
        черновик, _ = _апт_отсеять_домыслы(черновик, описание)
    ...
    # ОТКУДА ВЗЯЛОСЬ ВЕЩЕСТВО — ПЕРЕВОДИТ СЕРВЕР (BACKLOG №234, A.3).
    #
    # `источник` говорит про способ ВВОДА («фото», «ссылка», «скан»),
    # а на карточке нужна другая вещь — происхождение ЗНАЧЕНИЯ. Перевод
    # лежит в `medkit_defs`, и второго словаря в браузере быть не должно:
    # разойдясь, он поставил бы под карточкой «прочитано с упаковки»
    # там, где значение пришло со слов (§6.0.7).
    итог["вещество_откуда"] = (
        _апт_опр.ЧЕРНОВИК_К_ИСТОЧНИКУ.get(источник, "")
        if итог["поля"].get("substance") else "")
...
async def medkit_page(request: Request, user=Depends(get_current_user),
    ...
    return templates.TemplateResponse(
        ...
        context={"user": user, "формы": _апт_опр.ФОРМЫ,
                 ...
                 # ЕДИНИЦЫ ОБЪЁМА — ОТДЕЛЬНЫЙ СПИСОК, а не подмножество
                 # единиц приёма (BACKLOG №238, A.1): «мл» как объём
                 # флакона и «мл» как разовая доза сиропа — разные роли
                 # одного слова, и общий список склеил бы их обратно
                 "единицы_объёма": _апт_опр.ЕДИНИЦЫ_ОБЪЁМА,
...
async def medkit_create(request: Request, фон: BackgroundTasks,
    ...
    # ЭКРАН УЗНАЁТ, ЧТО ПОИСК ПОШЁЛ (BACKLOG №245, E.1). Замер:
    # ответ 25 мс, схема в базе через ~10 с — карточка на экране
    # оставалась без неё, и человек жал кнопку, делавшую то же
    # самое второй раз. Говорит СЕРВЕР: догадка браузера по полям
    # карточки была бы вторым мнением о том, идёт ли поиск
    return JSONResponse({**_апт_ответ_сетки(request, db, user),
                         "id": новый, "ищем_схему": новый})
...
async def medkit_update(item_id: int, request: Request, фон: BackgroundTasks,
    ...
    return JSONResponse({**_апт_ответ_сетки(request, db, user),
                         "ищем_схему": (item_id if сопоставление_поехало
                                        else None)})
...
async def medkit_dosage(item_id: int, request: Request,
    ...
    if not поз:
        return JSONResponse({"error": "Не найдено"}, status_code=404)
    # ═══════════════════════════════════════════════════════════════
    # СОЕДИНЕНИЕ ОТДАЁТСЯ ПЕРЕД ПОХОДОМ В ЧУЖУЮ СЕТЬ (§6.0.6)
    # ═══════════════════════════════════════════════════════════════
    #
    # BACKLOG №207, второе место того же класса. Здесь стояло «объект
    # переживёт await, но полагаться на это незачем» — верно про ОБЪЕКТ
    # и молчит про СОЕДИНЕНИЕ, а держалось оно всё время похода
    # к чужому сайту: до семи страниц по 90 с у второго источника,
    # то есть МИНУТЫ открытой транзакции чтения.
    #
    # Под WAL это мешает контрольной точке; замер на стенде дал три
    # строки «database is locked» у фоновой задачи, которая не смогла
    # записать собственный итог.
    #
    # СНИМОК ПОЛЕЙ, потом `db.close()`, потом сеть: после закрытия
    # объект ORM отцеплен, и читать его поля — то самое молчаливое
    # поведение, которое §6.0.5 разбирает на записи.
    искомое = _АптИскомое(поз)
    db.close()
    итог, причина, вид, подсказка = await _апт_найти_дозы(искомое)
    # Объект ПЕРЕСПРАШИВАЕМ: между запросом и записью прошли секунды,
    # и соединение мы отдавали — сессия открывает его заново сама
    поз = _апт_позиция(db, user, item_id)
    if not поз:
        return JSONResponse({"error": "Не найдено"}, status_code=404)
    ...
    if not итог:
        ...
        return JSONResponse({**_апт_ответ_сетки(request, db, user),
                             ...
                             # ПРОПУСК ПО ВРЕМЕНИ — ПОЛЕМ, А НЕ ПО ТЕКСТУ
                             # ПРИЧИНЫ (BACKLOG №241, C). Обход обязан
                             # ПОЙТИ ДАЛЬШЕ и СКАЗАТЬ про эту позицию;
                             # разбирать причину по началу строки нельзя
                             # по тому же доводу, что у `повтор_поможет`
                             # и у `предел` строкой выше
                             "пропуск_по_времени": вид == ДОЗЫ_ВИД_ВРЕМЯ})
    return JSONResponse(_апт_ответ_сетки(request, db, user))
...
# ПОТОЛОК СВОИХ ПОКАЗАНИЙ. Своё число, а не общее со схемой: раздел
# «показания» у Видаля короче раздела о дозировании — замер по боевой
# базе даёт медиану 340 знаков при максимуме 1014, — и общий потолок
# пропускал бы сюда вдвое больше того, что бывает у показаний вообще.
АПТ_СВОИ_ПОКАЗАНИЯ_ЗНАКОВ = int(os.getenv("MEDKIT_OWN_IND_CHARS", "1200"))
...
async def medkit_substance_from_ref(item_id: int, request: Request,
    ...
    # ПРОИСХОЖДЕНИЕ ПИШЕТСЯ ЗДЕСЬ ЖЕ, А НЕ ПРИХОДИТ ОТ КЛИЕНТА
    # (BACKLOG №234, A.3): значение поставил СЕРВЕР из справочника,
    # и спрашивать браузер, откуда оно, было бы вопросом не по адресу.
    поз.substance_src = "справочник"
    поз.updated_at = datetime.utcnow()
    # ВЫДЕРЖКА НЕ ТРОГАЕТСЯ, ЕСЛИ ОНА ЕСТЬ (BACKLOG №244, A.2).
    # Разбор и замер — у `_апт_подстановка_из_справочника`
    искать_заново = _апт_подстановка_из_справочника(поз)
    # ИСПОЛНЕННАЯ ПОДСКАЗКА СНИМАЕТСЯ. Соседнюю (форму) не трогаем:
    # она отвечает на свой вопрос и вписана не была
    поз.dosage_hint_substance = None
    db.commit()
    if искать_заново:
        фон.add_task(_апт_дозы_фоном, поз.user_id, item_id)
...
@app.post("/medkit/api/items/{item_id}/form-from-ref")
async def medkit_form_from_ref(item_id: int, request: Request,
                               фон: BackgroundTasks,
                               user=Depends(get_current_user),
                               db: Session = Depends(get_db)):
    """ВПИСАТЬ ФОРМУ ВЫПУСКА ТАК, КАК ЕЁ НАЗЫВАЕТ СПРАВОЧНИК.

    BACKLOG №239, блок B. Брат-близнец `medkit_substance_from_ref`,
    и заведён по той же причине: справочник знает про выпуск больше,
    чем успел записать человек, а переносить это руками в другом
    окне он не станет.

    ═══════════════════════════════════════════════════════════════════
    ТОЛЬКО ПО НАЖАТИЮ, И ПРЕЖНЕЕ ЗНАЧЕНИЕ ВИДНО ДО НЕГО (B.2)
    ═══════════════════════════════════════════════════════════════════

    Молча не подставляем НИКОГДА, даже верное: форма — поле карточки
    ВЛАДЕЛЬЦА, и правка её задним числом означала бы, что данные
    в аптечке меняются без его ведома. Что именно подставится, экран
    показывает ДО нажатия: он получает и код (`форма_подсказка`),
    и слово справочника (`форма_подсказка_слово`), и нашу подпись
    к этой форме (`форма_подсказка_подпись`).

    ═══════════════════════════════════════════════════════════════════
    ЗАСЛОН ДОМЫСЛОВ НЕ ОСЛАБЛЯЕТСЯ (B.5)
    ═══════════════════════════════════════════════════════════════════

    Значение берётся СО СТРАНИЦЫ СПРАВОЧНИКА, а не из памяти модели:
    подсказку записал `_апт_подсказки_страницы` из той страницы,
    которую выбрало сопоставление. Модель этого пути не касается
    ни в одной точке.

    БЕЛЫЙ СПИСОК НА ВЫХОДЕ, А НЕ ДОВЕРИЕ ПОДСКАЗКЕ. Код формы обязан
    быть одним из объявленных (`medkit_defs.ФОРМА_ПО_ID`): подсказка
    лежит в базе, а база правится и мимо этого кода.

    ═══════════════════════════════════════════════════════════════════
    ЕДИНИЦА ПРИЁМА СЛЕДУЕТ ЗА ФОРМОЙ, И БЕЗ ЭТОГО ПРАВКА ЛОМАЕТ СЧЁТ
    ═══════════════════════════════════════════════════════════════════

    У каждой формы свой набор единиц (§5.8, A.1): смени форму
    с «капли» на «раствор», не тронув единицу, — и остаток начнёт
    считаться в каплях у того, что мерится миллилитрами. Поэтому
    единица приводится к допустимой для НОВОЙ формы, а если прежняя
    в новый набор входит — остаётся как была.
    """
    if not user:
        return JSONResponse({"error": "Нужно войти"}, status_code=401)
    поз = _апт_позиция(db, user, item_id)
    if not поз:
        return JSONResponse({"error": "Не найдено"}, status_code=404)
    код = (поз.dosage_hint_form or "").strip()
    if not код:
        return JSONResponse(
            {"error": "Справочник не называл форму выпуска для этой "
                      "позиции однозначно — вписывать нечего"},
            status_code=400)
    опис_формы = _апт_опр.ФОРМА_ПО_ID.get(код)
    if not опис_формы:
        # ПОДСКАЗКА ИЗ БАЗЫ ПРОВЕРЯЕТСЯ, А НЕ ПРИНИМАЕТСЯ НА ВЕРУ
        return JSONResponse(
            {"error": "Форма из справочника не опознана"}, status_code=400)
    было = поз.form
    было_имя = (_апт_опр.ФОРМА_ПО_ID.get(было) or {}).get("имя", было or "")
    поз.form = код
    # ЕДИНИЦА ПРИЁМА — ЗА ФОРМОЙ. Разбор выше
    допустимые = опис_формы.get("единицы") or []
    if допустимые and поз.unit not in допустимые:
        поз.unit = допустимые[0]
    # ПРОИСХОЖДЕНИЕ ПИШЕТ СЕРВЕР: значение поставил он из справочника
    поз.form_src = "справочник"
    поз.updated_at = datetime.utcnow()
    # ВЫДЕРЖКА НЕ ТРОГАЕТСЯ, ЕСЛИ ОНА ЕСТЬ (BACKLOG №244, A.2):
    # подсказка формы снята С ТОЙ ЖЕ страницы, что и выдержка
    искать_заново = _апт_подстановка_из_справочника(поз)
    поз.dosage_hint_form = None
    поз.dosage_hint_form_word = None
    db.commit()
    if искать_заново:
        фон.add_task(_апт_дозы_фоном, поз.user_id, item_id)
    ответ = _апт_ответ_сетки(request, db, user)
    ответ["было_форма"] = было_имя
    ответ["стало_форма"] = опис_формы.get("имя", код)
    return JSONResponse(ответ)
...
@app.post("/medkit/api/items/{item_id}/own-indications")
async def medkit_own_indications(item_id: int, request: Request,
                                 user=Depends(get_current_user),
                                 db: Session = Depends(get_db)):
    """СВОЁ «ОТ ЧЕГО» — ТО, ЧТО НАПЕЧАТАНО НА ЭТОМ ВКЛАДЫШЕ.

    BACKLOG №245, блок D. Схему приёма вписать руками было можно
    с задачи 177, показания — НИЧЕМ: у владельца лежит коробка
    с вкладышем, а внести оттуда «от чего» некуда.

    ЦЕНА НЕ В УДОБСТВЕ, И ЭТО ЗАМЕР. Позиция без показаний в группы
    ответа ассистента не попадает ПО ПОСТРОЕНИЮ — группа обязана
    предъявить цитату, а цитировать нечего. По боевой базе таких
    37 из 77, то есть почти половина аптечки выпадала из ответа
    целиком, и вернуть её было нечем.

    УСТРОЙСТВО ПОВТОРЯЕТ `own-dosage` ДОСЛОВНО, и это не совпадение:
    вопрос тот же — «своё рядом со справочным, а не вместо него», —
    и второго ответа на него в проекте быть не должно. Отдельная
    колонка, метка источника на экране, пустая строка стирает запись,
    дата двигается только вместе с текстом.

    ЧЕГО ЗДЕСЬ НЕТ: разбора, сокращения и приведения к виду. Человек
    переписывает с вкладыша как есть — любое наше преобразование было
    бы пересказом, а пересказ показаний ничем не лучше пересказа
    дозировки (§5.8, D.1).
    """
    if not user:
        return JSONResponse({"error": "Нужно войти"}, status_code=401)
    ...
    данные = await request.json()
    текст = (данные.get("текст") or "").strip()
    if len(текст) > АПТ_СВОИ_ПОКАЗАНИЯ_ЗНАКОВ:
        return JSONResponse(
            {"error": "Запись длиннее %d знаков — это уже не «от чего». "
                      "Оставьте только то, при каких состояниях принимают."
                      % АПТ_СВОИ_ПОКАЗАНИЯ_ЗНАКОВ}, status_code=400)
    if текст != (поз.own_indications_text or ""):
        поз.own_indications_text = текст or None
        поз.own_indications_at = datetime.utcnow() if текст else None
        поз.updated_at = datetime.utcnow()
    db.commit()
    return JSONResponse(_апт_ответ_сетки(request, db, user))

...
def _апт_сгруппировать(позиции):
    ...
    for к in порядок:
        ...
        # осталось» там было бы выдумкой; смешивать штуки с миллилитрами
        # тоже нельзя, но этого и не случится — единица входит в форму,
        # а форма в ключ группы
        ...
        сводка["qty_left"] = sum(числа) if числа else None
        сводка["qty_total"] = sum(всего) if всего else None
        ...
        #   code_gtin     НЕ заимствуется: GTIN опознаёт УПАКОВКУ,
        #                 а не препарат, — у «№20» и «№30» он разный
        #                 при одном ключе группы. Подставив чужой,
        #                 мы привязали бы к пачке чужой штрих-код

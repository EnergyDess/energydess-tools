...
CSS = [p for p in CSS if p not in ВНЕ]
    ...
    for i, line in enumerate(t.split("\n"), 1):
        for m in re.finditer(r"(?<![\w-])([a-z-]+)\s*:\s*([^;{}]+)", line):
            ...
            for в, (pp, rx) in ВИДЫ.items():
                if not pp.match(prop): continue
                if в == "тень":
                    vv = v.strip()
                    if vv in ("none", "V", "inherit", "") or re.fullmatch(r"[V ,]+", vv): continue
                    if not (ЦВЕТ.search(vv) or ЧИСЛО.search(vv)): continue
                    hits = 1
...
def где(кл):
    return sorted(os.path.basename(p) for p, t in шабл_текст.items()
                  if re.search(r"(class=\"[^\"]*|classList\.\w+\(\s*['\"]|['\" ])" + re.escape(кл) + r"(?![\w-])", t))
P("")

...
Пропорции исходника сохраняются: фото на странице вертикальное, кроп
в квадрат срезал бы руки с зажигалкой.
"""
import os
import sys

ВЫХОД = os.path.join("static", "about-me.jpg")
# Вертикальное фото: пропорции исходника сохраняются, кроп не делается —
# в кадре важны и лицо, и руки с зажигалкой. Ширина 620 под показ 300px
# с двукратным запасом на retina
ШИРИНА = 620
МАКС_ВЫСОТА = 1200
...
def main():
    ...
    try:
        ...
        # Пропорции сохраняем: квадратный кроп срезал бы руки с зажигалкой,
        # и огонёк в кадре оставался бы без объяснения
        высота = round(img.height * ШИРИНА / img.width)
        if высота > МАКС_ВЫСОТА:
            ШИРИНА_ПОДГОН = round(img.width * МАКС_ВЫСОТА / img.height)
            img = img.resize((ШИРИНА_ПОДГОН, МАКС_ВЫСОТА), Image.LANCZOS)
        else:

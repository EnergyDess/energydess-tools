...
требований):
  ...
  3. ОКНО СПОСОБА ПРИЁМА для ПРЕПАРАТЫ В СИРОПА — то есть тот исход блока
...
покупок пуст, переписки нет, а «Препарат В сироп» в аптечке стенда нет
...
def _завести_состояние():
    """Четыре вида строк покупок, двухдневная переписка и сироп вещество В."""
    _прибрать()
    ...
    строки = [
        ...
        ("Препарат Г 10000", "Панкреатин, 10000 ЕД · Капсулы · истёк 03.2026",
         "expired", "Панкреатин, 10000 ЕД", "capsule", "capsule", None),
        ("Препарат Б", "Тримебутин, 200 мг · осталось 5 из 30", "low",
    ...
    # СИРОП ПРЕПАРАТЫ В ЗАВОДИТСЯ НЕ ЗДЕСЬ, А ЧЕРЕЗ ПРИЛОЖЕНИЕ (`_завести_сироп`):
...
async def _завести_сироп(pg):
    """«Препарат В сироп» — ЧЕРЕЗ ФОРМУ, чтобы отработал фоновый поиск.
    ...
    ответ = await pg.evaluate("""async (имя) => {
      const т = await fetch('/medkit/api/items', {method: 'POST',
        ...
        body: JSON.stringify({name: имя, substance: 'Препарата В',
...
async def кадры(pg, ширина, id_сиропа):
    ...
    # ── 3 · ОКНО СПОСОБА ПРИЁМА ДЛЯ СИРОПА ПРЕПАРАТЫ В ──────────────────
    #

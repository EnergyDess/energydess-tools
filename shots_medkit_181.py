...
Что снимается — ровно то, что просил пункт 5 постановки:
  ...
  1. ОКНО СПОСОБА ПРИЁМА ДЛЯ НЕОБУТИНА — тот кадр, ради которого
     заведён блок A: схема «по 100-200 мг 3 раза/сут» на карточке
     обычных таблеток 200 мг, которой владелец не мог получить;
  2. ОКНО ДЛЯ СОЛОДКИ СИРОПА — исход, где название и форма сошлись,
...
Ни «Необутина», ни «Солодки сиропа» в аптечке стенда нет, список
...
async def кадры(pg, ширина):
    ...
    # ── 1 · НЕОБУТИН: ТОТ САМЫЙ КАДР БЛОКА A ────────────────────────
    ид_н = await _завести(pg, НЕОБУТИН, "Тримебутин, 200 мг",
                          "tablet", "tablet", 1)
    if ид_н:
        await _окно(pg, ид_н, "okno-neobutin", ширина)

    # ── 2 · СОЛОДКИ СИРОП ───────────────────────────────────────────
    ид_с = await _завести(pg, СИРОП, "Солодки корней экстракт густой, 4 г",
                          "syrup", "ml", 5)
    if ид_с:

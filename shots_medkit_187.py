...
ЧТО СНИМАЕТСЯ И ЗАЧЕМ ИМЕННО ЭТО:
  ...
  схлопнутая      главный случай D.6 — две пачки Препарата А от РАЗНЫХ
...
def снять(b, ширина, выход):
    ...
    группа = стр.query_selector('.apt-card[data-find*="препарат А"]')
    if группа:
        группа.scroll_into_view_if_needed()
        стр.wait_for_timeout(400)
        кадр(стр, "группа-препарат А", ширина, '.apt-card[data-find*="препарат А"]')
...
def выйти_и_снять(b, ширина):
    ...
    кадр(стр, "после-выхода-препарат А", ширина,
         '.apt-card[data-find*="препарат А"]')
    ctx.close()

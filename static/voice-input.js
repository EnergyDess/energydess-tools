...
/* Распознавание. Адрес ОДИН на всех потребителей: второй эндпоинт
   ...
   ── ОТВЕТ РАЗЛОЖЕН НА ТРИ ИСХОДА (BACKLOG №246, блок F) ────────────

   ЗДЕСЬ ВОЗВРАЩАЛАСЬ ГОЛАЯ СТРОКА, и это был НЕМОЙ ОТКАЗ ровно того
   вида, ради которого написан §6.0. Сервер сочиняет ЧЕЛОВЕЧЕСКИЙ
   текст с советом — «сервис не ответил, попробуйте через минуту»
   против «сбой на нашей стороне, повтор не поможет» (§2.5, семь
   случаев задачи 128 и правка 245 E.3), — а `д.text` у такого тела
   не существует вовсе, и функция отдавала ПУСТУЮ СТРОКУ. Оба
   потребителя показывали на неё одну и ту же фразу «Не удалось
   распознать речь».

   ДОКАЗАНО ЖИВЬЁМ 2026-09-08: приложение поднято с адресом сервиса
   на закрытый порт, сервер ответил HTTP 503 и текстом «Не удалось
   связаться с сервисом распознавания речи. Попробуйте ещё раз через
   минуту — или наберите текстом», а `голосВТекст` вернула `''`.
   То есть вся работа задачи 128 по семи текстам сбоя до человека
   НЕ ДОЕЗЖАЛА НИ РАЗУ.

   ИСХОДА ТРИ, И ТРЕТИЙ НЕ РАВЕН ВТОРОМУ. «Сервис отказал» и «сервис
   ответил, но не расслышал» — разные факты с разными советами:
   первый чинится повтором или текстом, второй — переписать ближе
   к микрофону. Замер того же дня: тишина, запись 0.1 с и обрубленный
   файл дают HTTP 200 с ПУСТЫМ текстом, то есть второй случай
   наступает штатно и ошибкой не является. */
async function голосВТекст(blob) {
  ...
  try {
    ...
    if (д && д.error) return {текст: '', ошибка: д.error};
    return {текст: (д.text || '').trim(), ошибка: ''};
  } catch (e) {
    /* Сеть не дала даже тела: сказать нечего, кроме того, что связи
       не было. Молчать нельзя — это и есть немой отказ */
    return {текст: '', ошибка: 'Связь прервалась, пока шла расшифровка. '
                              + 'Попробуйте ещё раз или наберите текстом.'};
  }
}

/* ЧТО СКАЗАТЬ ЧЕЛОВЕКУ. Одно место на обоих потребителей: две копии
   разошлись бы, и фраза про пустую запись встала бы там, где сервис
   отказал (§6.0.7). */
const ГОЛОС_ПУСТО = 'Речь не распозналась — возможно, было слишком тихо '
  + 'или запись слишком короткая. Попробуйте ещё раз или наберите текстом.';

function голосСказать(итог) {
  return итог.ошибка || ГОЛОС_ПУСТО;
}

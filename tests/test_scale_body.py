...
def test_возраст_тела_идёт_с_возрастом_из_анкеты():
    ...
    С 2026-08-19 рядом стоит и РАЗНИЦА: два числа рядом всё равно
    вычитают, и делать это в уме человек не должен. Строку собирает
    `body_scales._сравнение_возраста` — здесь проверяется, что она
    доезжает до блока целиком, а не что она верна (её собственные
    случаи, включая склонение и «меньше», — в test_body_scales)."""
    записи = [Запись("2026-08-19", weight_kg=99.4, body_age=40)]
    блок = main._блок_состава(записи, Анкета(age=28))
    assert блок["показатели"][0]["сравнение"] == "в анкете — 28: на 12 лет больше"
